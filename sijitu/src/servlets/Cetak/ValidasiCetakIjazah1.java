...
/**
 * Servlet implementation class ValidasiCetakIjazah1
 */
@WebServlet("/ValidasiCetakIjazah1")
public class ValidasiCetakIjazah1 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ValidasiCetakIjazah1() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(Checker.isStringNullOrEmpty(nmmija)) {
			valid=false;
			tknError = tknError+"Nama mahasiswa harus diisi,";
		}
		String nmmjen = request.getParameter("namaJen");
		if(Checker.isStringNullOrEmpty(nmmjen)) {
			valid=false;
			tknError = tknError+"Gelar Program harus diisi,";
		}
		String nimija = request.getParameter("nimija");
		if(Checker.isStringNullOrEmpty(nimija)) {
			valid=false;
			tknError = tknError+"NIM mahasiswa harus diisi,";
		}
		String tplhrIja = request.getParameter("tplhrIja");
		if(Checker.isStringNullOrEmpty(tplhrIja)) {
			valid=false;
			tknError = tknError+"Tempat kelahiran mahasiswa harus diisi,";
		}
		...
		try {
			java.sql.Date dtTmp = java.sql.Date.valueOf(tglhrIja);
		}
		catch(Exception e) {
			tglhrIja = null;
		}
		if(Checker.isStringNullOrEmpty(tglhrIja)) {
			valid=false;
			tknError = tknError+"Tanggal lahir mahasiswa harus diisi,";
		}
		String noSeriIja = request.getParameter("noSeriIja");
		String noSeriNirl = request.getParameter("noSeriNirl");
		/*
		 * ngga butuh automatik utk no nirl dan noija
		 
		if(Checker.isStringNullOrEmpty(noSeriIja)) {
			valid=false;
			tknError = tknError+"Nama mahasiswa harus diisi,";
		}
		String noSeriNirl = request.getParameter("noSeriNirl");
		if(Checker.isStringNullOrEmpty(noSeriNirl)) {
			valid=false;
			tknError = tknError+"Nama mahasiswa harus diisi,";
		}
		*/
		...
		if(Checker.isStringNullOrEmpty(noSkr)) {
			valid=false;
			tknError = tknError+"No SK Rektor harus diisi,";
		}
		String tglre = request.getParameter("tglre");
		if(Checker.isStringNullOrEmpty(tglre)) {
			valid=false;
			tknError = tknError+"Tanggal SK Rektor harus diisi,";
		}
		
		if(valid) {
			String target = Constants.getRootWeb()+"/InnerFrame/konfirmasiCetakIjazah.jsp";
			String uri = request.getRequestURI();
			String url_ff = PathFinder.getPath(uri, target);
			request.getRequestDispatcher(url_ff+"?id_obj="+objId+"&nmm="+v_nmmhs+"&npm="+v_npmhs+"&obj_lvl="+v_obj_lvl+"&kdpst="+v_kdpst+"&cmd=percetakan").forward(request,response);	
		}
		else {
			//String target = Constants.getRootWeb()+"/InnerFrame/indexCetakIjazah.jsp";
			//System.out.println(target);
			//String uri = request.getRequestURI();
			//System.out.println(uri);
			//String url_ff = PathFinder.getPath(uri, target);
			//System.out.println(url_ff);
			//request.getRequestDispatcher("go.cetakIjazah?id_obj="+objId+"&nmm="+v_nmmhs+"&npm="+v_npmhs+"&obj_lvl="+v_obj_lvl+"&kdpst="+v_kdpst+"&cmd=percetakan&tknError="+tknError).forward(request,response);
			response.sendRedirect("go.cetakIjazah?id_obj="+objId+"&nmm="+v_nmmhs+"&npm="+v_npmhs+"&obj_lvl="+v_obj_lvl+"&kdpst="+v_kdpst+"&cmd=percetakan&tknError="+tknError);
		}
