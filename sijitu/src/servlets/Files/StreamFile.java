 ...
 *
 * @author BalusC
 * @link http://balusc.blogspot.com/2009/02/StreamFile-supporting-resume-and.html
 */
public class StreamFile extends HttpServlet {
    ...
    public void init() throws ServletException {
    	...
    	/*
    	this.basePath = Constants.getRootFolderFilesTutorial();

        // Validate base path.
        if (this.basePath == null) {
            throw new ServletException("StreamFile init param 'basePath' is required.");
        } else {
            File path = new File(this.basePath);
            if (!path.exists()) {
                throw new ServletException("StreamFile init param 'basePath' value '"
...
/*    
    public void init(String basedFolder, String nm_file) throws ServletException {
...
*/
            ...
            throws IOException
    {
		...
		this.basePath = new String(alm);
        //System.out.println("alm="+alm);
        String nm_file = request.getParameter("namaFile");
        while(nm_file.contains("~DAN~")) {
        	nm_file = nm_file.replace("~DAN~", "&");
		}
		while(nm_file.contains("~OR~")) {
			nm_file = nm_file.replace("~OR~", "/");
		}
		//System.out.println("target_nm_folder="+target_nm_folder);
		while(nm_file.contains("//")) {
			nm_file = nm_file.replace("//","/");
		}
		if(nm_file.endsWith("/")) {
			nm_file = nm_file.substring(0,nm_file.length()-1);
		}
		if(!nm_file.startsWith("/")) {
			nm_file = "/"+nm_file;
        }
        //System.out.println("nm_file="+nm_file);
        String requestedFile = new String(nm_file);
