 ...
 */
@WebServlet("/BukuTamu")
public class BukuTamu extends HttpServlet {
...
//		}
		...
		if(all_valid) {
    		...
    		String IDPAKETBEASISWA = "1";//default
    		String NPM_PA = "null";
    		String NMM_PA = "null";
			...
			sukses = udb.insertBukuTamu(tkn_info_civitas_tanpa_kdpti_kdpst_kdjen_npmhs, tkn_info_ext_civitas_tanpa_kdpst_npmhs);
	...
	//	}	
		...
		if(isu==null) {
			...
			url = PathFinder.getPath(uri, target);
			if(sukses) {
				request.getRequestDispatcher(url+"?nuform=true&valid_pt1=false&err_msg="+err_msg).forward(request,response);
			}
			else {
				request.getRequestDispatcher(url+"?valid_pt1="+valid_pt1+"&valid_pt2="+valid_pt2+"&err_msg="+err_msg).forward(request,response);
			}
		}
	}
