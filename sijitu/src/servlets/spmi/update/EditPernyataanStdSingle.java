...
/**
 * Servlet implementation class UbahUsulanJadiStandarSingle
 */
@WebServlet("/UbahUsulanJadiStandarSingle")
public class EditPernyataanStdSingle extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public EditPernyataanStdSingle() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//kode here
			...
			//System.out.println("masuk part1");
			String tombol = request.getParameter("tombol");
			String id_std_1 = request.getParameter("id_std");
			//System.out.println("id_std submit="+id_std_1);
			...
			else if(tombol!=null && tombol.equalsIgnoreCase("param_terkait")) {
				String target = Constants.getRootWeb()+"/InnerFrame/Spmi/Parameter/form_tambah_param.jsp";
    			String uri = request.getRequestURI();
    			String url = PathFinder.getPath(uri, target);
    			request.getRequestDispatcher(url+"?atMenu=form&backTo=mutu").forward(request,response);
    			//System.out.println("submit="+tombol);
		...
		//PrintWriter out = response.getWriter();
				//System.out.println("masuk part2");
				Vector v_err = null;
				...
				String strategi = ""+request.getParameter("strategi");
				String job = ""+request.getParameter("job");
				String job_mon = ""+request.getParameter("job_mon");
				...
				*/
				...
				if(Checker.isStringNullOrEmpty(strategi)) {
					...
					if(strategi.length()<15) {
						if(v_err==null) {
							v_err = new Vector();
							li = v_err.listIterator();
						}
						li.add("Strategi pelaksanaan minimal terdiri dari 15 char");
					}
				}
				/*
				String tipe_proses_pengawasan = request.getParameter("tipe_proses_pengawasan");
				...
				*/
	    		...
	    		if(v_err!=null && v_err.size()>0) {
	    			...
	    			//System.out.println("periode_unit_used="+periode_unit_used);
	    			...
	    			int updated = ur.updateIsiStandarSingle(Integer.parseInt(id_std),id_std_isi,rasionale,isi_std,periode_unit_used,lama_per_period,periode_awal,target1,unit1,target2,unit2,target3,unit3,target4,unit4,target5,unit5,target6,unit6,tkn_variable,tkn_doc,id_versi,pihak,pihak_mon,tkn_indikator,cakupan_std,tipe_proses_pengawasan,strategi);
	    			//System.out.println("updated "+updated);
	    			String backTo = request.getParameter("backTo");
	    			request.getRequestDispatcher("go.getListAllStd?mode=start&id_tipe_std="+id_tipe_std+"&id_master_std="+id_master_std+"&atMenu=&at_menu=nama_std&kdpst_nmpst_kmp="+kdpst_nmpst_kmp).forward(request,response);
