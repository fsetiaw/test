...
/**
 * Servlet implementation class UbahUsulanJadiStandar_v1
 */
@WebServlet("/UbahUsulanJadiStandar_v1")
public class UbahUsulanJadiStandar_v1 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UbahUsulanJadiStandar_v1() {
        super();
		...
		//PrintWriter out = response.getWriter();
				...
				*/
				...
				if(Checker.isStringNullOrEmpty(unit1)) {
					...
					li.add("Unit yang digunakan pada Target Besaran Periode I harus diisi");
				}
				...
				*/
	    		...
	    		if(v_err!=null && v_err.size()>0) {
	    			...
	    			String target = Constants.getRootWeb()+"/InnerFrame/Spmi/standar_creator/form/std_isi/edit_index_v1.jsp";
	    			String uri = request.getRequestURI();
	    			String url = PathFinder.getPath(uri, target);
	    			//System.out.println("?mode=edit&id_master="+id_master+"&id_tipe="+id_tipe+"&atMenu="+atMenu+"&tkn_pengawas="+pihak_mon);;
	    			request.getRequestDispatcher(url+"?mode=edit&id_master="+id_master+"&id_tipe="+id_tipe+"&atMenu="+atMenu+"&tkn_pengawas="+pihak_mon).forward(request,response);
	    			...
	    			int updated = ur.updateParameterDanIndikatorStandar(target_kdpst, id_std, id_std_isi, isi_std, periode_unit_used, lama_per_period, periode_awal, target1, unit1, target2, unit2, target3, unit3, target4, unit4, target5, unit5, target6, unit6, tkn_variable, versi, norut, tkn_indikator, cakupan_std)
	    			//int updated = ur.ubahUsulanJadiStandar(id_std,id_std_isi,rasionale,isi_std,periode_unit_used,lama_per_period,periode_awal,target1,unit1,target2,unit2,target3,unit3,target4,unit4,target5,unit5,target6,unit6,tkn_variable,tkn_doc,versi,pihak,pihak_mon,norut,tkn_indikator,cakupan_std,tipe_proses_pengawasan);
