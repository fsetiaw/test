...
/**
 * Servlet implementation class SetAktivasiMan_v1
 */
@WebServlet("/SetAktivasiMan_v1")
public class SetAktivasiMan_v1 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SetAktivasiMan_v1() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//PrintWriter out = response.getWriter();
			...
			if(!aktif) {
				...
				boolean manual_ada_tgl_perumusan = smm.apaManualSudahAdaTglPerumusanUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std));
				...
				boolean manual_ada_tgl_pemeriksaan = smm.apaManualSudahAdaTglPemeriksaanUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std));
				...
				boolean manual_ada_tgl_persetujuan = smm.apaManualSudahAdaTglPersetujuanUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std));
				...
				boolean manual_ada_tgl_penetapan = smm.apaManualSudahAdaTglPenetapanUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std));
				if(!manual_ada_tgl_penetapan) {
					if(v_err_man==null) {
						v_err_man=new Vector();
						li = v_err_man.listIterator();
					}
					li.add("Manual yang dipilih belum memiliki kegiatan penetapan manual. Penetapan standar dilakukan pada siklus perencanaan, silahkan diisi kegiatan penetapan standar pada menu MANUAL PERENCANAAN STANDAR");
				}
			}
			else {
			...
			if(v_err_man!=null) {
				...
				request.getRequestDispatcher("get.prepInfoMan_v1?id_versi="+id_versi+"&id_tipe_std="+id_tipe_std+"&id_master_std="+id_master_std+"&id_std="+id_std+"&kdpst_nmpst_kmp="+kdpst_nmpst_kmp+"&at_menu_dash="+ at_menu_dash+"&fwdto="+fwdto+"&darimana=").forward(request,response);
			}
			else {
				//System.out.println("v_err_man is null");
				UpdateManualMutu umm = new UpdateManualMutu();
				umm.toogleAktifasiManUmum(tipe_manual, Integer.parseInt(id_versi), Integer.parseInt(id_std), aktif);
