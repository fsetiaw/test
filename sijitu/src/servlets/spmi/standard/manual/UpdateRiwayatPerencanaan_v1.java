...
import beans.dbase.spmi.riwayat.pengendalian.UpdHistPerencanaan;
...
/**
 * Servlet implementation class UpdateRiwayatPerencanaan_v1
 */
@WebServlet("/UpdateRiwayatPerencanaan_v1")
public class UpdateRiwayatPerencanaan_v1 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UpdateRiwayatPerencanaan_v1() {
        super();
			...
			*/
			...
			if(v_err!=null) {
				...
				if(!Checker.isStringNullOrEmpty(id_plan)) {
					target = Constants.getRootWeb()+"/InnerFrame/Spmi/standar_spmi/form_manual_perencanaan/riwayat_perencanaan_v1.jsp";
				}
				else {
					target = Constants.getRootWeb()+"/InnerFrame/Spmi/standar_spmi/form_manual_perencanaan/form_survey_perencanaan_v1.jsp";
				}
				...
				UpdHistPerencanaan uhk = new UpdHistPerencanaan();

				int updated = uhk.updateRiwayatPerencanaanUmum(id_plan,id_versi,id_std,form_tgl_sta,form_waktu_sta,form_nmm_kegiatan,form_jenis_kegiatan,form_tujuan_kegiatan,form_isi_kegiatan,form_job_jawab,form_job_target,form_dok_mutu,form_started);
				if(updated<1) {
					if(v_err==null) {
						v_err = new Vector();
						li = v_err.listIterator();
					 ...
					 * 
					 */
					//ts.createSubFolderManualPerencanaan(id_std_isi, id_versi, norut_man, kdpst, form_jenis_kegiatan, form_nmm_kegiatan);
					
				}
				//System.out.println("updated="+updated);
				String target = Constants.getRootWeb()+"/InnerFrame/Spmi/standar_spmi/form_manual_perencanaan/riwayat_perencanaan_v1.jsp";
				String uri = request.getRequestURI();
				String url = PathFinder.getPath(uri, target);
