...
import beans.dbase.spmi.riwayat.pengendalian.UpdHistPeningkatan_ori;
...
/**
 * Servlet implementation class UpdateRiwayatPeningkatan
 */
@WebServlet("/UpdateRiwayatPeningkatan")
public class UpdateRiwayatPeningkatan extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UpdateRiwayatPeningkatan() {
        super();
			...
			*/
			...
			if(v_err!=null) {
				...
				if(!Checker.isStringNullOrEmpty(id_plan)) {
					target = Constants.getRootWeb()+"/InnerFrame/Spmi/standar_spmi/form_manual_peningkatan/riwayat_peningkatan.jsp";
				}
				else {
					target = Constants.getRootWeb()+"/InnerFrame/Spmi/standar_spmi/form_manual_peningkatan/form_survey_peningkatan.jsp";
				}
				...
				UpdHistPeningkatan_ori uhk = new UpdHistPeningkatan_ori();

				int updated = uhk.updateRiwayatPeningkatan(id_plan,id_versi,id_std_isi,norut_man,form_tgl_sta,form_waktu_sta,form_nmm_kegiatan,form_jenis_kegiatan,form_tujuan_kegiatan,form_isi_kegiatan,form_job_jawab,form_job_target,form_dok_mutu,form_started);
				if(updated<1) {
					if(v_err==null) {
						v_err = new Vector();
						li = v_err.listIterator();
					...
					ts.createSubFolderManualPeningkatan(id_std_isi, id_versi, norut_man, kdpst, form_jenis_kegiatan, form_nmm_kegiatan);
					
				}
				//System.out.println("updated="+updated);
				String target = Constants.getRootWeb()+"/InnerFrame/Spmi/standar_spmi/form_manual_peningkatan/riwayat_peningkatan.jsp";
				String uri = request.getRequestURI();
				String url = PathFinder.getPath(uri, target);
