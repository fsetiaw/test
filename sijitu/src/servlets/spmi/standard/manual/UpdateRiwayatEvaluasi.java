...
/**
 * Servlet implementation class UpdateRiwayatEvaluasi
 */
@WebServlet("/UpdateRiwayatEvaluasi")
public class UpdateRiwayatEvaluasi extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UpdateRiwayatEvaluasi() {
        super();
			...
			*/
			...
			if(v_err!=null) {
				...
				if(!Checker.isStringNullOrEmpty(id_plan)) {
					target = Constants.getRootWeb()+"/InnerFrame/Spmi/standar_spmi/form_manual_evaluasi/riwayat_evaluasi.jsp";
				}
				else {
					target = Constants.getRootWeb()+"/InnerFrame/Spmi/standar_spmi/form_manual_evaluasi/form_survey_evaluasi.jsp";
				}
				...
				int updated = uhk.updateRiwayatEvaluasi(id_plan,id_versi,id_std_isi,norut_man,form_tgl_sta,form_waktu_sta,form_nmm_kegiatan,form_jenis_kegiatan,form_tujuan_kegiatan,form_isi_kegiatan,form_job_jawab,form_job_target,form_dok_mutu,form_started);
				if(updated<1) {
					if(v_err==null) {
						v_err = new Vector();
						li = v_err.listIterator();
