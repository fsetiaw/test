...
/**
 * Servlet implementation class UpdateManualPeningkatan
 */
@WebServlet("/UpdateManualPeningkatan")
public class UpdateManualPeningkatan extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UpdateManualPeningkatan() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(isu==null) {
			...
			String no_urut_manual = request.getParameter("norut_man");
			...
			if(!Checker.isStringNullOrEmpty(hapus) && hapus.equalsIgnoreCase("true")) {
				...
				if(!no_err) {
					...
					String target = Constants.getRootWeb()+"/InnerFrame/Spmi/standar_spmi/form_manual/form_insert_manual_peningkatan.jsp";
					...
					int updated = um.updateManualPeningkatan(mode,no_urut_manual,id_versi,id_std_isi,kdpst,id_versi,id_std_isi,id_std,at_menu_dash,id_kendali,tipe_sarpras,catat_civitas,fwdto,job_rumus,	job_cek,job_stuju,job_tetap,job_kendali,job_survey,tujuan,lingkup,prosedur,kuali,doc,ref,definisi);
					
					String tmp = "get.prepInfoStd?starting_no=1&offset=0&id_versi="+id_versi+"&id_std_isi="+id_std_isi+"&id_std="+id_std+"&kdpst_nmpst_kmp="+kdpst_nmpst_kmp+"&at_menu_dash="+ at_menu_dash;
			...
			if(!Checker.isStringNullOrEmpty(hapus) && hapus.equalsIgnoreCase("true")) {
				...
				if(!no_err) {
					...
					UpdateManual um =new UpdateManual();
					int updated = um.updateManualPeningkatan(id_kendali, Integer.parseInt(id_versi), Integer.parseInt(id_std_isi), Integer.parseInt(norut), capaian, ket_proses, manual, null, null, Integer.parseInt(tot_repetisi), satuan, job[0], job_input[0], null, null, tipe_sarpras, Boolean.parseBoolean(catat_civitas));
