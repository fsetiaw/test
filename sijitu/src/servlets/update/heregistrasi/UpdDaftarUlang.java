package servlets.update.heregistrasi;
...
/**
 * Servlet implementation class UpdDaftarUlang
 */
@WebServlet("/UpdDaftarUlang")
public class UpdDaftarUlang extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UpdDaftarUlang() {
        super();
		...
		//kode here
			...
			if(submit!=null && submit.equalsIgnoreCase("copy")) {
				if(thsms_base!=null && !Checker.isStringNullOrEmpty(thsms_base)) {
					...
					String uri = request.getRequestURI();
					String url = PathFinder.getPath(uri, target);
					request.getRequestDispatcher(url+"?msg=Update Gagal, BASE THSMS belum diisi<br>Harap Menunggu Anda Sedang Dialihkan&nuFwdPage=prep.prepParamReg&atMenu=regrl").forward(request,response);
				}
			}
				...
				if(kdpst!=null && kdpst.length>0) {
					for(int i=0;i<kdpst.length;i++) {
						//System.out.println(">>"+kdpst[i]+","+tkn_verificator[i]+","+urutan[i]+","+kode_kampus[i]);
