...
import beans.tools.*;
/**
 * Servlet implementation class PrepDashboardContactWeb
 */
@WebServlet("/PrepDashboardContactWeb")
public class PrepDashboardContactWeb extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public PrepDashboardContactWeb() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("prepContactWeb");
		String target = Constants.getRootWeb()+"/InnerFrame/ContactUs/dashContactUsWeb.jsp";
		...
		/*
		 * dibawah ini = list nickname web yg boleh dikontak usr 
		 */
		Vector vTargetObjNickname = isu.getScopeObjNickname("allowContactWEB");
		String[]listObjNicknameWeb = Constants.getListObjNicknameWeb();
		ListIterator li = vTargetObjNickname.listIterator();
		
		while(li.hasNext()) {
			String usrAllowContactWEBnickname = (String) li.next();
			boolean match = false;
			for(int i=0;i<listObjNicknameWeb.length && !match;i++) {
				if(usrAllowContactWEBnickname.equalsIgnoreCase(listObjNicknameWeb[i])) {
					match = true;
				}
			}
			if(!match) {
				/*
				 * buang obj nickname web yg tidak masuk ke scope usr
				 */
				li.remove();
			}
		}
