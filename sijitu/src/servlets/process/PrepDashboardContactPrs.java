...
import beans.tools.*;
/**
 * Servlet implementation class PrepDashboardContactPrs
 */
@WebServlet("/PrepDashboardContactPrs")
public class PrepDashboardContactPrs extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public PrepDashboardContactPrs() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("prepContactPrs");
		String target = Constants.getRootWeb()+"/InnerFrame/ContactUs/dashContactUsPrs.jsp";
		 ...
		 */
		Vector vTargetObjNickname = isu.getScopeObjNickname("allowContactPRS");
		String[]listObjNicknamePrs = Constants.getListObjNicknamePrs();
		ListIterator li = vTargetObjNickname.listIterator();
		
		while(li.hasNext()) {
			String usrallowContactPRSnickname = (String) li.next();
			boolean match = false;
			for(int i=0;i<listObjNicknamePrs.length && !match;i++) {
				if(usrallowContactPRSnickname.equalsIgnoreCase(listObjNicknamePrs[i])) {
					match = true;
				}
			}
			if(!match) {
