...
import beans.tools.*;
/**
 * Servlet implementation class PrepDashboardContactRek
 */
@WebServlet("/PrepDashboardContactRek")
public class PrepDashboardContactRek extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public PrepDashboardContactRek() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("prepContactRek");
		String target = Constants.getRootWeb()+"/InnerFrame/ContactUs/dashContactUsRek.jsp";
		 ...
		 */
		Vector vTargetObjNickname = isu.getScopeObjNickname("allowContactREK");
		String[]listObjNicknameRek = Constants.getListObjNicknameRek();
		ListIterator li = vTargetObjNickname.listIterator();
		
		while(li.hasNext()) {
			String usrallowContactREKnickname = (String) li.next();
			boolean match = false;
			for(int i=0;i<listObjNicknameRek.length && !match;i++) {
				if(usrallowContactREKnickname.equalsIgnoreCase(listObjNicknameRek[i])) {
					match = true;
				}
			}
			if(!match) {
