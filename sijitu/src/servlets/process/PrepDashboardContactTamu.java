...
import beans.tools.*;
/**
 * Servlet implementation class PrepDashboardContactTamu
 */
@WebServlet("/PrepDashboardContactTamu")
public class PrepDashboardContactTamu extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public PrepDashboardContactTamu() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("prepContactTamu");
		String target = Constants.getRootWeb()+"/InnerFrame/ContactUs/dashContactUsTamu.jsp";
		...
		/*
		 * dibawah ini = list nickname tamu yg boleh dikontak usr 
		 */
		Vector vTargetObjNickname = isu.getScopeObjNickname("allowContactTAMU");
		String[]listObjNicknameTamu = Constants.getListObjNicknameTamu();
		ListIterator li = vTargetObjNickname.listIterator();
		
		while(li.hasNext()) {
			String usrAllowContactTAMUnickname = (String) li.next();
			boolean match = false;
			for(int i=0;i<listObjNicknameTamu.length && !match;i++) {
				if(usrAllowContactTAMUnickname.equalsIgnoreCase(listObjNicknameTamu[i])) {
					match = true;
				}
			}
			if(!match) {
				/*
				 * buang obj nickname tamu yg tidak masuk ke scope usr
				 */
				li.remove();
			}
		}
