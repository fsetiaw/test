...
import beans.tools.*;
/**
 * Servlet implementation class PrepDashboardContactWrk
 */
@WebServlet("/PrepDashboardContactWrk")
public class PrepDashboardContactWrk extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public PrepDashboardContactWrk() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("prepContactWrk");
		String target = Constants.getRootWeb()+"/InnerFrame/ContactUs/dashContactUsWrk.jsp";
		 ...
		 */
		Vector vTargetObjNickname = isu.getScopeObjNickname("allowContactWRK");
		String[]listObjNicknameWrk = Constants.getListObjNicknameWrk();
		ListIterator li = vTargetObjNickname.listIterator();
		
		while(li.hasNext()) {
			String usrallowContactWRKnickname = (String) li.next();
			boolean match = false;
			for(int i=0;i<listObjNicknameWrk.length && !match;i++) {
				if(usrallowContactWRKnickname.equalsIgnoreCase(listObjNicknameWrk[i])) {
					match = true;
				}
			}
			if(!match) {
