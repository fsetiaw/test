...
/**
 * Servlet implementation class ExecuteSql_mhs_registrasi
 */
@WebServlet("/ExecuteSql_mhs_registrasi")
public class ExecuteSql_mhs_registrasi extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ExecuteSql_mhs_registrasi() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//kode here
			...
			String tot_pengajuan=request.getParameter("tot_pengajuan");
			...
			lih.add("TOTAL PENGAJUAN DISETUJUI`TOTAL MENUNGGU PERSETUJUAN");
			lih.add((Integer.parseInt(tot_pengajuan)-Integer.parseInt(tot_wip))+"`"+tot_wip);
			v_multi_header.add(0,v_header);//[0]
			v_multi_header.add(0,"SUMMARY PROSES HEREGISTRASI ["+Converter.getDetailKdpst_v1(target_kdpst)+"]");//[1]
			...
			if(!Checker.isStringNullOrEmpty(target_kdkmp)&&!Checker.isStringNullOrEmpty(target_kdpst)&&!Checker.isStringNullOrEmpty(target_thsms)) {
				String sql_cmd = "SELECT A.NPMHS as NPM, NMMHSMSMHS as NAMA, SMAWLMSMHS as ANGKATAN, ALL_APPROVED as 'DISETUJUI (true)' from DAFTAR_ULANG A INNER JOIN CIVITAS B on NPMHS=NPMHSMSMHS inner join OBJECT C on B.ID_OBJ=C.ID_OBJ where THSMS='"+target_thsms+"' and A.KDPST='"+target_kdpst+"' and KODE_KAMPUS_DOMISILI='"+target_kdkmp+"' order by ALL_APPROVED,NPMHS";	
