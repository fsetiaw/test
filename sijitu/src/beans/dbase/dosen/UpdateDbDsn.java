...
public class UpdateDbDsn extends UpdateDb {
    ...
    public void updateDosenAjarCpThsmsKrs(String[]infoDsn) {
    	...
    	String thsmsTarget = Checker.getThsmsBukaKelas();
    ...
    public void updateDosenAjarTrakd(String thsms,String[]kdpst,String[]kdkmk,String[]shift,String[]kelas,String[]old_kelas,String[]target_dos) {
    	...
    	if(kdpst!=null && kdpst.length>0) {
    		try {
        		...
        		ds = (DataSource)envContext.lookup("jdbc"+beans.setting.Constants.getDbschema());
        		con = ds.getConnection();
        		stmt = con.prepareStatement("UPDATE TRAKD set NODOSTRAKD=?,NMDOSTRAKD=?,KELASTRAKD=? where THSMSTRAKD=? and KDPSTTRAKD=? and KDKMKTRAKD=? and KELASTRAKD=? and SHIFTTRAKD=?");
        		int tot = kdpst.length;
        		for(int i=0;i<tot;i++) {
        			
        			StringTokenizer st = new StringTokenizer(target_dos[i],"`");
        			//System.out.println("target_dos[i]="+target_dos[i]);
        			////System.out.println("kelas[i]="+kelas[i]);
        			String nmdos = st.nextToken();
        			////System.out.println("nmdos[i]="+nmdos);
        			String nidnn = "";
        			String nomor = "";
        			if(!nmdos.equalsIgnoreCase("null")) {
        				nidnn = st.nextToken();
        				nomor = st.nextToken();
        				if(Checker.isStringNullOrEmpty(nidnn)) {
        					if(!Checker.isStringNullOrEmpty(nomor)) {
        						nidnn = new String(nomor);
        					}
        				}	
        			}
        			//update hanya yg ada nomornya
        			if(!Checker.isStringNullOrEmpty(nidnn)||(kelas[i]!=null && !Checker.isStringNullOrEmpty(kelas[i]))) {
        				if(Checker.isStringNullOrEmpty(nmdos)||nmdos.equalsIgnoreCase("null")) {
        				...
        				if(Checker.isStringNullOrEmpty(kelas[i])) {
        					stmt.setNull(3, java.sql.Types.VARCHAR);
        				}
        				else {
        					stmt.setString(3, kelas[i]);	
        				}
        				
        				stmt.setString(4, thsms);
        				stmt.setString(5, kdpst[i]);
        				stmt.setString(6, kdkmk[i]);
        				stmt.setString(7, old_kelas[i]);
        				stmt.setString(8, shift[i]);
        				////System.out.println("kelas[i]="+thsms[i]);
        				//System.out.println("thsms[i]="+thsms);
        				//System.out.println("kdpst[i]="+kdpst[i]);
        				//System.out.println("kdkmk[i]="+kdkmk[i]);
        				//System.out.println("kelas[i]="+kelas[i]);
        				//System.out.println("old_kelas[i]="+old_kelas[i]);
        				//System.out.println("shift[i]="+shift[i]);
        				int j = stmt.executeUpdate();
        				
        				//System.out.println(i+". exe="+j);
        			}
        		}
    ...
    public void updateDosenAjarTrakd(String thsms,String[]kls_info,String[]target_dos) {
    	
    	if(kls_info!=null && kls_info.length>0) {
    		try {
        		//String ipAddr =  request.getRemoteAddr();
        		Context initContext  = new InitialContext();
        		Context envContext  = (Context)initContext.lookup("java:/comp/env");
        		ds = (DataSource)envContext.lookup("jdbc"+beans.setting.Constants.getDbschema());
        		con = ds.getConnection();
        		stmt = con.prepareStatement("UPDATE TRAKD_EPSBED set NODOSTRAKD=?,NMDOSTRAKD=?,TTMHSTRAKD=? where THSMSTRAKD=? and KDPSTTRAKD=? and KDKMKTRAKD=? and KELASTRAKD=?");
        		int tot = kls_info.length;
        		
        		for(int i=0;i<tot;i++) {
        			StringTokenizer st = new StringTokenizer(kls_info[i],"`");
        			...
        			st = new StringTokenizer(target_dos[i],"`");
        			//nmdos+"`"+nodos+"`"+nomor+"`"+npmdos+"`"+nidk+"`"+nup
        			//System.out.println("kls_info["+i+"]="+kls_info[i]);
        			//System.out.println("target_dos["+i+"]="+target_dos[i]);
        			////System.out.println("kelas[i]="+kelas[i]);
        			String nmdos = st.nextToken();
        			////System.out.println("nmdos[i]="+nmdos);
        			...
        			if(!nmdos.equalsIgnoreCase("null")) {
            			...
            			nidk = st.nextToken();
            			nup = st.nextToken();
        				if(Checker.isStringNullOrEmpty(nidnn)&&Checker.isStringNullOrEmpty(nidk)&&Checker.isStringNullOrEmpty(nup)) {
        					if(!Checker.isStringNullOrEmpty(nomor)) {
        						nidnn = new String(nomor);
        					}
        				}	
        			...
        			//update hanya yg ada nomornya
        			//if(!Checker.isStringNullOrEmpty(nidnn)||!Checker.isStringNullOrEmpty(kode_kls)) {
        			if(Checker.isStringNullOrEmpty(nmdos)||nmdos.equalsIgnoreCase("null")) {
        				stmt.setNull(1, java.sql.Types.VARCHAR);
        				stmt.setNull(2, java.sql.Types.VARCHAR);
        			}
        			...
        			stmt.setInt(3, tot_mhs_per_kelas);
        			stmt.setString(4, thsms);
        			stmt.setString(5, kdpst);
        			stmt.setString(6, kdkmk);
        			stmt.setString(7, kode_kls);
        			int j = stmt.executeUpdate();
    ...
    /*
     * deprecated jgan dipake lagi
     */
    public void updateSuratTugasDosenAjarTrakd(String thsms,String[]kls_info,String[]surat_tugas) {
    	...
    	if(kls_info!=null && kls_info.length>0) {
    		try {
        		...
        		ds = (DataSource)envContext.lookup("jdbc"+beans.setting.Constants.getDbschema());
        		con = ds.getConnection();
        		stmt = con.prepareStatement("UPDATE TRAKD_EPSBED set SURAT_TUGAS=? where THSMSTRAKD=? and KDPSTTRAKD=? and KDKMKTRAKD=? and KELASTRAKD=?");
        		int tot = kls_info.length;
        		...
        		for(int i=0;i<tot;i++) {
        			...
        			//update hanya yg ada nomornya
        			/*
        			 * KARENA BILA SATU KTU HANYA INPUT PRODINYA SAJA, BILA KOSONG NANTI PRODI YG LAEN KE RESET NULL 
        			 */
        			if(!Checker.isStringNullOrEmpty(surat_tugas[i])) {
        				stmt.setString(1, surat_tugas[i]);
    ...
    public int updateSuratTugasDosenAjarTrakd_v1(String target_thsms,String[]kdpst,String[]nodos,String[]surat_tugas) {
    	int updated = 0;
    	if((kdpst!=null && nodos!=null && surat_tugas!=null) && (kdpst.length==nodos.length && kdpst.length==surat_tugas.length)) {
    		try {
    			Context initContext  = new InitialContext();
        		Context envContext  = (Context)initContext.lookup("java:/comp/env");
        		ds = (DataSource)envContext.lookup("jdbc"+beans.setting.Constants.getDbschema());
        		con = ds.getConnection();
        		stmt = con.prepareStatement("update ignore TRAKD set SURAT_TUGAS=? where THSMSTRAKD=? and KDPSTTRAKD=? and NODOSTRAKD=?");
        		for(int i=0;i<kdpst.length;i++) {
        			if(!Checker.isStringNullOrEmpty(surat_tugas[i])) {
        				stmt.setString(1, surat_tugas[i]);
        				stmt.setString(2, target_thsms);
        				stmt.setString(3, kdpst[i]);
        				stmt.setString(4, nodos[i]);
        				updated = updated+stmt.executeUpdate();
        				
        			}
        		}
        		stmt = con.prepareStatement("update ignore TRAKD_EPSBED set SURAT_TUGAS=? where THSMSTRAKD=? and KDPSTTRAKD=? and NODOSTRAKD=?");
        		for(int i=0;i<kdpst.length;i++) {
        			if(!Checker.isStringNullOrEmpty(surat_tugas[i])) {
        				stmt.setString(1, surat_tugas[i]);
        				stmt.setString(2, target_thsms);
