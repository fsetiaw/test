...
/**
 * Session Bean implementation class SrcHistPelaksanaan
 */
@Stateless
@LocalBean
public class SrcHistPelaksanaan extends SearchDb {
	String operatorNpm;
	String operatorNmm;
     ...
     */
    public SrcHistPelaksanaan(String operatorNpm) {
        super(operatorNpm);
     ...
     */
    public SrcHistPelaksanaan(Connection con) {
        super(con);
        // TODO Auto-generated constructor stub
    }
    ...
    public Vector getListRiwayatPelaksanaan(int versi_id, int std_isi_id, int norut_man, int offset, int limit) {
    	/*
    	...
    	try {
    		...
    		String sql = "select * from RIWAYAT_PELAKSANAAN A inner join STANDARD_MANUAL_PELAKSANAAN B on (A.VERSI_ID=B.VERSI_ID AND A.STD_ISI_ID=B.STD_ISI_ID AND A.NORUT=B.NORUT) where A.VERSI_ID=? AND A.STD_ISI_ID=? AND A.NORUT=? order by A.ID_PLAN desc limit ?,?";
    ...
    public Vector getListRiwayatPelaksanaanUmum(int versi_id, int id_std, int offset, int limit) {
    	/*
    	 * std_isi_id menentukan apakah ini generik ato untuk kdpst tertentu
    	 */
    	Vector v = null;
    	...
    	try {
        	...
        	ds = (DataSource)envContext.lookup("jdbc"+beans.setting.Constants.getDbschema());
        	con = ds.getConnection();
    		String sql = "select * from RIWAYAT_PELAKSANAAN_UMUM A inner join STANDARD_MANUAL_PELAKSANAAN_UMUM B on (A.VERSI_ID=B.VERSI_ID AND A.ID_STD=B.ID_STD) where A.VERSI_ID=? AND A.ID_STD=? order by A.ID_PLAN desc limit ?,?";
    		stmt = con.prepareStatement(sql);
    		stmt.setInt(1, versi_id);
