...
@Stateless
@LocalBean
public class SrcHistPeningkatan extends SearchDb {
	String operatorNpm;
	String operatorNmm;
     ...
     */
    public SrcHistPeningkatan(String operatorNpm) {
        super(operatorNpm);
     ...
     */
    public SrcHistPeningkatan(Connection con) {
        super(con);
        // TODO Auto-generated constructor stub
    }
    ...
    public Vector getListRiwayatPeningkatanUmum(int versi_id, int id_std, int offset, int limit) {
    	/*
    	 * std_isi_id menentukan apakah ini generik ato untuk kdpst tertentu
    	 */
    	Vector v = null;
    	...
    	try {
        	...
        	ds = (DataSource)envContext.lookup("jdbc"+beans.setting.Constants.getDbschema());
        	con = ds.getConnection();
    		String sql = "select * from RIWAYAT_PENINGKATAN_UMUM A inner join STANDARD_MANUAL_PENINGKATAN_UMUM B on (A.VERSI_ID=B.VERSI_ID AND A.ID_STD=B.ID_STD) where A.VERSI_ID=? AND A.ID_STD=? order by A.ID_PLAN desc limit ?,?";
    		stmt = con.prepareStatement(sql);
    		stmt.setInt(1, versi_id);
