...
@Stateless
@LocalBean
public class KelasKuliahImporter extends SearchDb {
	String operatorNpm;
	String operatorNmm;
     ...
     */
    public KelasKuliahImporter(String operatorNpm) {
        super(operatorNpm);
     ...
     */
    public KelasKuliahImporter(Connection con) {
        super(con);
        // TODO Auto-generated constructor stub
    }
    ...
    public Vector importKelasKuliah(String thsms) {
    	...
    	if(v!=null && v.size()>0) {
        	...
        	try {
    			...
    			//cek norut idkls terakhir
    			int norut = 0;
    			stmt = con.prepareStatement("select id_kls from kelas_kuliah where id_smt=? and length(id_kls)<5");
    			stmt.setString(1, thsms);
    			rs = stmt.executeQuery();
    			if(rs.next()) {
    				int prev_no = Integer.parseInt(rs.getString(1));
    				norut = prev_no;
    				while(rs.next()) {
    					norut = Integer.parseInt(rs.getString(1));
    					if(norut>prev_no) {
    						prev_no = norut;
    					}
    				}
    			...
    			stmt = con.prepareStatement("insert ignore into kelas_kuliah(id_kls,id_sms,id_smt,nm_kls,sks_mk,sks_tm,sks_prak,sks_prak_lap,sks_sim,id_mk)values(?,?,?,?,?,?,?,?,?,?)");
    			li = v.listIterator();
    			while(li.hasNext()) {
    				norut++;
    				String brs = (String)li.next();
    				...
    				stmt.setString(1, ""+norut);
    				stmt.setString(2, id_sms);
    				stmt.setString(3, thsms);
    				stmt.setString(4, kelas);
    				stmt.setInt(5, Integer.parseInt(sksmk));
    				stmt.setInt(6, Integer.parseInt(sks_tm));
    				stmt.setInt(7, Integer.parseInt(sks_prak));
    				stmt.setInt(8, Integer.parseInt(sks_prak_lap));
    				stmt.setInt(9, Integer.parseInt(sks_sim));
    				stmt.setString(10, id_mk);
