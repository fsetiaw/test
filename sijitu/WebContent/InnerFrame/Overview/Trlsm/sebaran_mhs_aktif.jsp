...
<%
int norut=1;
if((v_scope_kdpst_list_distinct_npm_trnlm_at_target_thsms==null || v_scope_kdpst_list_distinct_npm_trnlm_at_target_thsms.size()<1)&&(v_scope_kdpst_list_distinct_npm_cuti_ato_nonaktif_at_target_thsms==null || v_scope_kdpst_list_distinct_npm_cuti_ato_nonaktif_at_target_thsms.size()<1) ) {
...
%>
	...
	<thead>
		<tr>
			<th colspan="5" style="text-align: center; padding: 0px 10px;font-size:1.5em">SEBARAN MAHASISWA AKTIF <%=target_thsms %><br><%=Converter.getNamaKampus(target_kdkmp) %> </th>
		</tr>
		<tr>
			<th width="5%">NO</th>
			<th width="45%">PRODI</th>
			<th width="20%">AKTIF KULIAH</th>
			<th width="20%">CUTI / NON-AKTIF</th>
	...
	while(lisc.hasNext()) {
		...
		if(brs.startsWith(target_kdkmp+"`")) {
			...
			while(st.hasMoreTokens()) {
				...
				int tot_mhs_aktif = 0, tot_mhs_non_aktif=0;
				//hitung mhs nkrs
				if(v_scope_kdpst_list_distinct_npm_trnlm_at_target_thsms!=null && v_scope_kdpst_list_distinct_npm_trnlm_at_target_thsms.size()>0) {
					ListIterator li_krs = v_scope_kdpst_list_distinct_npm_trnlm_at_target_thsms.listIterator();
						...
						if(baris.startsWith(target_kdkmp+"`")) {
							...
							if(stt.hasMoreTokens()&&stt.countTokens()>1) {
								...
								while(stt.hasMoreTokens()&& !match_prodi) {
									...
									if(match_prodi) {
										tot_mhs_aktif=1;//match token pertama ketemu diatas
										while(stt.hasMoreTokens() && match_prodi) {
											prodi = stt.nextToken();
											npm = stt.nextToken();
											if(!prodi.equalsIgnoreCase(kdpst_row)) {
				...
				//hitung mhs non
				if(v_scope_kdpst_list_distinct_npm_cuti_ato_nonaktif_at_target_thsms!=null && v_scope_kdpst_list_distinct_npm_cuti_ato_nonaktif_at_target_thsms.size()>0) {
					ListIterator li_krs = v_scope_kdpst_list_distinct_npm_cuti_ato_nonaktif_at_target_thsms.listIterator();
					boolean match_kampus = false;
					while(li_krs.hasNext() && !match_kampus) {
						String baris = (String)li_krs.next();
						if(baris.startsWith(target_kdkmp+"`")) {
							...
							if(stt.hasMoreTokens()&&stt.countTokens()>1) {
								...
								while(stt.hasMoreTokens()&& !match_prodi) {
									...
									if(match_prodi) {
										tot_mhs_non_aktif=1;//match token pertama ketemu diatas
										while(stt.hasMoreTokens() && match_prodi) {
											prodi = stt.nextToken();
											npm = stt.nextToken();
											if(!prodi.equalsIgnoreCase(kdpst_row)) {
...
%>
		<tr onclick="location.href='go.executeSql_mhs_aktif?tot_mhs_non_aktif=<%=tot_mhs_non_aktif %>&tot_mhs_aktif=<%=tot_mhs_aktif %>&target_kdkmp=<%=target_kdkmp %>&target_thsms=<%=target_thsms %>&target_kdpst=<%=kdpst_row%>'">
			...
			<td align="center" style="vertical-align: middle; padding: 0px 5px"><%=tot_mhs_aktif%></td>
			<td align="center" style="vertical-align: middle; padding: 0px 5px"><%=tot_mhs_non_aktif%></td>
			<td align="center" style="vertical-align: middle; padding: 0px 5px"><%=tot_mhs_aktif + tot_mhs_non_aktif%></td>
			<%total_aktif =  total_aktif + tot_mhs_aktif + tot_mhs_non_aktif;%>
		</tr>
<%
			}
		}	
...
%>		
		<tr>
			...
			<th colspan="1" style="text-align: center; padding: 0px 10px;font-size:1.25em"><%=total_aktif %> </th>
			</thead>
		</tr>
