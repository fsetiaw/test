...
<%
String target = Constants.getRootWeb()+"/InnerFrame/sql/ResultSet_v5.jsp";
...
if(tot_data>0 && last_index>1) {
	if(ada_prev) {
		...
		if(first_index>1) {
		%>
	<a href="<%=url %>?backto=<%=backto %>&first_index=<%=first_index %>&at_page=1&max_data_per_pg=<%=max_data_per_pg %>&norut_col_npm=<%=norut_colnpm %>" style="font-weight:bold;font-size:1.6em" target="inner_iframe"><<</a>&nbsp&nbsp&nbsp&nbsp&nbsp
		<%
		}
	%>
	<a href="<%=url %>?backto=<%=backto %>&first_index=<%=first_index %>&at_page=<%=at_page-1 %>&max_data_per_pg=<%=max_data_per_pg %>&norut_col_npm=<%=norut_colnpm %>" style="font-weight:bold;font-size:1.5em" target="inner_iframe"><</a>&nbsp&nbsp&nbsp&nbsp&nbsp
	<%
	}
//InnerFrame/sql/ResultSet_v4.jsp?at_page=1&max_data_per_pg=25";
	for(int i=first_index;i<=last_index;i++) {
		if(i==at_page) {
		%>
	<a href="<%=url %>?backto=<%=backto %>&first_index=<%=first_index %>&at_page=<%=i %>&max_data_per_pg=<%=max_data_per_pg %>&norut_col_npm=<%=norut_colnpm %>" target="inner_iframe" style="font-weight:bold;font-size:1.5em"><%=i %></a>&nbsp&nbsp&nbsp&nbsp&nbsp
			<%	
		}
		else {
		%>
	<a href="<%=url %>?backto=<%=backto %>&first_index=<%=first_index %>&at_page=<%=i %>&max_data_per_pg=<%=max_data_per_pg %>&norut_col_npm=<%=norut_colnpm %>" target="inner_iframe"><%=i %></a>&nbsp&nbsp&nbsp&nbsp&nbsp
		<%
		}
	}
...
if(ada_next) {
		%>
	<a href="<%=url %>?backto=<%=backto %>&first_index=<%=first_index %>&at_page=<%=at_page+1 %>&max_data_per_pg=<%=max_data_per_pg %>&norut_col_npm=<%=norut_colnpm %>" target="inner_iframe" style="font-weight:bold;font-size:1.5em">></a>&nbsp&nbsp&nbsp&nbsp&nbsp
		<%	
		if(last_index<max_index) {
			%>
	<a href="<%=url %>?backto=<%=backto %>&first_index=<%=first_index %>&at_page=<%=max_index %>&max_data_per_pg=<%=max_data_per_pg %>&norut_col_npm=<%=norut_colnpm %>" target="inner_iframe" style="font-weight:bold;font-size:1.6em">>></a>&nbsp&nbsp&nbsp&nbsp&nbsp
				<%	
		}
}
