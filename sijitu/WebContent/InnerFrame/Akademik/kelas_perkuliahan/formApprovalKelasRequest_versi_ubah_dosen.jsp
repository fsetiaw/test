...
<!-- Column 1 start -->
<%
Vector v_list_all_dosen = Getter.getListDosen_v1(false);
...
<center>
<br>
	<form action="go.updateDosenAjar" method="post">
	<table class="table" style="width:80%">
		<input type="hidden" name="target_thsms" value="<%=target_thsms %>"/>
		<input type="hidden" name="no_edit" value="<%=non_editable %>"/>
  	...
  	while(li.hasNext()) {
  		...
  		<tr>
  			...
  			<td align="center" style="vertical-align: middle; padding: 0px 5px">
  				<select name="dosen" style="border:none;weight:30px;width:100%;text-align-last:center">
  		<%
  			ListIterator lidos = v_list_all_dosen.listIterator();
			//li.add(nmdos+"`"+nodos+"`"+nomor+"`"+npmdos+"`"+nidk+"`"+nup);
			boolean match=false;
			while(lidos.hasNext()&&!match) {
				String brs = (String)lidos.next();
				StringTokenizer std = new StringTokenizer(brs,"`");
				//nmdos+"`"+nodos+"`"+nomor+"`"+npmdos+"`"+nidk+"`"+nup
				String nmdos_onlist = std.nextToken();
				String nodos_onlist = std.nextToken();
				String nomor_onlist = std.nextToken();
				String npmdos_onlist = std.nextToken();
				String nidk_onlist = std.nextToken();
				String nup_onlist = std.nextToken();
				String info_dos = new String(nmdos_onlist);
				if(!Checker.isStringNullOrEmpty(nodos_onlist)) {
					info_dos=info_dos+" ["+nodos_onlist+"]";
				}
				else if(!Checker.isStringNullOrEmpty(nidk_onlist)) {
					info_dos=info_dos+" ["+nidk_onlist+"]";
				}
				else if(!Checker.isStringNullOrEmpty(nup_onlist)) {
					info_dos=info_dos+" ["+nup_onlist+"]";
				}
			   	if(info_nmdos.equalsIgnoreCase(nmdos_onlist)||info_npmdos.equalsIgnoreCase(npmdos_onlist)) {
			   		%>
					<option value="<%= info_cuid%>~<%=nidk_onlist %>~<%=npmdos_onlist%>~<%=nmdos_onlist%>"><%=info_dos %></option>
					<%
			   	}
			   	else {
			   		%>
  		...
  		<tr>
  			...
  			<td align="center" style="vertical-align: middle; padding: 0px 0px">
  				<select name="dosen" style="border:none;height:30px;width:100%;text-align-last:center">
  		<%
  			ListIterator lidos = v_list_all_dosen.listIterator();
			//li.add(nmdos+"`"+nodos+"`"+nomor+"`"+npmdos+"`"+nidk+"`"+nup);
			boolean match=false;
			while(lidos.hasNext()&&!match) {
				...
				if(!Checker.isStringNullOrEmpty(nodos)) {
			   	...
			   	if(nmmdos.equalsIgnoreCase(nmdos_onlist)||npmdos.equalsIgnoreCase(npmdos_onlist)) {
			   		%>
					<option value="<%= cuid%>~<%=nidk_onlist %>~<%=npmdos_onlist%>~<%=nmdos_onlist%>" selected="selected"><%=info_dos %></option>
					<%
			   	}
			   	else {
			   		%>
					<option value="<%= cuid%>~<%=nidk_onlist %>~<%=npmdos_onlist%>~<%=nmdos_onlist%>"><%=info_dos %></option>
					<%   		
			   	}
...
<%		
	...
	<section class="gradient">
		<div style="text-align:center">
			<button type="submit" style="padding: 5px 50px;font-size: 20px;">UPDATE DOSEN AJAR</button>
		</div>
	</section>
</center>
