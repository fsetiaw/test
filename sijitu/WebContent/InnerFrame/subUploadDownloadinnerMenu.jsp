...
<ul>
	...
	if(validUsr.isAllowTo("hasUpDownFileMhs")>0) {
		...
		if(cmd.equalsIgnoreCase("uploadFile")) {
			%>
			<li><a href="uploadFile.jsp?id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>&cmd=uploadFile" target="_self" class="active">UPLOAD<span>FILES</span></a></li>
			<%
		}
		else {
			%>
			<li><a href="uploadFile.jsp?id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>&cmd=uploadFile" target="_self" >UPLOAD<span>FILES</span></a></li>
			<%
		}	
		
		if(cmd.equalsIgnoreCase("downloadFile")) {
			%>
			<li><a href="downloadFile.jsp?id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>&cmd=downloadFile" target="_self" class="active">DOWNLOAD<span>FILES</span></a></li>
			<%
		}
		else {
			%>
			<li><a href="downloadFile.jsp?id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>&cmd=downloadFile" target="_self" >DOWNLOAD<span>FILES</span></a></li>
			<%
		}
	}
