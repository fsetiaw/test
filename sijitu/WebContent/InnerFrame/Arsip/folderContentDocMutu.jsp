...
//	setTimeout(popup, 3000);
	...
	//if (username == "" || password == ""){
		if (nameFolder == ""){
			...
			//$("#logindiv").css("display", "inline");
			$.post('file.createNuFolderDocMutu?kdpst=<%=kdpst%>', $('#login').serialize(), function(data) {
				window.location.replace("go.redirectAfterGenericUploadFileDocMutu?kdpst=<%=kdpst%>");
				//document.getElementById('konten').style.visibility='visible';
			    //$('#konten').html(); 
			    //$('#konten').html(data);
...
<%
	//System.out.println("logindiv cur_dir="+cur_dir);
		//System.out.println("logindiv kdpst="+kdpst);
		//System.out.println("logindiv root_dir="+root_dir);
		String target_nm_folder = new String(cur_dir);
		while(target_nm_folder.contains("~DAN~")) {
			target_nm_folder = target_nm_folder.replace("~DAN~", "&");
		}
		while(target_nm_folder.contains("~OR~")) {
			target_nm_folder = target_nm_folder.replace("~OR~", "/");
		}
...
%>
	...
	<input type="hidden" name="root_dir" value="<%=target_nm_folder%>"/>
	<input type="hidden" name="alm" value="<%=target_nm_folder%>"/>
...
<form class="form" action="#" id="login" name="login"  method="post" enctype="multipart/form-data">
	...
	target_nm_folder = new String(cur_dir);
	while(target_nm_folder.contains("~DAN~")) {
		target_nm_folder = target_nm_folder.replace("~DAN~", "&");
	}
	while(target_nm_folder.contains("~OR~")) {
	...
	//System.out.println("target_nm_folder1="+target_nm_folder);
	%>
	<input type="hidden" name="saveToFolder" id="saveToFolder" value="<%=target_nm_folder %>" />
...
%>			
				<a href="get.folderContentDocMutu?root_dir=<%=root_dir %>&keter=<%=nameKeterDir %>&alm=<%=backToDir %>&hak=<%=hakAksesUsrUtkFolderIni %>&kdpst=<%=kdpst%>"><img style="border:0;" src="<%=Constants.getRootWeb() %>/css/folderIcon/folder_previous_grey.png" alt="Prev Dir" ></a>
			</td>
			<td>
			<h2>
			<%
			...
			String nm_dir_ori = new String(nm_dir);
			while(nm_dir_ori.contains("~DAN~")) {
				nm_dir_ori = nm_dir_ori.replace("~DAN~", "&");
			}
			while(nm_dir_ori.contains("~OR~")) {
				nm_dir_ori = nm_dir_ori.replace("~OR~", "/");
			}
			out.print(nm_dir_ori);
		...
		<tr>
			...
			String nm_folder = "";
			StringTokenizer stt = new StringTokenizer(tmp_cur_dir,"/");
			int tot_token = stt.countTokens();
			nm_folder = Tool.getTokenKe(tmp_cur_dir, tot_token, "/");
			String nm_dir_ori = new String(nm_folder);
			while(nm_dir_ori.contains("~DAN~")) {
				nm_dir_ori = nm_dir_ori.replace("~DAN~", "&");
			}
			while(nm_dir_ori.contains("~OR~")) {
...
if(listFileFolder!=null && listFileFolder.length>0) {
  //if(v_nm_alm_access!=null && v_nm_alm_access.size()>0) {		
	...
	//only show directory	
		if(cur_dir.endsWith("/monitoring")||cur_dir.endsWith("/kegiatan ami")) {
			...
			if (listFileFolder[i].isDirectory()) {
				...
				if(nm_folder.equalsIgnoreCase(nm_prodi)) {
				    ...
				    <a href="get.folderContentDocMutu?root_dir=<%=root_dir %>&keter=<%=listFileFolder[i].getName() %>&alm=<%=cur_dir %>/<%=listFileFolder[i].getName() %>&hak=<%=hakAksesUsrUtkFolderIni %>&kdpst=<%=kdpst%>" class="docFolder"></a><div style="text-align:left"><%= nm_dir_ori%></div>
				    </td>
				<%                
		...
		/*
		%>		
				<a href="go.downloadFileAsIs?root_dir=<%=root_dir %>&keter=<%=listFileFolder[i].getName() %>&alm=<%=cur_dir %>&namaFile=<%=listFileFolder[i].getName() %>&hak=<%=hakAksesUsrUtkFolderIni %>&kdpst=<%=kdpst%>" class="docIcon"></a><div style="text-align:left"><%=cur_dir %>/<%= listFileFolder[i].getName()%></div>
		<%
		*/
		%>
				<a href="go.streamFile?root_dir=<%=root_dir %>&keter=<%=listFileFolder[i].getName() %>&alm=<%=cur_dir %>&namaFile=<%=listFileFolder[i].getName() %>&hak=<%=hakAksesUsrUtkFolderIni %>&kdpst=<%=kdpst%>" class="docIcon"></a><div style="text-align:left"><%= listFileFolder[i].getName()%></div>	
		    	</td>
		<%
			}
		}
