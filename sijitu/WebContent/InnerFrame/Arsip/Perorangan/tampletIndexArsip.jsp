...
<%
	...
	String root_individual_folder = request.getParameter("root_individual_folder");
...
if(list_folder_at_root_dir!=null && !Checker.isStringNullOrEmpty(list_folder_at_root_dir)) {
	...
	while(st.hasMoreTokens()) {
		...
		String alamat_folder = root_individual_folder+"/"+keter_folder;
	...
	//only show directory
		...
		//if(true) {	
			...
			//if(j%4==0) {
			StringTokenizer sth = new StringTokenizer(list_hidden_folder,"`");
			boolean hidden_folder = false;
			while(sth.hasMoreTokens() && !match) {
				String hd_folde = sth.nextToken();
				if(hd_folde.equalsIgnoreCase(keter_folder)) {
					hidden_folder = true;
				}
			}		
%>
...
%>
	...
	//System.out.println("keter_folder="+keter_folder);
	%>
	  <a href="get.folderContent_v1?hidden_folder=true&list_hidden_folder=<%=list_hidden_folder %>&list_tipe_personal_folder=<%=list_tipe_personal_folder %>&list_folder_at_root_dir=<%=list_folder_at_root_dir %>&id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>&cmd=arsip&root_dir=<%=root_individual_folder %>&keter=<%=keter_folder %>&alm=<%=alamat_folder %>&hak=<%=nick_and_hak.replace("`", "-") %>" class="docFolderRed"></a><div style="text-align:left"><%= keter_folder.replaceAll("_"," ")%></div>
	<%
		}
		else {
	%>
      <a href="get.folderContent_v1?hidden_folder=false&list_hidden_folder=<%=list_hidden_folder %>&list_tipe_personal_folder=<%=list_tipe_personal_folder %>&list_folder_at_root_dir=<%=list_folder_at_root_dir %>&id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>&cmd=arsip&root_dir=<%=root_individual_folder %>&keter=<%=keter_folder %>&alm=<%=alamat_folder %>&hak=<%=nick_and_hak %>" class="docFolderBlue"></a><div style="text-align:left"><%= keter_folder.replaceAll("_"," ")%></div>
    <%
		}
    %>
    </div>
