...
StringTokenizer st = new StringTokenizer(list_color,",");
String warna = "";
%>    
    ...
    <div class="tile-area tile-area-scheme-dark fg-white" style="height: 100%; max-height: 100% !important;">
        ...
        <div class="tile-group quadro">
			<span class="tile-group-title">RIWAYAT PESAN</span>
...
if(jsoa!=null && jsoa.length()>0) {
	...
	for(int i=0;i<jsoa.length();i++) {
		...
		try {
			unread = (Integer)job.get("UNREAD");
		}
		catch(JSONException e) {}
		try {
			send = (Integer)job.get("SEND");
		}
		catch(JSONException e) {}
		try {
			list_info_numsg = (String)job.get("NU_MSG_INFO");
		}
		catch(JSONException e) {}
		try {
			participated = (String)job.get("PARTICIPATED_GROUP");
		}
		catch(JSONException e) {}
		//ignore
		
		//System.out.println(unread+","+send+","+list_info_numsg+","+participated);
		if(list_info_numsg!=null && !Checker.isStringNullOrEmpty(list_info_numsg)) {
		  	//if(true) {
		  	StringTokenizer stt = new StringTokenizer(list_info_numsg,"`");
		  	while(stt.hasMoreTokens()) {
		  		String nm_group = stt.nextToken();
		  		String group_id = stt.nextToken();
		  		String tot_nu_msg = stt.nextToken();
		  		no_item=false;
			  	if(st.hasMoreTokens()) {
			  		warna = st.nextToken();
			  	}
			  	else {
			  		st = new StringTokenizer(list_color,",");
			  		warna = st.nextToken();
			  	}
			  	%>
					<a href="#" target="inner_iframe" class="tile bg-<%=warna %> fg-white" data-role="tile">
		            	<div class="padding20">
		                	<p class="no-margin text-shadow"><span class="text-bold"><%=nm_group %></span><br/></p>
						</div>
					</a>       			
			                	<%	
			}
		...
		if(participated!=null && !Checker.isStringNullOrEmpty(participated)) {
		            		  	//if(true) {
			StringTokenizer stt = new StringTokenizer(participated,"`");
		    while(stt.hasMoreTokens()) {
		    	String nm_group = stt.nextToken();
		        String group_id = stt.nextToken();
    		   ...
    		   // request.getRequestDispatcher(url).forward(request,response);
    		  	%>
    				...
    				<a href="<%=Constants.getRootWeb() %>/InnerFrame/Pesan/conversation_main_frame.jsp?str_range=<%=str_range %>&grp_nm=<%=nm_group %>&grp_id=<%=group_id %>" target="inner_iframe" class="tile bg-<%=warna %> fg-white" data-role="tile">

    	            	<div class="padding20">
    	                	<p class="no-margin text-shadow"><span class="text-bold"><%=nm_group %></span><br/></p>
    					</div>
    					<div style="padding:40px 0 0 30px">
    	                	<p class="no-margin text-shadow">PESAN BARU :<span class="text-bold">&nbsp&nbsp&nbsp&nbsp0</span><br/></p>
    					</div>
    				</a>       			
    		                	<%	
		  	}       		  	
