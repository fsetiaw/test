...
%>		
		...
		<form action="go.updatePpRules" align="center">
<%
...
%>
		<table class="table">	
			<thead>
				<tr>
        			<th colspan="3" style="text-align: left; padding: 0px 10px">PINDAH PRODI RULES <%=Checker.getThsmsHeregistrasi() %></th>
        		</tr>	
        		<tr>
...
%>
		...
		<table class="table">	
			<thead>
				<tr>
        			<th colspan="3" style="text-align: left; padding: 0px 10px">FORM PINDAH PRODI</th>
        		</tr>
        		<tr>
		...
		while(litmp.hasNext()) {
			...
			if(false){	
				...
				while(li1.hasNext() && !match) {
					...
					if(kdprod.equalsIgnoreCase(prod)) {
						match = true;
					}
			...
			if(counter%2!=0) {
				if(match) {
%>
									<td><input type="checkbox" name="prodi" value="<%=kdprod%>" checked="checked"> <%=nmprod %> (<%=kdjen %>)</td>
<%						
				}
				else {
%>
		...
		//System.out.println(bar);
			...
			if(false){
				...
				while(li1.hasNext() && !match) {
					...
					String prod = st2.nextToken();
					String kampus = st2.nextToken();
					if(kdkmp.equalsIgnoreCase(kampus)) {
						match = true;
					}
					//li.add(nmjob+"`"+kdpst+"`"+kdkmp);
				}
			...
			if(counter%1!=0) {
				if(match) {
%>
									<td><input type="checkbox" name="kmp" value="<%=kdkmp%>" checked="checked"> <%=nmkmp %></td>
<%						
				}
				else {
%>
