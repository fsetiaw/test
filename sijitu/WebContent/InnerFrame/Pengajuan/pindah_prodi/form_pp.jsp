...
String title_pengajuan = table_nm.replace("_RULES", "");
title_pengajuan = title_pengajuan.replace("_", " ");
...
if(!validUsr.isHakAksesReadOnly("pp")) {
	if(at_menu!=null && at_menu.equalsIgnoreCase("pp")) {
		...
		<li>
			<a href="<%=Constants.getRootWeb()+"/" %>InnerFrame/Pengajuan/<%=folder_pengajuan%>/form_pp.jsp?id_obj=<%=id_obj%>&nmm=<%=nmm%>&npm=<%=npm %>&obj_lvl=<%=obj_lvl %>&kdpst=<%=kdpst %>&cmd=reqMhs&atMenu=pp" target="_self" class="active">FORM PINDAH<span>PROGRAM STUDI</span></a>
		</li>
	<%		
	}
	else {
		...
		<li>
			<a href="<%=Constants.getRootWeb()+"/" %>InnerFrame/Pengajuan/<%=folder_pengajuan%>/form_pp.jsp?id_obj=<%=id_obj%>&nmm=<%=nmm%>&npm=<%=npm %>&obj_lvl=<%=obj_lvl %>&kdpst=<%=kdpst %>&cmd=reqMhs&atMenu=pp" target="_self">FORM PINDAH<span>PROGRAM STUDI</span></a>
		</li>
	<%		
	}
...
<div class="colmask fullpage">
	...
	if(!sdh_ada_rules) {
		...
		<div style="width:90%">
		Dengan ini saya, <%=nmm %>, mengajukan permohonan pindah ke <br/>prodi baru :
		<select name="nu_prodi" style="width:21%" required> 
			<option value="">- PILIH PRODI TUJUAN -</option>
		<%
		ListIterator li = v_list_prodi.listIterator();
		while(li.hasNext()) {
			...
			String nm_prodi = st.nextToken();
		%>
			<option value="<%=baris%>"><%=nm_prodi %> (<%=kdjen %>)</option>
		<%	
		}
		%>
		</select>
		</br>dengan alasan sbb :<br/>
