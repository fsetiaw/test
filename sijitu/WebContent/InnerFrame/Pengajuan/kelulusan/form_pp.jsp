...
<div class="colmask fullpage">
	...
	if(!sdh_ada_rules) {
		...
		<form onsubmit="button_submit.disabled = true; return true;" action="go.ajukanLulus?id_obj=<%=id_obj%>&nmm=<%=nmm%>&npm=<%=npm %>&obj_lvl=<%=obj_lvl %>&kdpst=<%=kdpst %>&cmd=<%=cmd %>&target_thsms=<%=target_thsms %>&scope=<%=scope %>&table=<%=table_nm %>&folder_pengajuan=<%=folder_pengajuan%>" method="post">
		<div style="width:90%">
		Dengan ini mahasiswa dibawah ini : <br/> 
			<div style="padding:0 0 0 25px"><%=nmm %><br/><%=npm %><br>Tanggal Kelulusan : <input type="text" width="150px" name="tglls" placeholder="dd/mm/yyyy"/><br></div>telah memenuhi persyaratan kelulusan
		</div>	
			<br/>
			<section class="gradient">
				<div style="text-align:left">
				<button style="padding: 5px 50px;font-size: 20px;">Ajukan Kelulusan</button>
				</div>
			</section>
		</form>
        <%
