...
ListIterator li = v_status_doc_mutu.listIterator();
...
<style type="text/css">
	...
	table.StateTable tbody tr:hover td.nopad { background:<%=Constant.lightColorGrey() %> }*/
...
<%
	...
	while(li.hasNext()) {
		...
		String this_kdfak = st.nextToken();
		String this_kdpst = st.nextToken();
		String this_kdjen = st.nextToken();
		String this_nmpst = st.nextToken();
		String this_tot_doc = st.nextToken();
		String this_doc_uploaded = st.nextToken();
...
%>
	<table style="width:70%">
		...
		<th style="width:20%;background:#369;color:#fff;font-weight:bold;border-right:1px solid #369;padding:5px 5px">
				DOKUMEN TERUNGGAH <hr> TOTAL DOKUMEN
		</th> 
	</table>
...
<%			
			...
			while(lit.hasNext()) {
				...
				String nm_doc = stt.nextToken();
				String doc_status = stt.nextToken();
...
%>
		<tr onclick="window.open('go.prepDokumenMutu?kdpst_nmpst_kmp=<%=this_kdpst%>~<%=this_nmpst %>~PST&nm_doc=<%=nm_doc_tmp.toUpperCase()%>','popup','width=850,height=450')">		
			<td class="nopad" colspan="2" style="border:1px solid #369;text-align:left;padding:0 0 0 100px">
				<%=nm_doc %>
			</td>
			
			<%
			if(doc_status.equalsIgnoreCase("true")) {
			%>
				<td class="nopad" style="border:1px solid #369;font-size:1.5em;color:green">
			<%	
				out.print("&#10004");
			}
			else {
				%>
				<td class="nopad" style="border:1px solid #369;font-size:1.5em;color:red">
			<%	
				out.print("&#10006");
			}
			%></td>
