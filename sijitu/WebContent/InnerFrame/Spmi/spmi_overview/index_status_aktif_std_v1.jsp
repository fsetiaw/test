...
if(v_status_aktif_std!=null && v_status_aktif_std.size()>0) {
	ListIterator li = v_status_aktif_std.listIterator();
...
<%
	if(li.hasNext()) {
		...
		String prev_id_versi	= st.nextToken();
		%>
			<table class="StateTable" rules="all" cellpadding="0" cellspacing="0" style="padding:50px 0 50px 0;border:1px solid white;width:70%;background:white;">	
				...
				<tbody>
					...
					<%=no_std++ %>.
						...
						<td style="background:<%=Constant.lightColorBlu() %>;width:70%;color:#369;text-align:left;padding:0 0 0 10px;border:1px solid #369">
					<%=prev_ket_tipe_std %>
						</td>
						<td style="background:<%=Constant.lightColorBlu() %>;width:20%;color:#369;text-align:center;padding:0 0 0 10px;border:1px solid #369">
					<%
		if(!Checker.isStringNullOrEmpty(prev_tgl_sta)) {
			out.print("AKTIF");
		}
		else {
			out.print("NON-AKTIF");
