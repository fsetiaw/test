...
%>
	<div style="text-align:center">
		<h2 style="color:orange">BELUM ADA RIWAYAT KEGIATAN AMI UNTUK STANDAR INI</h2>
	</div>
<%
}
else {
	...
	Vector v_master = ToolSpmi.getIdDanNmStandar(Integer.parseInt(id_master_std)) ;
...
%>
		<th onclick="(function(){
					...
					location.href='get.histAmiProdiMasterStd?id_master_std=<%=id_master_std %>&fwdto=hist_ami_prodi&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>'})()"
          style="width:<%=(75/tot_master_std)%>%;color:#fff;font-weight:bold;border-right:1px solid #369;padding:5px 5px;font-size:0.9em">
				<%
				String tmp = nm_master.replace("STANDAR", "");
...
<%			
			...
			//id_std+"~"+
			String prev_id_std_ami = st.nextToken();
			//id_tipe+"~"+
			String prev_id_tipe_ami = st.nextToken();
			//nm_master+"~"+
			String prev_nm_master_ami = st.nextToken();
			//nm_std+"~"+~"+
			String prev_nm_std_ami = st.nextToken();
			//tot_nilai+"~"+
			...
			//max_nilai
			String prev_nilai_max = st.nextToken();
...
<%			
			...
			while(lim.hasNext() && !match) {
				...
				String id_std = st.nextToken();
				String nm_master = st.nextToken();	
				if(Integer.parseInt(prev_id_std_ami)==Integer.parseInt(id_std)) {
					match = true;
					
%>
			<td>
			<%=""+Converter.convertToPercentage(Double.parseDouble(prev_tot_nilai),Double.parseDouble(prev_nilai_max)) %> %
			</td>
<%					
				}
				else {
			...
			while(li.hasNext()) {
				...
				//id_std+"~"+
				String id_std_ami = st.nextToken();
				//id_tipe+"~"+
				String id_tipe_ami = st.nextToken();
				//nm_master+"~"+
				String nm_master_ami = st.nextToken();
				//nm_std+"~"+~"+
				String nm_std_ami = st.nextToken();
				//tot_nilai+"~"+
			...
			<%="0 %" %>
					...
					//id_std+"~"+
					prev_id_std_ami = id_std_ami;
					//id_tipe+"~"+
					prev_id_tipe_ami = id_tipe_ami;
					//nm_master+"~"+
					prev_nm_master_ami = nm_master_ami;
					//nm_std+"~"+~"+
					prev_nm_std_ami = nm_std_ami;
					//tot_nilai+"~"+
...
<%					
					...
					while(lim.hasNext() && !match) {
						...
						String id_std = st.nextToken();
						String nm_master = st.nextToken();	
						if(Integer.parseInt(id_std_ami)==Integer.parseInt(id_std)) {
							match = true;
		%>
					<td>
					<%=""+Converter.convertToPercentage(Double.parseDouble(tot_nilai),Double.parseDouble(nilai_max)) %> %
					</td>
		<%					
						}
						else {
