...
//--------------------------------------------
//System.out.println("form pelaksanaan");
...
if(v_QA!=null && v_QA.size()>0) {
	...
	//System.out.println("tot_QA_do="+tot_QA);
	//System.out.println("tot_pg_do="+tot_pg);
	...
	int next_page=0;
	int prev_page=0;
	//System.out.println("this_page="+this_page);
	if(this_page<=1) {
		...
		if(max_per_page>1) {
			...
			norut_max_this_pg=norut_max_this_pg;
		}
...
<form id="updateHasilAmi" action="go.updateHasilAmi" method="post">
	<input type='hidden' id='hiddenField' name='goto_index' value='false' />
	<input type="hidden" name="at_page" value="<%=at_page%>"/>
	<input type="hidden" name="tombol" id="tombol" value=""/>
...
%>			
			...
			<input type=image onclick="(function(){
           		//scroll(0,0);
				parent.scrollTo(0,0);      
        		var x = document.getElementById('wait');
             	...
             	document.getElementById('tombol').value='prev'; //passing parameter to hidden field
             	document.getElementById('updateHasilAmi').submit();
             	})();"
			   border="0" alt="PLAY" src="<%=Constants.getRootWeb() %>/Metro-UI-CSS-master/docs/images/blue_left_arrow.png" width="50" height="50" style="text-decoration: none;" /> 
			<!--  input name="tombol" id="tombol" type=image border="0" alt="PLAY" src="<%=Constants.getRootWeb() %>/Metro-UI-CSS-master/docs/images/blue_left_arrow.png" width="50" height="50" style="text-decoration: none;" value="prev" / -->
<%
	}
%>			
			</td>
			<td style="width:40%;text-align:center;vertical-align:middle">
			<a href="#" class="img" onclick="document.getElementById('updateHasilAmi').submit();"><img src="<%=Constants.getRootWeb() %>/Metro-UI-CSS-master/docs/images/save_changes_v1.png" width="150" height="40" title="SAVE"><br>
			hal: <%=at_page%>/<%=tot_pg %>
			</a><br>
			</td>
			<td style="width:30%;text-align:center">
...
%>		
			...
			<input type=image onclick="(function(){
        		...
        		var x = document.getElementById('wait');
             	...
             	x.style.display = 'block';
             	y.style.display = 'none';
             	document.getElementById('tombol').value='next'; //passing parameter to hidden field
             	document.getElementById('updateHasilAmi').submit();
             	})();"
			   border="0" alt="PLAY" src="<%=Constants.getRootWeb() %>/Metro-UI-CSS-master/docs/images/blue_right_arrow.png" width="50" height="50" style="text-decoration: none;" /> 
<%
	}
%>			
			</td>
	...
	<input type="hidden" name="dari_pelaksanaan_ami" value="true"/>
...
<table class="StateTable" rules="all" cellpadding="0" cellspacing="0" style="width:95%">
   	<thead>
      	...
      	<tr>
         	...
         	<th style="width:70%;text-align:left">Pertanyaan & Pilihan Jawaban</th>
         	<th style="width:20%">Bobot Penilaian</th>
      </tr>
   </thead>
   <tbody>
...
<%
	...
	while(li.hasNext()) {
		...
		//System.out.println("norut_min_this_pg="+norut_min_this_pg);
		//System.out.println("norut_max_this_pg="+norut_max_this_pg);
...
%>         
            <table class="CityTable" rules="all" cellpadding="0" cellspacing="0" style="width:100%">
               <thead>
                  ...
                  <tr>
                     <th style="width:10%;text-align:left;border:1px solid #1C79C6;border-right:none"><%=no_soal %>.</th>
                     <th colspan="2" style="text-align:left;border:1px solid #1C79C6;border-left:none"><%=question %></th>
                     
                  </tr>
               </thead>
               <tbody>
<%
			st = new StringTokenizer(tkn_jawaban,"`");
			StringTokenizer st1 = new StringTokenizer(tkn_bobot,"`");
			int norut_jawaban=0;
			while(st.hasMoreTokens()) {
				norut_jawaban++;
				String opt_jawaban = st.nextToken();
				String opt_bobot = st1.nextToken();
%>
					<tr>
						<input type="hidden" name="id_question<%=counter %>" value="<%=id_question%>"/>
                     <td style="font-weight:bold;color:black;width:10%;text-align:center;border:1px solid #1C79C6;border-right:none">
<%
				if(opt_jawaban.equalsIgnoreCase(jawaban)) {
				%>                     
             			<input type="radio" name="answer<%=counter %>" value="<%=opt_jawaban%>~<%=opt_bobot%>" checked="checked">
<%				
				}
				else {
%>                     
                     	<input type="radio" name="answer<%=counter %>" value="<%=opt_jawaban%>~<%=opt_bobot%>">
<%
				}
%>                     	
                     </td>
                     <td style="font-weight:bold;color:black;width:70%;text-align:left;border:1px solid #1C79C6;border-left:none;border-right:none"><%=opt_jawaban %></td>
                     <td style="font-weight:bold;color:black;width:20%;text-align:center;text-align:center;border:1px solid #1C79C6;border-left:none;background: #C7DBF1"><%=opt_bobot %></td>
                  </tr>
<%			
			}
%>
                  ...
                  <tr>
                     <td colspan="2" style="padding:0 0 0 10px;border:1px solid #1C79C6;border-right:none;border-bottom:none;background:#C7DBF1;font-weight:bold;color:black;text-align:left;vertical-align:top">Apakah terjadi pelanggaran</td>
                     <td style="border:1px solid #1C79C6;border-left:none;border-bottom:none;background:white;text-align:center">
                     	<select name="pelanggaran<%=counter %>" style="width:100%;height:35px;border:none;background:white;text-align-last:center">
<%
			if(pelanggaran.equalsIgnoreCase("0")) {
%>
...
%>                     	                     		
                  ...
                  <!--  tr>
                  	<td style="padding:0 10px 0 0;background:<%=Constant.lightColorGrey() %>;border:1px solid #1C79C6;border-right:none;border-bottom:none;border-top:none;font-weight:bold;color:black;text-align:right;vertical-align:top">Note :</td>
                     <td colspan="2" style="background:<%=Constant.lightColorGrey() %>;border:1px solid #1C79C6;border-left:none;border-bottom:none;border-top:none;text-align:center"><textarea name="note<%=counter %>" rows="" cols="" style="width:100%;border:none;height:70px"><%=Checker.pnn_v1(note_hasil_ami) %></textarea></td>
                  </tr -->
                  <tr>
                  	<td colspan="3" style="padding:10px 10px 0 10px;border:1px solid #1C79C6;border-right:none;border-top:none;background:<%=Constant.lightColorGrey() %>;font-weight:bold;color:black;text-align:left;vertical-align:top">Buat paragraph berisikan kondisi ataupun saran (optional) :
                  	</td>
                  </tr>
                  <tr>	
                     <td colspan="3" style="border:1px solid #1C79C6;border-left:none;border-top:none;background:<%=Constant.lightColorGrey() %>;text-align:left">
                     <%
                     if(!Checker.isStringNullOrEmpty(rekomendasi_hasil_ami)) {
                    	 %>
 						<textarea name="saran<%=counter %>" rows="" cols="" style="width:100%;border:none;height:150px;text-align:left"><%=rekomendasi_hasil_ami %></textarea>
 <%  
                     }
                     else {
%>
						<textarea name="saran<%=counter %>" rows="" cols="" style="width:100%;border:none;height:150px;text-align:left"></textarea>
<%                    	 
                     }
                     %></textarea></td>
                  </tr>
...
%>           
   ...
   <input type="hidden" name="ending_number" value="<%=(counter) %>"/>
</table>
</form>
