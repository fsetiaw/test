...
// UNTUK EDIT TGL RIL & DONE tidak digunakan
...
<%	
	...
	if(!Checker.isStringNullOrEmpty(anggota_tim)) {
		...
		int tot_data = st.countTokens();
		for(int i=0;i<tot_data;i++) {
			if(i==0) {
	%>
					<input type="text" name="anggota_tim" placeholder="isikan nama anggota tim" value="<%=st.nextToken() %>" style="text-align:center;height:30px;width:100%;border:1px solid #369" required/>
	<%				
			}
			else {
	%>
					<input type="text" name="anggota_tim" placeholder="isikan nama anggota tim" value="<%=st.nextToken() %>" style="text-align:center;height:30px;width:100%;border:1px solid #369"/>
	<%			
			}
		}
		for(;tot_data<=5;tot_data++) {
	%>
...
<%
			if(!Checker.isStringNullOrEmpty(id_cakupan_std)) {
				boolean match = false;
				st = new StringTokenizer(id_cakupan_std,",");
				while(st.hasMoreTokens() && !match) {
					String id = st.nextToken();
					if(id.equalsIgnoreCase(id_master_std)) {
						match=true;
					}
				}
				if(!match) {
				...
				<td colspan="3" style="padding:5px 0">
					<section class="gradient">
     					<div style="text-align:center">
           					<button style="padding: 5px 50px;font-size: 20px;">Update Rencana AMI</button>
     					</div>
					</section>
				</td>
			</tr>
