...
<%
	...
	if(!Checker.isStringNullOrEmpty(id_versi)) {
		ts.createFolderStructureStandarUmum(id_versi,"pelaksanaan");
		//File[] listFileFolder = (File[])session.getAttribute("listFileFolder");
		v_nm_alm_access = new Vector();
		li = v_nm_alm_access.listIterator();
		li.add("Folder Utama`"+ts.getRootStandarSpmiFolder(id_versi,"pelaksanaan")+"`reid");
		ts.createFolderRiwayatKegiatanUmum(id_versi, "pelaksanaan");
		ts.createFolderRiwayatKegiatanMonitoring(id_std, id_versi, kdpst);
	}
	//ts.createFolderStructureManualUmum(id_versi,"pelaksanaan");
	
	//ts.createFolderRiwayatKegiatanUmum(id_std, "pelaksanaan");
