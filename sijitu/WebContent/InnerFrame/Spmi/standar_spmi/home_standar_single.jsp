...
String tmp_at_page = "1";
String tmp_max_data_per_pg = "1";
...
//System.out.println("id_std_single="+id_std);
Vector v= ssm.getListSingleStandarIsi(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi));
		...
		$("#copyStd<%=i%>").submit(function(e) {	
	        ...
	        $.ajax({
        	    ...
        	    success:function(data){
        	        ...
        	        window.location.href = "go.getListAllStd?mode=start&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&atMenu=&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_page=<%=at_page %>&max_data_per_pg=<%=max_data_per_pg %>&at_menu=nama_std";
        	    	//window.location.href = "/InnerFrame/Spmi/standar_spmi/home_standar.jsp?id_master_std=0&id=0&kdpst_nmpst_kmp=<=kdpst_nmpst_kmp %>&at_page=<=at_page%>&max_data_per_pg=<max_data_per_pg%>";
        	    },
        	    error:function(){
...
<div id="wait" style="padding: 100px 0;text-align:center;position:absolute; width:99%; height:100%"><img src="<%=Constants.getRootWeb() %>/ajax/animation/ajax-loader.gif" alt="Wait ... " height="82" width="82">
...
<div class="colmask fullpage">
	...
	<%=Checker.pnn(Getter.getNamaTipeStandar(id_master_std, id_tipe_std))   %>
	<%
	if(true) {
	%> 
...
if(v!=null && v.size()>0) {
	...
	while(li.hasNext() && !done) {
		...
		if(norut>=urutan_pertama && norut<=urutan_akhir ) {
			...
			st.nextToken();//String id_versi = 
			st.nextToken();//String id_std_isi = 
			st.nextToken();//String id_std = st.nextToken();
			...
			<table style="border:none;width:99%">
				...
				<tr>
					...
					<%
				if(!disabled) {	
						//System.out.println("id_std edit="+id_std);
						%>
						<a class="img" href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_creator/form/std_isi/edit_index_single.jsp?first_index=1&mode=<%=mode%>&id_master_std=<%=id_master_std %>&id_tipe_std=<%=id_tipe_std %>&id_std_isi=<%=id_std_isi%>&id_std=<%=id_std%>&id_versi=<%=id_versi%>&atMenu=edit_isi&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_page=<%=at_page %>">
			...
			%>
						<a class="img2" href="go.deleteButirStd?mode=<%=mode%>&id_master_std=<%=id_master_std %>&id_tipe_std=<%=id_tipe_std %>&id_std_isi=<%=id_std_isi%>&id_std=<%=id_std%>&id_versi=<%=id_versi%>&atMenu=&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu=nama_std" style="text-decoration: none;" onclick="if (confirm('Apakah anda yakin akan menghapus isi standar ini ?!? ')) return true;return false; ">
							<img border="0" alt="HAPUS" src="<%=Constants.getRootWeb() %>/Metro-UI-CSS-master/docs/images/delete_icon.png" width="30" height="30">
						</a>	
					</td>
				</tr>
			</table>
	...
	%>		
			...
			<table class="table" id="tabel" style="width:100%">
				...
				<thead>
					<tr>
						<th  colspan="1" rowspan="2" style="border-right: 1px solid #369;vertical-align:middle;padding:0 5px;height:45px;width:5%;font-size:1.5em;font-weight:bold">
						(<%=norut %>)
						</th>
						<th colspan="9" style="border-right: 2px solid #369;border-left: 1px solid #369;text-align:center;vertical-align:middle;padding:5px 5px;width:95%;font-size:1.5em;font-weight:bold">
						PERNYATAAN ISI STANDAR <%=info_status_kdpst %>
						</th>
					</tr>
					<tr>
						<th colspan="9" style="border-left: 1px solid #369;text-align:center;vertical-align:middle;padding:0 0;width:95%;border:none;font-size:1em;font-weight:bold">
						<% 
				...
				<tbody>
					<tr>
						<td colspan="10" style="text-align:left;padding:10px 10px;font-size:1.3em">
							<%=Tool.cetakWebFormat(isi_std,"N/A")%>
						</td>
					</tr>
				</tbody>
				...
				<thead>
					<tr>
						...
						<%=Checker.pnn_v1(periode_start, "PERIODE")  %>
						</th>
						<th colspan="1" style="vertical-align:middle;padding:0 5px;height:35px;width:7%;border:1px solid #8AA2A4;font-size:0.9em;font-weight:bold">
						<%
					...
					<tr>
						<td colspan="2" style="text-align:center;padding:10px 0;font-size:0.9em;font-weight:bold">
							<%=Checker.pnn_v1(tkn_param.replace(",", "<br>"), "N/A")  %>
						</td>
						<td colspan="3" style="text-align:left;padding:10px 0;font-size:0.9em;font-weight:bold;padding:5px 5px">
							<%
							out.print(Tool.cetakWebFormat(tkn_indikator,"N/A"));
							%>
						</td>
						<td style="text-align:center;padding:10px 0;font-size:0.9em;font-weight:bold">
							<%=Checker.pnn_v1(target1, "N/A")  %>
							<%=Checker.pnn_v1(Checker.pnn_v1(unit_used_byTarget), "")  %>
						</td>
						<td style="text-align:center;padding:10px 0;font-size:0.9em;font-weight:bold">
							<%=Checker.pnn_v1(target2, "N/A")  %>
							<%=Checker.pnn_v1(Checker.pnn_v1(unit_used_byTarget), "")  %>
						</td>
						<td style="text-align:center;padding:10px 0;font-size:0.9em;font-weight:bold">
							<%=Checker.pnn_v1(target3, "N/A")  %>
							<%=Checker.pnn_v1(Checker.pnn_v1(unit_used_byTarget), "")  %>
						</td>
						<td style="text-align:center;padding:10px 0;font-size:0.9em;font-weight:bold">
							<%=Checker.pnn_v1(target4, "N/A")  %>
							<%=Checker.pnn_v1(Checker.pnn_v1(unit_used_byTarget), "")  %>
						</td>
						<td style="text-align:center;padding:10px 0;font-size:0.9em;font-weight:bold">
							<%=Checker.pnn_v1(target5, "N/A")  %>
							<%=Checker.pnn_v1(Checker.pnn_v1(unit_used_byTarget), "")  %>
						</td>
					</tr>
				<thead>	
					<tr>
						<th  colspan="10" style="text-align:left;vertical-align:middle;padding:0 5px;height:25px;border:0px solid #8AA2A;font-size:0.9em;font-weight:bold">
						STRATEGI PELAKSANAAN STANDAR
						</th>
					</tr>
					<tr>
						<td colspan="10" style="background:#fff;text-align:center;padding:10px 0;font-size:0.9em;font-weight:bold">
							...
							<td style="border:none;width:98%;text-align:left;padding:5px 2px 5px 5px">
								<%
								while(strategi.contains("\n")) {
									strategi=strategi.replace("\n", "<br>");
								}
								%>
								<%=Checker.pnn_v1(strategi, "Strategi pencapaian isi standar harap dijelaskan") %>
							</td>
						</tr>
							</table>
						</td>
