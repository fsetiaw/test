...
<%@ page import="beans.dbase.spmi.riwayat.pengendalian.SearchQandA" %>
...
<div id="upload">
		<iframe id="glue"  onload="myFunction()" src="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/dashArsipEvaluasi.jsp?kdpst=<%=kdpst %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>" seamless="seamless" width="100%" height="1000"  name="inner_iframe"></iframe>
</div>
<div id="ami">
<br>&nbsp<br>
<%
...
<%			
			...
			Vector v_list_std_isi = ssm.getListStandarIsi(kdpst, Integer.parseInt(id_std));
%>		
		<tr>
...
%>		
		<tr>
			<td colspan="3" style="backgroundtext-align:center;padding:0 0 0 0;background:#369;vertical-align:middle;font-size:1.5em;text-align:center;font-weight:bold;color:#fff">
<% 
			if(v_list_std_isi!=null) {
	%>
				PILIH STANDAR YANG AKAN DISIAPKAN<br>
	...
	//tmp = id_versi+"`"+id_std_isi+"`"+id_std+"`"+isi_std+"`"+butir+"`"+kdpst+"`"+rasionale+"`"+tgl_sta+"`"+tgl_end+"`"+target1+"`"+target2+"`"+target3+"`"+target4+"`"+target5+"`"+target6+"`"+pihak+"`"+dokumen+"`"+indikator+"`"+periode_start+"`"+unit_used_by_periode_start+"`"+besaran_interval_per_period+"`"+unit_used_byTarget+"`"+pengawas+"`"+param+"`"+aktif+"`"+cakupan+"`"+tgl_mulai_aktif+"`"+tgl_stop_aktif+"`"+tipe_survey+"`"+id_master+"`"+id_tipe;
		...
		%>
				<table class="StateTable" rules="all" cellpadding="0" cellspacing="0" style="width:100%">
					...
					<tr class="statetablerow">
						<td style="border:none;width:2%;text-align:left;padding:5px 0 5px 5px;background:<%=Constant.lightColorBlu()%>;color:#369;font-size:1.1em">
					<%=no++ %>.
						</td>
						<td colspan="3" style="border:none;width:98%;text-align:left;padding:5px 2px 5px 5px;background:<%=Constant.lightColorBlu()%>;color:#369;font-size:1.1em">
					<%
					if(isi_std_rs!=null) {
						while(isi_std_rs.contains("\n")) {
							isi_std_rs = isi_std_rs.replace("\n", "<br>");
		...
		%>			
					...
					<%
						if(spmi_editor) {
						%>		
							<td rowspan="1" style="width:100px;background:<%=Constant.lightColorGrey() %>;color:#369;text-align:center;font-size:1em">
							Bobot Nilai
							</td>
				<%	
						}
					...
					%>		
							<td rowspan="2" style="width:100px;background:<%=Constant.lightColorGrey() %>;color:#369;text-align:center;font-size:1em">
								Bobot Nilai
							</td>
					<%
						}
						...
						while(lit.hasNext()) {
							...
							String tkn_doc_terkait=(String)lit.next();
							String note=(String)lit.next();
					%>		
						</tr>
						<tr>
							<td rowspan="5" style="background:#fff;color:#369;text-align:center;font-size:0.9em">
							<%=nomor++ %>.
							</td>
					<%
							if(spmi_editor) {
				...
				<%	
					...
					%>		
							<td colspan="2" style="padding:0 0 0 5px;background:#fff;color:#369;text-align:left;font-size:0.9em">
							<%=pertanyaan %>
							</td>
					<%
							}
							if(spmi_editor) {
					%>
							<td style="border-left:0px;color:#fff;width:100px;background:#fff;color:#369;text-align:center;font-size:0.9em">
					...
					%>			
						...
						<tr>
					<%
							String jawab_nilai = (String)lit.next();
					
							System.out.println("jawab_nilai="+jawab_nilai);
							st = new StringTokenizer(jawab_nilai,"~");
							String jawaban = st.nextToken();
							String nilai = st.nextToken();
					%>	
							<td style="border-right:0px;color:#fff;background:#fff;color:#369;text-align:center;width:5%;font-size:0.9em">
							a.
							</td>
							<td style="border-left:0px;color:#fff;padding:0 0 0 5px;background:#fff;color:#369;text-align:left;width:70%;font-size:0.9em">
							<%=jawaban %>
							</td>
							<td style="background:#fff;color:#369;text-align:center;width:25%;font-size:0.9em">
							<%=nilai %>
							</td>
						</tr>
						<tr>
					<%
							jawab_nilai = (String)lit.next();
							st = new StringTokenizer(jawab_nilai,"~");
							jawaban = st.nextToken();
							nilai = st.nextToken();
					%>	
							<td style="border-right:0px;color:#fff;background:#fff;color:#369;text-align:center;font-size:0.9em">
							b.
							</td>
							<td style="border-left:0px;color:#fff;padding:0 0 0 5px;background:#fff;color:#369;text-align:left;font-size:0.9em">
							<%=jawaban %>
							</td>
							<td style="background:#fff;color:#369;text-align:center;font-size:0.9em">
							<%=nilai %>
							</td>
						</tr>
						<tr>
					...
					<%
							...
							if(!lit.hasNext()) {
								%>
							<tr>
								<td colspan="4" style="background:#fff;color:#369;text-align:center">
