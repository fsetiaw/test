...
//smm.autoInsertManual(Integer.parseInt(id_versi),Integer.parseInt(id_std_isi), "pengendalian");
%>
<style>
a.img:hover {
...
if(!Checker.isStringNullOrEmpty(darimana)&&darimana.equalsIgnoreCase("riwayat")) {
	%>
<body onload="window.resizeTo(screen.width, 4500);document.getElementById('chart').style.display='none';document.getElementById('form').style.display='block';document.links[7].className = 'active';document.links[6].className = '';document.links[8].className = '';document.links[9].className = '';document.getElementById('upload').style.display='none';document.getElementById('ami').style.display='none';">
<%
}
else if(!Checker.isStringNullOrEmpty(darimana)&&darimana.equalsIgnoreCase("ami")) {
	%>
<body onload="window.resizeTo(screen.width, 4500);document.getElementById('chart').style.display='none';document.getElementById('form').style.display='none';document.getElementById('upload').style.display='none';document.getElementById('ami').style.display='block';document.links[9].className = 'active';document.links[7].className = '';document.links[6].className = '';document.links[8].className = '';">
<%	
}
else {
%>
...
<div id="header">
<ul>
	<li><a href="#form" onclick="document.getElementById('chart').style.display='block';document.links[6].className = 'active';document.links[7].className = '';document.links[8].className = '';document.links[9].className = '';document.getElementById('form').style.display='none';document.getElementById('upload').style.display='none';document.getElementById('ami').style.display='none';" >MANUAL PENGENDALIAN<br>PELAKSANAAN STANDAR</a></li>
	<li><a href="#form" onclick="document.getElementById('chart').style.display='none';document.getElementById('form').style.display='block';document.links[7].className = 'active';document.links[6].className = '';document.links[8].className = '';document.links[9].className = '';document.getElementById('upload').style.display='none';document.getElementById('ami').style.display='none';" >AKTIFASI / EDIT / TAMBAH KEGIATAN<br>PENGENDALIAN PELAKSANAAN STANDAR</a></li>
	<li><a href="#form" onclick="document.getElementById('glue').contentWindow.location.reload();document.getElementById('chart').style.display='none';document.getElementById('form').style.display='none';document.getElementById('upload').style.display='block';document.links[8].className = 'active';document.links[9].className = '';document.links[7].className = '';document.links[6].className = '';document.getElementById('ami').style.display='none';" >ARSIP FILE<br>DOKUMEN</a></li>
	<li><a href="#form" onclick="document.getElementById('chart').style.display='none';document.getElementById('form').style.display='none';document.getElementById('upload').style.display='none';document.getElementById('ami').style.display='block';document.links[9].className = 'active';document.links[7].className = '';document.links[6].className = '';document.links[8].className = '';" >KEGIATAN PENGENDALIAN<br>AUDIT MUTU INTERNAL</a></li>
</ul>
...
Vector v_kendal= sm.searchListManualPengendalian(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi));
Vector v_best_manual_kendal = sm.getManualPengendalianAktifAtoLatestDraft(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi));
if(!editor) {	
%>
...
%>
	<thead>	
		...
		<tr>
			...
			PROSEDUR DAN LANGKAH PENGENDALIAN STANDAR
			</th>
...
boolean ada_penetapan_man= ts.apaSudahAdaPenetapanManual(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi));
...
%>
			<form action="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual/form_insert_manual_pengendalian_v1.jsp" method="post">
				...
				<input type="hidden" name="fwdto" value="dashboard_std_manual_pengendalian_kendali.jsp"/>
			...
			<!--  form action="#" method="post">
				...
				<input type="hidden" name="fwdto" value="dashboard_std_manual_pengendalian_evaluasi.jsp"/>
				<input type="hidden" name="at_menu_dash" value="<=at_menu_dash%>"/>
...
%>									
			...
			<td onclick="window.open('<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_pengendalian/riwayat_pengendalian.jsp?status_manual=<%=status_manual%>&starting_no=1&unit_used=<%=unit_used %>&norut_man=<%=norut_man %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&norut_man=<%=norut_man %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=riwayat&at_menu_dash=kendali','popup','width=850,height=600')" style="text-align:left;padding:0 2px;font-size:0.9em">
				<%=prosedur %>						
			</td>
			<td style="text-align:center;padding:0 2px;font-size:0.9em">
				...
				if(status_manual.equalsIgnoreCase("AKTIF")) {
					%>
				<a class="img2" href="set.aktivasiMan?status=aktif&tipe_manual=pengendalian&norut_man=<%=norut_man %>&id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_pengendalian_kendali.jsp" style="text-decoration: none;" onclick="if (confirm('Apakah anda yakin akan memberhentikan penggunaan manual no.<% %>=norut > ?!? \nNote:\n1. Anda tidak dapat mengaktifkan kembali manual yg sudah diberhentikan.\n2. Pemberhentian manual dilakukan bila manual digantikan dengan revisi yg baru')) return true;return false; ">	
					...
					%>
				<a class="img2" href="set.aktivasiMan?status=nonaktif&tipe_manual=pengendalian&norut_man=<%=norut_man %>&id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_pengendalian_kendali.jsp" style="text-decoration: none;" onclick="if (confirm('Apakah anda yakin akan memberhentikan penggunaan manual no.<% %>=norut > ?!? \nNote:\n1. Anda tidak dapat kengaktifkan kembali manual yg sudah diberhentikan.\n2. Pemberhentian manual dilakukan bila standard digantikan dengan revisi yg baru')) return true;return false; ">
					<img border="0" alt="STOP" src="<%=Constants.getRootWeb() %>/images/play_blue.png" width="30" height="30">
				</a>	
					<%
				}
				%>							
...
boolean ada_penetapan_man= ts.apaSudahAdaPenetapanManual(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi));
if(editor && !ada_manual_yg_bisa_diedit && ada_penetapan_man) {

	%>	
		<tr>
			<td colspan="3" style="text-align:center;padding:0 0 10px 10px;font-size:0.9em;valign:middle">
			<a class="img" href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual/form_insert_manual_pengendalian_v1.jsp?id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_pengendalian_kendali.jsp" style="vertical-align:middle;valign:middle">
...
boolean ada = ts.apaSudahAdaPenetapanManual(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi));
	if(!ada) {
		//belum ada kegiatan penetapan manual
		%>
		<p style="text-align:center;color:red;font-weight:bold;font-size:2.0em">
			Belum ada Penetapan Manual Standar
		</p>
		<p style="text-align:center;color:red;font-weight:italic;font-size:1.3em">
			Harap kembali dan selesaikan proses I: MANUAL PERENCANAAN STANDAR
		</p>
		<%		
	}
	else {
...
<h1 style="text-align:center;color:red">
	Belum Ada Manual Evaluasi Pelaksanaan Standar <br> Silahkan Input Manual Evaluasi Pelaksanaan Standar yang Sudah Dirumuskan Pada Proses Perencanaan Standar
</h1>
<%	
	}
}
	...
	if(li.hasNext()) {
		...
		String no_dok_spmi = ts.getPenomoranDokMutu(id_std_isi, id_versi, "MANUAL", "PENGENDALIAN");
%>	
	<br>&nbsp<br>
	...
	if(Checker.isStringNullOrEmpty(ts.getPenetapanTanggalManual(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi), Integer.parseInt(norut_man), "penetapan","perencanaan"))) {
			out.print("DRAFT ");
				...
				%>
	MANUAL PENGENDALIAN PELAKSANAAN STANDARD</h1>
	<br><br>
	<table class="table1" id="table1" style="width:100%">
		<thead>	
			<tr>
	...
	<table class="table1" id="table1" style="width:100%">
		<thead>	
			...
			<tr>
				<th style="text-align:left;vertical-align:middle;width:100%;font-size:1.5em;font-weight:bold;padding:0 0 0 5px">
					2. TUJUAN DAN MAKSUD DOKUMEN MANUAL PENGENDALIAN PELAKSANAAN 
				</th>
			</tr>
			<tr>	
			...
			<tr>
				<th style="text-align:left;vertical-align:middle;width:100%;font-size:1.5em;font-weight:bold;padding:0 0 0 5px">
					3. LUAS LINGKUP MANUAL PENGENDALIAN PELAKSANAAN
				</th>
			</tr>
			<tr>	
				<td style="background:white;text-align:left;vertical-align:middle;width:100%;font-size:0.9em;font-weight:bold;padding:5px 5px 5px 20px">
			...
			<tr>
				<th style="text-align:left;vertical-align:middle;width:100%;font-size:1.5em;font-weight:bold;padding:0 0 0 5px">
					4. DEFINISI ISTILAH TERKAIT MANUAL PENGENDALIAN PELAKSANAAN
				</th>
			</tr>
			<tr>	
				<td style="background:white;text-align:left;vertical-align:middle;width:100%;font-size:0.9em;font-weight:bold;padding:5px 5px 5px 20px">
			...
			<tr>
				<th style="text-align:left;vertical-align:middle;width:100%;font-size:1.5em;font-weight:bold;padding:0 0 0 5px">
					5. PROSEDUR DAN LANGKAH PENGENDALIAN PELAKSANAAN
				</th>
			</tr>
			<tr>	
				<td style="background:white;text-align:left;vertical-align:middle;width:100%;font-size:0.9em;font-weight:bold;padding:5px 5px 5px 20px">
...
<div id="upload">
	<iframe id="glue"  onload="myFunction()" src="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/dashArsipPengendalian.jsp?kdpst=<%=kdpst %>&id_std_isi=<%=id_std_isi %>&id_versi=<%=id_versi %>" seamless="seamless" width="100%" height="1000"  name="inner_iframe"></iframe>
</div>
<div id="ami">
<br>&nbsp<br>
<%
...
<%		
		//ada_penetapan_man= ts.apaSudahAdaPenetapanManual(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi));
 		ada_penetapan_man= ts.apaSudahAdaPenetapanManualUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std));
		boolean man_sdh_aktif= ts.apaManualSudahAktifUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std), "pengendalian");
...
%>									
		<tr>
			<td colspan="3" style="text-align:center;padding:0 0 10px 10px;font-size:0.9em;valign:middle">
			<a class="img" onclick="window.open('<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_pengendalian/riwayat_kendali_ami.jsp?status_manual=<%=status_manual%>&starting_no=1&unit_used=<%=unit_used %>&norut_man=<%=norut_man %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&norut_man=<%=norut_man %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=riwayat&at_menu_dash=kendali','popup','width=850,height=600')" style="text-align:left;padding:0 2px;font-size:0.9em">
				<img border="0" alt="Add Manual" src="<%=Constants.getRootWeb() %>/images/add_ico.png" width="50px" height="50px" style="vertical-align:bottom;font-size:1.5em">Riwayat Kegiatan Audit Mutu Internal 
			</a>		
			</td>
		</tr>
