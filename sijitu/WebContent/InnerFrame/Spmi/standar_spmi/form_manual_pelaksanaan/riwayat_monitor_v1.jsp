...
<body>
<jsp:include page="menu_man_mon.jsp" />
	...
	//if(((am_i_surveyor||team_spmi) && sdh_aktif)||editor) {
		...
		<tr>
			<td style="text-align:center;font-size:1.3em;font-weight:bold" align="center">
				...
				<form action="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/form_manual_pelaksanaan/form_survey_pelaksanaan_monitor_v1.jsp">
					...
					<input type="image" src="<%=Constants.getRootWeb() %>/images/add_ico.png" width="35" height="35" alt="Submit Form" onclick="this.form.submit()" />TAMBAH KEGIATAN MONITORING
				</form>
			</td>
		</tr>
		<%
...
%>
	...
	if((am_i_surveyor && sdh_aktif)||editor) {
		...
		<section style="align:center;text-align:center">	
			<p>
				<form action="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/form_manual_pelaksanaan/form_survey_pelaksanaan_monitor_v1.jsp" style="font-weight:bold;color:#369">
					<input type="hidden" name="scope_std" value="<%=scope_std %>" />
					<input type="hidden" name="std_kdpst" value="<%=std_kdpst %>" />
					<input type="hidden" name="id_versi" value="<%=id_versi %>" />
					<input type="hidden" name="id_std_isi" value="<%=id_std_isi %>" />
...
<table align="center" style="width:99%">
	<tr>
		<td colspan="8" style="font-size:2em;text-align:center;font-weight:bold;color:#369">
			RIWAYAT KEGIATAN MONITORING
		</td>
	</tr>

</table>			
<section style="text-align:center">	
		<jsp:include page="navigasi_mon_v1.jsp" flush="true" />
		</section>
<%	
	...
	if(li.hasNext()) {
		...
		while(li.hasNext()&& (no_yg_tampil <= max_yg_tampil)) {
			...
			//System.out.println("target_unit="+target_unit);
%>


	<table class="StateTable" rules="all" cellpadding="0" cellspacing="0" style="border:1px solid #369;width:99%;background:white;">
		<thead>
			<tr class="statetablerow">
				...
				<td class="nopad" colspan="2" style="vertical-align:middle;font-size:2em;text-align:center;font-weight:bold;color:#fff">
					<%=counter++%>.  KEGIATAN MONITORING <%=Converter.convertFormatTanggalKeFormatDeskriptif(tgl_eval)%> &nbsp&nbsp
					<%
				...
				if(!Checker.isStringNullOrEmpty(target_unit)) {
					...
					out.print(target_unit);
						...
						<tr>
							...
							<td style="background:#fff;border:1px solid #369;vertical-align:middle;text-align:center;font-size:1em;font-weight:bold;color:#000"><%=ril_val %></td>
						</tr>
					</table>
				</td>
			</tr>
		...
		<tbody-->	
			...
			<tr>
				...
				<td style="background:<%=Constant.darkColorBlu()%>;color:#fff;width:20%;border:1px solid <%=Constant.lightColorBlu()%>;padding:0 0 0 10px;text-align:left;font-weight:bold">
					AUDITOR / SURVEYOR
				</td>
				<td style="background:<%=Constant.lightColorBlu()%>;color:#000;width:75%;border:1px solid <%=Constant.darkColorBlu()%>;padding:0 0 0 10px;text-align:left;font-weight:bold">
...
<%				
	...
	<%
		...
		out.print("BELUM ADA RIWAYAT MONITORING");
	}
