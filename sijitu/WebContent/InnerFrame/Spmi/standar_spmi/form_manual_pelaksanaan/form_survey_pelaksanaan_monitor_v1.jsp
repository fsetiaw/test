...
SearchStandarMutu sm = new SearchStandarMutu();
...
Vector v = sm.getStandardInfo(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi));
...
<body>
<jsp:include page="menu_back_mon_v1.jsp" />
...
if(v!=null && v.size()>0) {
	...
	if(li.hasNext()) {
		...
		String no_butir  = (String)li.next();
		String kdpst_std_ignore = (String)li.next();
		String rasionale_std = (String)li.next();
		String aktif_di_std_tabel = (String)li.next();
		String tgl_aktif_di_std_tabel = (String)li.next();
		String tgl_stop_aktif_di_std_tabel = (String)li.next();
		String scope = (String)li.next();
		String tipe_proses_pengawasan = (String)li.next();
		id_versi = (String)li.next();
		String id_man_penetapan = (String)li.next();
		String id_man_pelaksanaan = (String)li.next();
		String id_man_evaluasi = (String)li.next();
		String id_man_pengendalian = (String)li.next();
		String id_man_peningkatan = (String)li.next();
		...
		String norut_tampil_ignore = (String)li.next();
		...
		String target_unit_thsms2 = new String(target_unit_thsms1);
		String target_unit_thsms3 = new String(target_unit_thsms1);
		String target_unit_thsms4 = new String(target_unit_thsms1);
		String target_unit_thsms5 = new String(target_unit_thsms1);
		String target_unit_thsms6 = new String(target_unit_thsms1);
		String monitoree = (String)li.next();
		String param = (String)li.next();
		String aktif = (String)li.next();
		String kdpst_bila_spesifik = (String)li.next();
		...
		if(Checker.isStringNullOrEmpty(target_thsms_sta)) {
			if(error_desc==null) {
				error_desc= new String();
			}
			else {
				error_desc=error_desc+"Error pada isi standar: Periode isi standar belum diisi dengan benar.<br>";
			}
		}
		if(Checker.isStringNullOrEmpty(satuan_periode)) {
			if(error_desc==null) {
				error_desc= new String();
			}
			else {
				error_desc=error_desc+"Error pada isi standar: Satuan Periode belum diisi dengan benar.<br>";
			}
		}
		try {
			Integer.parseInt(lama_per_period);
		}
		catch (Exception e) {
			if(error_desc==null) {
				error_desc= new String();
				...
				error_desc=error_desc+"Error pada isi standar: Lama per periode belum diisi dengan benar.<br>";
			}
		}
		//System.out.println("target_thsms_sta="+target_thsms_sta);
		//System.out.println("satuan_periode="+satuan_periode);
		//System.out.println("lama_per_period="+lama_per_period);
		if(Checker.isStringNullOrEmpty(error_desc)) {
...
<form action="go.updRiwayatMonitoring" method="post">
	...
	<!-- scope = prodi/univ/ -->
	<input type="hidden" name="scope_isi_std" value="<%=scope %>" />
	...
	%>			
			...
			<tr>
				<thead>
				<th colspan="6" style="font-size:1.3em;text-align:left;font-weight:bold;padding:0 0 0 10px">
					F. NAMA / NIP CIVITAS / KODE RUANGAN YG SEDANG DIMONITOR (OPTIONAL)
				</th>
				</thead>
			</tr>
			<tr style=";vertical-align:middle;">
				<td colspan="6" style="valign:top;font-size:1.3em;text-align:left;font-weight:bold;padding:0 0 0 0;vertical-align:middle">
				...
				<td colspan="6" style="padding:5px 0">
					<section class="gradient">
     					<div style="text-align:center">
           					<button style="padding: 5px 50px;font-size: 20px;">Insert Hasil MONITORING</button>
     					</div>
					</section>
				</td>
			</tr>
	...
	%>
		<div style="font-weight:bold;font-size:1.1em;color:red;text-align:center">
		<%
		error_desc = error_desc+"Harap Perbaiki/Edit Standarnya";
		out.print(error_desc); 
		%>
		</div>
	<%		
		}
