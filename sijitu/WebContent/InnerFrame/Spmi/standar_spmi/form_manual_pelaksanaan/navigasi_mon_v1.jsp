...
%>
		<td style="width:50%;text-align:left;padding:0 0 0 10px">
			<a href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_pelaksanaan/riwayat_monitor_v1.jsp?starting_no=<%=prev_sta_no %>&offset=<%=prev_offset %>&unit_used=<%=target_unit_used %>&scope_std=<%=scope_std%>&id_std_isi=<%=id_std_isi %>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=ami&at_menu_dash=do" style="font-weight:bold;font-size:1.6em" target="_self">Prev <</a>
		</td>	
<%
}
if(next) {
%>
		<td style="width:50%;text-align:right;padding:0 10px 0 0">
			<a href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_pelaksanaan/riwayat_monitor_v1.jsp?starting_no=<%=next_sta_no %>&offset=<%=next_offset %>&unit_used=<%=target_unit_used %>&scope_std=<%=scope_std%>&id_std_isi=<%=id_std_isi %>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=ami&at_menu_dash=do" style="font-weight:bold;font-size:1.5em" target="_self">> Next</a>
		</td>	
<%
}
%>
