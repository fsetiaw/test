...
%>
	...
	<input type="hidden" name="id_versi" value="<%=id_versi %>"/>
	<input type="hidden" name="id_std_isi" value="<%=id_std_isi %>"/>
	<input type="hidden" name="scope_std_isi" value="<%=scope_std_isi %>"/>
	<input type="hidden" name="id_std" value="<%=id_std %>"/>
	<input type="hidden" name="id_tipe_std" value="<%=id_tipe_std %>"/>
	<input type="hidden" name="id_master_std" value="<%=id_master_std %>" />
	<input type="hidden" name="norut_man" value="<%=norut_man %>"/>
	...
	<table class="StateTable" rules="all" cellpadding="0" cellspacing="0" style="border:none;width:99%;background:<%=Constant.lightColorBlu()%>;">
		<thead>
			<tr class="statetablerow">
				...
				<%
				String tanggal = new String(tgl_eval);
				while(tanggal.contains("-")) {
					tanggal = tanggal.replace("-", "~");
				}
				%>	
					<a href="#" onclick="window.open('<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/linkToArsipMonitor.jsp?kdpst=<%=kdpst %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&id_std_isi=<%=id_std_isi %>&id_versi=<%=id_versi %>&tgl_eval=<%=tanggal %>','popup','width=850,height=450')"><img src="<%=Constants.getRootWeb() %>/Metro-UI-CSS-master/docs/images/folder-doc-save.png" alt="Smiley face" height="50" width="50"></a>
				</td>
				<td class="nopad" colspan="2" style="vertical-align:middle;font-size:2em;text-align:center;font-weight:bold;color:#fff">
					<%=counter++%>.  KEGIATAN MONITORING <%=Converter.convertFormatTanggalKeFormatDeskriptif(tgl_eval)%> 
					<%
		...
		<tbody>	
			<tr>
				<td colspan="3">
					<table class="table" id="tabel" style="width:100%;border:1px solid <%=Constant.lightColorBlu()%>;">
						<tr>
							<td rowspan="2" colspan="1" style="border:1px solid #369;vertical-align:middle;width:25%;text-align:center;background:<%=Constant.lightColorGrey()%>;color:#369;padding:0 0 0 0;font-size:1.5em;font-weight:bold">
								PARAMETER YG DIEVALUASI
							</td>
							<td rowspan="2" colspan="1" style="border:1px solid #369;vertical-align:middle;width:55%;text-align:center;background:<%=Constant.lightColorGrey()%>;color:#369;padding:0 0 0 0;font-size:1.5em;font-weight:bold">
								INDIKATOR CAPAIAN
							</td>
							<td colspan="1" style="border:1px solid #369;width:10%;text-align:center;background:<%=Constant.lightColorGrey()%>;color:#369;font-size:1.25em;font-weight:bold">
								BESARAN TARGET
							</td>
							<td colspan="1" style="border:1px solid #369;width:10%;text-align:center;background:<%=Constant.lightColorGrey()%>;color:#369;font-size:1.25em;font-weight:bold">
								BESARAN RIL
						...
						<tr>
								<td colspan="2" style="border:1px solid #369;text-align:center;font-size:1.15em;background:<%=Constant.lightColorGrey()%>;color:#369;font-weight:bold">
								<%
				//if(!Checker.isStringNullOrEmpty(target_unit_used)) {
				//	out.print(target_unit_used);
				//}
				//else 
				if(!Checker.isStringNullOrEmpty(target_unit)) {
					out.print(target_unit);
					//System.out.println("target unitnya apa?"+target_unit);
				}
								%>
								</td>
						</tr>
						<tr>
							<td style="background:#fff;border:1px solid #369;vertical-align:top;text-align:center;padding:0 5px 0 10px;font-size:1em;font-weight:bold;color:#000"><%=param %></td>
							<td style="background:#fff;border:1px solid #369;vertical-align:top;text-align:justify;padding:0 5px 0 10px;font-size:1em;font-weight:bold;color:#000"><%=indikator %></td>
							<td style="background:#fff;border:1px solid #369;vertical-align:middle;text-align:center;font-size:1em;font-weight:bold;color:#000"><%=target_val %></td>
							<td style="background:#fff;border:1px solid #369;vertical-align:middle;text-align:center;font-size:1em;font-weight:bold;color:#000"><%=real_val %></td>
						</tr>
					</table>
				</td>
			</tr>
			...
			<tr>	
				...
				<td style="background:<%=Constant.lightColorBlu()%>;color:#000;width:75%;border:1px solid <%=Constant.lightColorBlu()%>;padding:0 0 0 10px;text-align:left;font-weight:bold">
					<%=Tool.cetakWebFormat(rekomendasi, "") %>
				</td>
			</tr>
			<%
			/*
...
<%
	...
	%>
							TGL SURVEY : <input type="text" name="tgl_kendal" value="<%=dt %>" placeholder="tgl/bln/thn" style="height:30px;vertical-align:middle;border:none;width:150px;text-align:center;font-size:1.2em" required/>
	<%	
					}
					else {
	%>
							TGL SURVEY : <input type="text" name="tgl_kendal" value="<%=tgl_kendal %>" placeholder="tgl/bln/thn" style="height:30px;vertical-align:middle;border:none;width:150px;text-align:center;font-size:1.2em" required/>
	<%
					}
%>															
														</td>
...
%>		
					...
					if(!Checker.isStringNullOrEmpty(target_id_eval) && target_id_eval.equalsIgnoreCase(id_eval) && !Checker.isStringNullOrEmpty(target_tindakan_kendal)) {
					%>								
												<textarea name="rasionale_kendal" placeholder="Jelaskan dasar pemikiran dalam menentukan tindakan pengendalian yang dilakukan" style="width:100%;height:100px;row:5;border:none" required="required"><%=target_tindakan_kendal %></textarea>
<%					
					}
					else {
%>								
...
<%
	...
	%>
							TGL SURVEY BERIKUTNYA : <input type="text" name="next_tgl_survey" value="" placeholder="tgl/bln/thn" style="height:30px;vertical-align:middle;border:none;width:150px;text-align:center;font-size:1.2em" required/>
	<%	
					}
					else {
	%>
							TGL SURVEY BERIKUTNYA : <input type="text" name="next_tgl_survey" value="<%=next_tgl_survey %>" placeholder="tgl/bln/thn" style="height:30px;vertical-align:middle;border:none;width:150px;text-align:center;font-size:1.2em" required/>
	<%
					}
%>															
														</td>
			...
			<tr>
				<td colspan="3" style="border: 1px solid <%=Constant.lightColorBlu()%>;vertical-align:middle;text-align:center;background:<%=Constant.lightColorBlu()%>;color:#fff;font-weight:bold">
					<table class="CityTable" rules="all" cellpadding="0" cellspacing="0" style="border: 1px solid <%=Constant.darkColorBlu()%>;vertical-align:middle;width:100%;background:<%=Constant.lightColorBlu()%>;color:#369;">
						<thead>
							<tr>
								<th class="blue" colspan="3" style="border: 1px solid <%=Constant.darkColorBlu()%>;vertical-align:middle;text-align:center;color:#fff;padding:0 0 0 0;font-size:2em;font-weight:bold">
