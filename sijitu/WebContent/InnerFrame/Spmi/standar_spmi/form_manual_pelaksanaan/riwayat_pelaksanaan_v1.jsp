...
String tgl_sta_man = ""+ts.getTglStartManualUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std), "pelaksanaan");
...
<body>
<jsp:include page="menu_man_pelaksanaan_v1.jsp" />
...
v_hist_plan=shk.getListRiwayatPelaksanaanUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std),offset,Integer.parseInt(src_manual_limit)-1);
...
%>	
	...
	if((team_spmi||editor)&&!man_expired) {	
		...
		if(man_sdh_aktif) {
		%>	
				<form action="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/form_manual_pelaksanaan/form_survey_pelaksanaan_v1.jsp">
		<%
		}
		%>			
...
%>
	...
	if(!Checker.isStringNullOrEmpty(tgl_tetap_man) && Checker.isStringNullOrEmpty(tgl_sta_man)&&!man_expired) {
		...
		<section style="align:center;text-align:center">	
			<p>
				<form action="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/form_manual_pelaksanaan/form_survey_pelaksanaan_v1.jsp" style="font-weight:bold;color:#369">
					<input type="hidden" name="status_manual" value="<%=status_manual %>" />
					<input type="hidden" name="scope_std" value="<%=scope_std %>" />
					<input type="hidden" name="std_kdpst" value="<%=std_kdpst %>" />
					<input type="hidden" name="id_versi" value="<%=id_versi %>" />
		...
		<tbody>	
			...
			<tr>
				...
				<td style="width:70%;background:#D7D7D7;color:#369;font-size:0.8em;text-align:center;padding:0 0 0 0">
					<select name="jenis_kegiatan" style="height:30px;width:100%;border:none;text-align-last:center;border-radius: 0px;" readonly="readonly">
			<%
					/*
			%>			
				...
				*/
				if(jenis_kegiatan_hist!=null && jenis_kegiatan_hist.equalsIgnoreCase("kendali_std")) {
					kegiatan = "pengendalian";
					%>
						<option value="kendali_std" selected="selected">Pelaksanaan Standar</option>
					<%	
				}
				else {
				%>
...
%>				
<form action="go.updRiwayatHasilPelaksanaan_v1" method="post">
	<input type="hidden" name="status_manual" value="<%=status_manual %>" />
	<input type="hidden" name="target_unit_used" value="<%=target_unit_used %>" />
	<input type="hidden" name="kdpst_nmpst_kmp" value="<%=kdpst_nmpst_kmp %>" />
	<input type="hidden" name="id_versi" value="<%=id_versi %>" />
