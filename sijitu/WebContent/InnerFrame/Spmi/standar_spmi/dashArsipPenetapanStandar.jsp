...
String man_or_std = request.getParameter("man_or_std");
String kegiatan = request.getParameter("kegiatan");
String target_folder = request.getParameter("target_folder");
String origin_folder = (String)session.getAttribute("origin_folder");
//System.out.println("target_folder="+target_folder);
//System.out.println("origin_folder="+origin_folder);
Vector v_nm_alm_access=null;
ListIterator li=null;
if(!Checker.isStringNullOrEmpty(id_versi)) {
	ts.createFolderStructureManualUmum(id_versi,"penetapan");
	ts.createFolderStructureStandarUmum(id_versi,"penetapan");
	//File[] listFileFolder = (File[])session.getAttribute("listFileFolder");
	v_nm_alm_access = new Vector();
	li = v_nm_alm_access.listIterator();
	li.add("Folder Utama`"+ts.getRootSpmiFolder(id_versi,"penetapan")+"/"+man_or_std+"/"+kegiatan+"/"+target_folder+"`reid");
	//li.add("Folder Utama`"+ts.getRootSpmiFolder()+"`reid");
	ts.createFolderRiwayatKegiatanUmum(id_versi, "penetapan");
