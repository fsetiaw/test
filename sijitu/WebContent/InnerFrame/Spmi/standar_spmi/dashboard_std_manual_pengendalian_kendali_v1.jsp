...
<div id="header">
<ul>
	<li><a href="#form" onclick="document.getElementById('chart').style.display='block';document.links[6].className = 'active';document.links[7].className = '';document.links[8].className = '';document.links[9].className = '';document.getElementById('form1').style.display='none';document.getElementById('upload').style.display='none';document.getElementById('ami').style.display='none';" >MANUAL PENGENDALIAN<br>PELAKSANAAN STANDAR</a></li>
	<li><a href="#form" onclick="document.getElementById('chart').style.display='none';document.getElementById('form1').style.display='block';document.links[7].className = 'active';document.links[6].className = '';document.links[8].className = '';document.links[9].className = '';document.getElementById('upload').style.display='none';document.getElementById('ami').style.display='none';" >KEGIATAN PENGENDALIAN<br>PELAKSANAAN STANDAR</a></li>
	<li><a href="#form" onclick="document.getElementById('glue').contentWindow.location.reload();document.getElementById('chart').style.display='none';document.getElementById('form1').style.display='none';document.getElementById('upload').style.display='block';document.links[8].className = 'active';document.links[9].className = '';document.links[7].className = '';document.links[6].className = '';document.getElementById('ami').style.display='none';" >ARSIP FILE<br>DOKUMEN</a></li>
<%
/*
%>	
	<li><a href="#form" onclick="document.getElementById('chart').style.display='none';document.getElementById('form1').style.display='none';document.getElementById('upload').style.display='none';document.getElementById('ami').style.display='block';document.links[9].className = 'active';document.links[7].className = '';document.links[6].className = '';document.links[8].className = '';" >KEGIATAN PENGENDALIAN<br>AUDIT MUTU INTERNAL</a></li>
<%
*/
%>
</ul>
...
Vector v_kendal= sm.searchListManualPengendalianUmum(Integer.parseInt(id_std));
...
%>	
		<tr>
			<td colspan="3" style="text-align:center;padding:0 0;font-size:0.9em">
				Belum Ada Manual Pengendalian Pelaksanaan Standar <br> Silahkan Input Manual Pelaksanaan Standar yang Sudah Dirumuskan Pada Proses Perencanaan Standar					
			</td>
		</tr>
<%
	}
...
%>
			<form action="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual/form_insert_manual_pengendalian_v1.jsp" method="post">
				...
				<input type="hidden" name="fwdto" value="dashboard_std_manual_pengendalian_kendali_v1.jsp"/>
...
%>									
			...
			<td onclick="window.location.href='<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_pengendalian/riwayat_pengendalian_v1.jsp?status_manual=<%=status_manual%>&starting_no=1&unit_used=<%=unit_used %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=riwayat&at_menu_dash=kendali'" style="text-align:left;padding:0 2px;font-size:0.9em;padding:0 0 0 5px">
				<%=prosedur %>						
			</td>
			<td style="text-align:center;padding:0 2px;font-size:0.9em">
				...
				if(status_manual.equalsIgnoreCase("AKTIF")) {
					%>                
				<a class="img2" href="set.aktivasiMan_v1?status=aktif&tipe_manual=pengendalian&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_pengendalian_kendali_v1.jsp" style="text-decoration: none;" onclick="if (confirm('Apakah anda yakin akan memberhentikan penggunaan manual ini ?!? \nNote:\n1. Anda tidak dapat mengaktifkan kembali manual yg sudah diberhentikan.\n2. Pemberhentian manual dilakukan bila manual digantikan dengan revisi yg baru')) return true;return false; ">	
					...
					%>
				<a class="img2" href="set.aktivasiMan_v1?status=nonaktif&tipe_manual=pengendalian&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_pengendalian_kendali_v1.jsp" style="text-decoration: none;" onclick="if (confirm('Apakah anda yakin akan mengaktifkan penggunaan manual ini ?!? \nSyarat Aktivasi:\n1. Draf manual telah melalui kegiatan perumusan, pemeriksaan, persetujuan dan penetapan.\n2. Pastikan draf manual telah diisi dengan benar, karena manual yg telah diaktifkan tidak dapat diedit')) return true;return false; ">
...
if(false) {
	%>	
		<tr>
			...
			<a class="img" href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual/form_insert_manual_pengendalian_v1.jsp?mode=insert&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_pengendalian_kendali_v1.jsp" style="vertical-align:middle;valign:middle">
...
if(v_kendal==null || v_kendal.size()<1) {
	...
	<h1 style="text-align:center;color:red">
		BELUM ADA MANUAL PENGENDALIAN PELAKSANAAN STANDAR
	</h1>
	<table width="100%;border:none">
	<tr>	
		<td colspan="3" style="text-align:center;padding:0 0 10px 10px;font-size:0.9em;valign:middle">
			<a class="hover" href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual/form_insert_manual_pengendalian_v1.jsp?mode=insert&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_pengendalian_kendali_v1.jsp" style="text-decoration: none;" >
					<img border="0" alt="Add Manual" src="<%=Constants.getRootWeb() %>/images/add_ico.png" width="50px" height="50px">Tambah Manual Baru2
			</a>		
		</td>
	...
	if(li.hasNext()) {
		...
		//versi_id+"~"+std_isi_id+"~"+norut+"~"+tgl_sta+"~"+tgl_end+"~"+tkn_jab_rumus+"~"+tkn_jab_cek+"~"+tkn_jab_stuju+"~"+tkn_jab_tetap+"~"+tkn_jab_kendal+"~"+tujuan+"~"+lingkup+"~"+definisi+"~"+prosedur+"~"+kualifikasi+"~"+dokuman+"~"+referensi;
		String id_versi = st.nextToken();
		st.nextToken();//id_std ignore = request para
		String tgl_sta = st.nextToken();
		String tgl_end = st.nextToken();
		String tkn_jab_rumus = st.nextToken();
		...
		boolean man_expired = ts.apaManualSudahExpiredUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std), "PENGENDALIAN");
		String no_dok_spmi = ts.getPenomoranDokMutuUmum(id_std, "MANUAL", "PENGENDALIAN");
%>	
	<br>&nbsp<br>
<%
	...
	%>			
				...
				if(spmi_editor) {
					...
					<a href="#" onclick="if(confirm('Copy/Salin manual versi <%=id_versi%> ini untuk seluruh standar?\nCatatan:\nBila standar sudah memiliki manual versi <%=id_versi%> maka akan digantikan dengan salinan ini.'))(function(){
						...
						location.href='set.copyManForAll?status=aktif&tipe_manual=pengendalian&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_pengendalian_kendali_v1.jsp'
						})();return false;"
					>
...
<div id="upload">
	<%
	session.setAttribute("ppepp_asal", "pengendalian");
	...
	%>
	<iframe id="glue"  onload="myFunction()" src="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/dashArsipPengendalian.jsp?kdpst=<%=kdpst %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&id_versi=<%=versi_standar %>" seamless="seamless" width="100%" height="1000"  name="inner_iframe"></iframe>
</div>
<div id="ami">
<br>&nbsp<br>
<%
...
%>									
	...
	//tmp = id_versi+"`"+id_std_isi+"`"+id_std+"`"+isi_std+"`"+butir+"`"+kdpst+"`"+rasionale+"`"+tgl_sta+"`"+tgl_end+"`"+target1+"`"+target2+"`"+target3+"`"+target4+"`"+target5+"`"+target6+"`"+pihak+"`"+dokumen+"`"+indikator+"`"+periode_start+"`"+unit_used_by_periode_start+"`"+besaran_interval_per_period+"`"+unit_used_byTarget+"`"+pengawas+"`"+param+"`"+aktif+"`"+cakupan+"`"+tgl_mulai_aktif+"`"+tgl_stop_aktif+"`"+tipe_survey+"`"+id_master+"`"+id_tipe;
		...
		%>
			<tr onclick="window.location.href='<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_pengendalian/riwayat_kendali_ami_v1.jsp?backto=<%=backto %>&status_manual=<%=status_manual%>&starting_no=1&unit_used=<%=unit_used %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi_rs %>&id_tipe_std=<%=id_tipe_rs %>&id_master_std=<%=id_master_rs %>&id_std=<%=id_std_rs %>&id_std_isi=<%=id_std_isi_rs %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=riwayat&at_menu_dash=kendali'" style="text-align:left;padding:0 2px;font-size:0.9em;padding:0 0 0 5px">
				<td style="border:none;width:2%;text-align:left;padding:5px 0 5px 5px">
					<%=no++ %>.
				</td>
