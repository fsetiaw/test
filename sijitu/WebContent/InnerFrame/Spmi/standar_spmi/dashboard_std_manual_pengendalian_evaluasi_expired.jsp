...
<ul>
	...
	<li><a href="#" onclick="document.getElementById('chart').style.display='block';document.getElementById('form').style.display='none';document.getElementById('upload').style.display='none';" >MANUAL EVALUASI<br>PELAKSANAAN</a></li>	
	<li><a href="#" onclick="document.getElementById('chart').style.display='none';document.getElementById('form').style.display='block';document.getElementById('upload').style.display='none';" >FORM UPDATE MANUAL<br>EVALUASI PELAKSANAAN</a></li>
	<li><a href="#" onclick="document.getElementById('chart').style.display='none';document.getElementById('form').style.display='none';document.getElementById('upload').style.display='block';" >ARSIP FILE<br>DOKUMEN</a></li>
</ul>
...
%>	
		...
		<tr onclick="window.open('<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_evaluasi/riwayat_evaluasi.jsp?am_i_controller=<%=am_i_controller %>&am_i_surveyor=<%=am_i_surveyor %>&am_i_pengawas=<%=am_i_pengawas %>&am_i_terkait=<%=am_i_pengawas %>&jabatan_pengendali=<%=jabatan_pengawas %>&jabatan_inputer=<%=jabatan_inputer%>&catat_civitas=<%=catat_civitas %>&tipe_sarpras=<%=tipe_sarpras %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&id_std=<%=id_std %>&id_kendali=<%=id_kendali %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&unit_used=<%=unit_used%>&at_menu_kendal=riwayat&at_menu_dash=control','popup','width=850,height=600')">
...
if(editor) {
	%>	
		<tr>
			...
			<a class="img" href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual/form_insert_manual_evaluasi.jsp?id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_pengendalian_evaluasi.jsp" style="vertical-align:middle;valign:middle">
...
<div id="chart">
	<jsp:include page="${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/Flowchart/manual_ep.jsp" />
</div>
<div id="upload">
	uploader file
</div>
