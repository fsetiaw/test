...
<div id="header">
	...
	<ul>
	<li><a href="get.prepInfoMan_v1?id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_perencanaan_v1.jsp&darimana=riwayat">BACK <span><b style="color:#eee">&nbsp</b></span></a></li>
<%
if(at_menu_kendal.equalsIgnoreCase("riwayat")) {
%>	
                                            
	<li><a href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_perencanaan/riwayat_perencanaan_v1.jsp?status_manual=<%=status_manual %>&starting_no=1&unit_used=<%=target_unit_used %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=riwayat&at_menu_dash=<%=at_menu_dash %>" class="active">RIWAYAT<span>PERENCANAAN</span></a></li>
<%
}
else {
%>	
	<li><a href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_perencanaan/riwayat_perencanaan_v1.jsp?status_manual=<%=status_manual %>&starting_no=1&unit_used=<%=target_unit_used %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=riwayat&at_menu_dash=<%=at_menu_dash %>">RIWAYAT<span>PERENCANAAN</span></a></li>
<%	
}
