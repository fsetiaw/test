...
String kegiatan = "";	
String ppepp = "penetapan";
//System.out.println("id_versi="+id_versi);
//System.out.println("id_std="+id_std);
ToolSpmi ts = new ToolSpmi();
String tgl_sta_man = ""+ts.getTglStartManualUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std), "perencanaan");
String tgl_rumus_man = ""+ts.getPenetapanTanggalManualUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std), "perumusan","perencanaan");
//System.out.println("tgl_rumus_man="+tgl_rumus_man);
String tgl_cek_man = ""+ts.getPenetapanTanggalManualUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std), "pemeriksaan","perencanaan");
//System.out.println("tgl_cek_man="+tgl_cek_man);
String tgl_stuju_man = ""+ts.getPenetapanTanggalManualUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std), "persetujuan","perencanaan");
//System.out.println("tgl_stuju_man="+tgl_stuju_man);
...
boolean manual_valid_to_start=false;
if(!Checker.isStringNullOrEmpty(tgl_rumus_man)&&!Checker.isStringNullOrEmpty(tgl_cek_man)&&!Checker.isStringNullOrEmpty(tgl_stuju_man)&&!Checker.isStringNullOrEmpty(tgl_tetap_man)) {
	manual_valid_to_start=true;
}
//System.out.println("manual_valid_to_start="+manual_valid_to_start);
boolean man_expired = ts.apaManualSudahExpiredUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std), "PERENCANAAN");

//System.out.println("man_expired="+man_expired);
//System.out.println("tgl_tetap_man="+tgl_tetap_man);
...
<body>
<jsp:include page="menu_man_perencanaan_v1.jsp" />
...
*/
SrcHistPerencanaan shk = new SrcHistPerencanaan();
//System.out.println("offset="+offset);

v_hist_plan=shk.getListRiwayatPerencanaanUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std),offset,Integer.parseInt(src_manual_limit)-1);
...
%>	
		...
		<tr>
			<td colspan="7" style="text-align:center;padding:0 0;font-size:2.0em;font-weight:bold">
				Belum Ada Riwayat Kegiatan Penetapan						
			</td>
		</tr>
		<%
	...
	//if(editor) {
		...
		<tr>
			<td style="text-align:center;font-size:1.3em;font-weight:bold" align="center">
				...
				<form action="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/form_manual_perencanaan/form_survey_perencanaan_v1.jsp">
		...
		<%
		if(!manual_valid_to_start) {
		%>
					TAMBAH KEGIATAN PENETAPAN MANUAL<br>Untuk <i>Penetapan Standar</i> dilakukan setelah ada kegiatan perumusan, pemeriksaan, persetujaun dan penetapan manual standar.
		<%
		}
		else {
			%>
					TAMBAH KEGIATAN PENETAPAN STANDAR
		<%	
		}
		%>			
				</form>
		...
		<tr>
			<td style="text-align:center;font-size:1.3em;font-weight:bold" align="center">
				<input type="image" src="<%=Constants.getRootWeb() %>/images/add_ico.png" width="35" height="35" alt="Submit Form" onclick="this.form.submit()" />
				TAMBAH KEGIATAN PENETAPAN MANUAL<br>[Anda tidak memiliki akses untuk menambah kegiatan]
			</td>
		</tr>			
		<%
		}
...
%>
	...
	//if(manual_valid_to_start && Checker.isStringNullOrEmpty(tgl_sta_man)&&!man_expired) {
	if(false) {	
		...
		<section style="align:center;text-align:center;font-weight:bold;color:red">	
			<p style="align:center;text-align:center;font-weight:bold;color:#369">	
			<input type="image" src="<%=Constants.getRootWeb() %>/images/add_ico.png" width="35" height="35" alt="Submit Form" />TAMBAH KEGIATAN PENETAPAN STANDAR
			<br>
		...
		<section style="align:center;text-align:center">	
			<p>
				<form action="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/form_manual_perencanaan/form_survey_perencanaan_v1.jsp" style="font-weight:bold;color:#369">
					<input type="hidden" name="status_manual" value="<%=status_manual %>" />
					<input type="hidden" name="scope_std" value="<%=scope_std %>" />
					<input type="hidden" name="std_kdpst" value="<%=std_kdpst %>" />
					<input type="hidden" name="id_versi" value="<%=id_versi %>" />
			...
			if(!manual_valid_to_start) {
		%>
					TAMBAH KEGIATAN PENETAPAN MANUAL / STANDAR<br>Untuk <i>Penetapan Standar</i> dilakukan setelah ada kegiatan perumusan, pemeriksaan, persetujaun dan penetapan manual standar.
		<%
			}
			else {
			%>
...
<table align="center" style="width:100%">
	<tr>
		<td colspan="8" style="font-size:2em;text-align:center;font-weight:bold;color:#369">
			RIWAYAT PENETAPAN MANUAL & STANDAR
		</td>
	</tr>
...
<%	
	...
	if(li.hasNext()) {
		...
		while(li.hasNext()&& (no_yg_tampil <= max_yg_tampil)) {
			...
			String tgl_rumus_std_set_hist=st.nextToken();
			String tgl_cek_std_set_hist=st.nextToken();
			String tgl_stuju_std_set_hist=st.nextToken();
			String tgl_tetap_std_set_hist=st.nextToken();
			String tgl_kendali_std_set_hist=st.nextToken();
...
%>
<form action="go.updRiwayatPerencanaan_v1" method="post">
	<input type="hidden" name="status_manual" value="<%=status_manual %>" />
	<input type="hidden" name="target_unit_used" value="<%=target_unit_used %>" />
	<input type="hidden" name="kdpst_nmpst_kmp" value="<%=kdpst_nmpst_kmp %>" />
	<input type="hidden" name="id_versi" value="<%=id_versi %>" />
...
%>				
<form action="go.updRiwayatHasilPerencanaan_v1" method="post">
	<input type="hidden" name="status_manual" value="<%=status_manual %>" />
	<input type="hidden" name="target_unit_used" value="<%=target_unit_used %>" />
	<input type="hidden" name="kdpst_nmpst_kmp" value="<%=kdpst_nmpst_kmp %>" />
	<input type="hidden" name="id_versi" value="<%=id_versi %>" />
	...
	<table class="StateTable" rules="all" cellpadding="0" cellspacing="0" style="border:none;width:99%;background:white;">
		<thead>
			...
			<tr class="statetablerow">
				<td colspan="2" style="background:#369;color:#fff;valign:middle;font-size:1.2em;text-align:center;font-weight:bold;padding:0 0 2px 0;vertical-align:middle">
		...
		<tbody>	
			...
			<tr>
				...
				<td style="font-weight:bold;border: 1px solid #fff;background:<%=Constant.lightColorBlu()%>;color:#369;font-size:1em;text-align:left;padding:0 0 0 10px">
				<%
				isi_kegiatan_hist=Tool.cetakWebFormat(isi_kegiatan_hist, "");
				out.print(isi_kegiatan_hist); 
				%>
				</td>
			</tr>
			<tr>
			...
			%>					
				...
				if(!Checker.isStringNullOrEmpty(jenis_kegiatan_hist)) {
					...
					if(jenis_kegiatan_hist.equalsIgnoreCase("rumus")) {
						jenis_tmp = "PERUMUSAN MANUAL";
						kegiatan = "perumusan";
					}
					else if(jenis_kegiatan_hist.equalsIgnoreCase("cek")) {
						jenis_tmp = "PEMERIKSAAN MANUAL";
						kegiatan = "pemeriksaan";
					}
					else if(jenis_kegiatan_hist.equalsIgnoreCase("stuju")) {
						jenis_tmp = "PERSETUJUAN MANUAL";
						kegiatan = "persetujuan";
					}
					else if(jenis_kegiatan_hist.equalsIgnoreCase("tetap")) {
						jenis_tmp = "PENETAPAN MANUAL";
						kegiatan = "penetapan";
					}
					else if(jenis_kegiatan_hist.equalsIgnoreCase("kendali")) {
						jenis_tmp = "PENGENDALIAN MANUAL";
						kegiatan = "pengendalian";
					}
					else if(jenis_kegiatan_hist.equalsIgnoreCase("rumus_std")) {
						jenis_tmp = "PERUMUSAN STANDAR";
						kegiatan = "perumusan";
					}
					else if(jenis_kegiatan_hist.equalsIgnoreCase("cek_std")) {
						jenis_tmp = "PEMERIKSAAN STANDAR";
						kegiatan = "pemeriksaan";
					}
					else if(jenis_kegiatan_hist.equalsIgnoreCase("stuju_std")) {
						jenis_tmp = "PERSETUJUAN STANDAR";
						kegiatan = "persetujuan";
					}
					else if(jenis_kegiatan_hist.equalsIgnoreCase("tetap_std")) {
						jenis_tmp = "PENETAPAN STANDAR";
						kegiatan = "penetapan";
					}
					else if(jenis_kegiatan_hist.equalsIgnoreCase("kendali_std")) {
						jenis_tmp = "PENGENDALIAN STANDAR";
						kegiatan = "pengendalian";
					}
				%>
										<tr>
											...
											<td style="width:25%;border: 1px solid #FE9A2E;vertical-align:top;text-align:left;background:#F5D0A9;color:#369;padding:0 5px 0 10px;font-size:1em;font-weight:bold">
												TETAPKAN SEBAGAI TANGGAL<br>PENETAPAN <%=jenis_tmp.toUpperCase() %>
											</td>
											<td style="border: 1px solid #FE9A2E;width:75%;background:#fff;color:#369;font-size:1em;text-align:left;padding:0 5px 0 10px">
				
				<%
					if(tgl_rumus_set_hist.equalsIgnoreCase("1") || tgl_cek_set_hist.equalsIgnoreCase("1") || tgl_stuju_set_hist.equalsIgnoreCase("1") || tgl_tetap_set_hist.equalsIgnoreCase("1") || tgl_kendali_set_hist.equalsIgnoreCase("1")) {
				%>									
												<input type="radio" name="penetapan_tgl" value="<%=jenis_kegiatan_hist%>" checked="checked"> Ya
				<%
					}
					else {
				%>								
												<input type="radio" name="penetapan_tgl" value="<%=jenis_kegiatan_hist%>"> Ya
				<%
					}									
				%>
												<br>
				<%
					if(tgl_rumus_set_hist.equalsIgnoreCase("0") && tgl_cek_set_hist.equalsIgnoreCase("0") && tgl_stuju_set_hist.equalsIgnoreCase("0") && tgl_tetap_set_hist.equalsIgnoreCase("0") && tgl_kendali_set_hist.equalsIgnoreCase("0")) {
				%>									
												<input type="radio" name="penetapan_tgl" value="no" checked="checked"> Tidak
				<%
					}
					else {
				%>										
												<input type="radio" name="penetapan_tgl" value="no"> Tidak
				<%
					}									
				%>							
...
%>
			...
			<%
				...
				if(!Checker.isStringNullOrEmpty(jenis_kegiatan_hist)&&(tgl_rumus_set_hist.equalsIgnoreCase("1")||tgl_cek_set_hist.equalsIgnoreCase("1")||tgl_stuju_set_hist.equalsIgnoreCase("1")||tgl_tetap_set_hist.equalsIgnoreCase("1")||tgl_kendali_set_hist.equalsIgnoreCase("1"))
					||
					(tgl_rumus_std_set_hist.equalsIgnoreCase("1")||tgl_cek_std_set_hist.equalsIgnoreCase("1")||tgl_stuju_std_set_hist.equalsIgnoreCase("1")||tgl_tetap_std_set_hist.equalsIgnoreCase("1")||tgl_kendali_std_set_hist.equalsIgnoreCase("1"))) {
					String jenis_tmp = "";
					if(jenis_kegiatan_hist.equalsIgnoreCase("rumus")) {
						jenis_tmp = "PERUMUSAN MANUAL";
						kegiatan = "perumusan";
