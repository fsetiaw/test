...
String tgl_sta_man = ""+ts.getTglStartManual(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi), Integer.parseInt(norut_man), "pengendalian");
...
<body>
<jsp:include page="menu_man_kendal.jsp" />
...
v_hist_plan=shk.getListRiwayatPengendalian(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi), Integer.parseInt(norut_man),offset,Integer.parseInt(src_manual_limit)-1);
...
%>	
		...
		<tr>
			<td colspan="7" style="text-align:center;padding:0 0;font-size:2.0em;font-weight:bold">
				Belum Ada Riwayat Kegiatan Pengendalian					
			</td>
		</tr>
		<%
	//pertama kali, hanya boleh kegiatan manual
	if((team_spmi||editor) && man_sdh_aktif) {	
	//if(editor) {
		%>
		<tr>
			<td style="text-align:center;font-size:1.3em;font-weight:bold" align="center">
			
				<form action="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/form_manual_pengendalian/form_survey_pengendalian.jsp">
					...
					TAMBAH KEGIATAN PENGENDALIAN STANDAR		
				</form>
			</td>
		</tr>
		<%
		...
		<tr>	
			<td style="text-align:center;font-size:1em;font-weight:bold;color:red" align="center">
				Saat ini kegiatan belum bisa ditambah, karena manual pengendalian pelaksanaan standar belum ada yang aktif<br>
				Silahkan aktifkan manual dengan menekan tombol 'play' 
			</td>
		</tr>			
		<%
	}
...
%>
	...
	if(!Checker.isStringNullOrEmpty(tgl_tetap_man) && Checker.isStringNullOrEmpty(tgl_sta_man)) {
		...
		<section style="align:center;text-align:center;font-weight:bold;color:red">	
			...
			<input type="image" src="<%=Constants.getRootWeb() %>/images/add_ico.png" width="35" height="35" alt="Submit Form" />TAMBAH KEGIATAN PENGENDALIAN STANDAR
			<br>
		...
		<section style="align:center;text-align:center">	
			<p>
				<form action="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/form_manual_pengendalian/form_survey_pengendalian.jsp" style="font-weight:bold;color:#369">
					<input type="hidden" name="status_manual" value="<%=status_manual %>" />
					<input type="hidden" name="scope_std" value="<%=scope_std %>" />
					<input type="hidden" name="std_kdpst" value="<%=std_kdpst %>" />
					<input type="hidden" name="id_versi" value="<%=id_versi %>" />
...
<table align="center" style="width:100%">
	<tr>
		<td colspan="8" style="font-size:2em;text-align:center;font-weight:bold;color:#369">
			RIWAYAT PENGENDALIAN MANUAL & STANDAR
		</td>
	</tr>
...
%>
<form action="go.updRiwayatPengendalian" method="post">
	<input type="hidden" name="status_manual" value="<%=status_manual %>" />
	<input type="hidden" name="target_unit_used" value="<%=target_unit_used %>" />
	<input type="hidden" name="kdpst_nmpst_kmp" value="<%=kdpst_nmpst_kmp %>" />
	<input type="hidden" name="id_versi" value="<%=id_versi %>" />
...
%>				
<form action="go.updRiwayatHasilPengendalian" method="post">
	<input type="hidden" name="status_manual" value="<%=status_manual %>" />
	<input type="hidden" name="target_unit_used" value="<%=target_unit_used %>" />
	<input type="hidden" name="kdpst_nmpst_kmp" value="<%=kdpst_nmpst_kmp %>" />
	<input type="hidden" name="id_versi" value="<%=id_versi %>" />
