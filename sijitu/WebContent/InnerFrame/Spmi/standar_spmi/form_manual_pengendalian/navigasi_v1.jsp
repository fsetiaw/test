...
boolean next=false;
boolean prev = false;
Vector v_hist_eval=shk.getListRiwayatPengendalianUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std),offset,(Integer.parseInt(src_manual_limit)));
int tot_rec = 0;
if(v_hist_eval!=null) {
...
%>
		<td style="width:50%;text-align:left;padding:0 0 0 10px">
			<a href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_pengendalian/riwayat_kendali_v1.jsp?starting_no=<%=prev_sta_no %>&offset=<%=prev_offset %>&unit_used=<%=target_unit_used %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=riwayat&at_menu_dash=control" style="font-weight:bold;font-size:1.6em" target="_self">PREV <</a>
		</td>	
<%
}
if(next) {
%>
		<td style="width:50%;text-align:right;padding:0 10px 0 0">
			<a href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_pengendalian/riwayat_kendali_v1.jsp?starting_no=<%=prev_sta_no %>&offset=<%=prev_offset %>&unit_used=<%=target_unit_used %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=riwayat&at_menu_dash=control" style="font-weight:bold;font-size:1.5em" target="_self">> Next</a>
		</td>	
<%
}
%>
