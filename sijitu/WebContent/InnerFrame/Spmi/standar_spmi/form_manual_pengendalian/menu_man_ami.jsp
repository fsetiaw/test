...
<div id="header">
	...
	<ul>
	<li><a href="get.prepInfoMan_v1?id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_pengendalian_kendali_v1.jsp&darimana=ami">BACK <span><b style="color:#eee">&nbsp</b></span></a></li>
<%
if(at_menu_kendal.equalsIgnoreCase("riwayat")) {
%>	
                                            
	<li><a href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_pengendalian/riwayat_kendali_ami_v1.jsp?status_manual=<%=status_manual%>&starting_no=1&unit_used=<%=target_unit_used %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=riwayat&at_menu_dash=kendali" class="active">RIWAYAT<span>PENGENDALIAN</span></a></li>
<%
}
else {
%>	
	<li><a href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/fform_manual_pengendalian/riwayat_kendali_ami_v1.jsp?status_manual=<%=status_manual%>&starting_no=1&unit_used=<%=target_unit_used %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=riwayat&at_menu_dash=control">RIWAYAT<span>PENGENDALIAN</span></a></li>
<%	
}
