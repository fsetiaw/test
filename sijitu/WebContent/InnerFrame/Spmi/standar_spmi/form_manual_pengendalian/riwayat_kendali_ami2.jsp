...
v_hist_eval=shk.getListRiwayatPengendalianAmi(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi), Integer.parseInt(norut_man),offset,Integer.parseInt(src_manual_limit));
	...
	if(false) {	
		...
		<tr>
			<td style="text-align:center;font-size:1.3em;font-weight:bold" align="center">
				...
				<form action="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/form_manual_evaluasi/form_survey_evaluasi.jsp">
...
%>
	...
	if(false) {
		...
		<section style="align:center;text-align:center">	
			<p>
				<form action="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/form_manual_evaluasi/form_survey_evaluasi.jsp" style="font-weight:bold;color:#369">
					<input type="hidden" name="scope_std" value="<%=scope_std %>" />
					<input type="hidden" name="std_kdpst" value="<%=std_kdpst %>" />
					<input type="hidden" name="id_versi" value="<%=id_versi %>" />
					<input type="hidden" name="id_std_isi" value="<%=id_std_isi %>" />
...
%>
	...
	<table class="table1" id="tabel" style="width:100%">	
		<thead>
			...
			<th style="width:95%">Tanggal Evaluasi </th>
		</thead>
		<%	
		...
		while(li.hasNext()&& (no_yg_tampil <= max_yg_tampil)) {
			...
			//versi_id+"~"+std_isi_id+"~"+norut_man+"~"+tgl_sta+"~"+tgl_end+"~"+tkn_jab_rumus+"~"+tkn_jab_cek+"~"+tkn_jab_stuju+"~"+tkn_jab_tetap+"~"+tkn_jab_kendali+"~"+tkn_jab_lap+"~"+tujuan+"~"+lingkup+"~"+definisi+"~"+prosedur+"~"+kuali+"~"+doc+"~"+ref+"~"+id_eval+"~"+npm_eval+"~"+tgl_eval+"~"+waktu_eval+"~"+eval_timstamp+"~"+kondisi_eval+"~"+analisa_eval+"~"+rekomen_eval+"~"+tgl_next_eval+"~"+wktu_next_eval+"~"+target_val_eval+"~"+real_val_eval+"~"+id_kendal+"~"+rasionale_kendal+"~"+tindakan_kendal+"~"+tgl_kendal+"~"+waktu_kendal+"~"+npm_kendal+"~"+kendal_timstamp;
			st = new StringTokenizer(brs,"~");
			
			String versi_id=st.nextToken();
			String std_isi_id=st.nextToken();
			norut_man=st.nextToken();
			...
			String eval_timstamp = st.nextToken();
			String kondisi_eval = st.nextToken();
			String analisa_eval = st.nextToken();
			String rekomen_eval = st.nextToken();
			String tgl_next_eval = st.nextToken();
			String waktu_next_eval = st.nextToken();
			String target_val_eval = st.nextToken();
			String real_val_eval = st.nextToken();
...
%>
		<form action="go.updRiwayatKendaliAmi" style="font-weight:bold;color:#369">
			<input type="hidden" name="status_manual" value=<%=status_manual %> />
			...
			<input type="hidden" name="id_versi" value="<%=id_versi %>"/>
			<input type="hidden" name="id_std_isi" value="<%=id_std_isi %>"/>
			<input type="hidden" name="std_isi" value="<%=id_std %>"/>
			<input type="hidden" name="norut_man" value="<%=norut_man %>"/>
			<input type="hidden" name="std_kdpst" value="<%=std_kdpst %>"/>
			<input type="hidden" name="scope_std" value="<%=scope_std %>"/>
			<input type="hidden" name="target_unit_used" value="<%=target_unit_used %>"/>
...
%>
		<tbody>
			<tr>
				<td rowspan="2" style="text-align:center;background:#92B9D2;color:white;font-size:1.25em;font-weight:bold"><%=counter++ %></td>
				<td style="text-align:center;background:#92B9D2;color:white;font-size:1.25em;font-weight:bold"><%=Converter.convertFormatTanggalKeFormatDeskriptif(tgl_eval) %> [<%=waktu_eval %>]</td>
			</tr>
			<tr>
				<td>
					<table class="StateTable" rules="all" cellpadding="0" cellspacing="0" >
						<thead>
							<tr class="statetablerow">							
								<th colspan="2" style="font-size:1.0em;text-align:left;font-weight:bold;padding:0 5px 0 10px">
									SURVEYOR
								</th>
								<td colspan="4" style="text-align:center"><%=Checker.getNmmhs(npm_eval) %></td>
							</tr>
							<tr class="statetablerow">
								<th colspan="2" style="font-size:1.0em;text-align:left;font-weight:bold;padding:0 5px 0 10px">
									JADWAL SURVEY BERIKUTNYA
								</th>
								<td colspan="4" style="text-align:center"><%=Converter.convertFormatTanggalKeFormatDeskriptif(tgl_next_eval) %> [<%=waktu_next_eval %>]</td>
							</tr>
<%
			if(man_sdh_aktif) {	
				if(Checker.isStringNullOrEmpty(id_kendal)) {
%>
							<tr class="statetablerow">
								<th colspan="6" style="border: 1px solid #fff;background:#fff;color:#cc0000;font-size:0.8em;text-align:center;font-weight:italic;padding:1px 1px">
<%
					if(!Checker.isStringNullOrEmpty(target_id_eval) && target_id_eval.equalsIgnoreCase(id_eval) && v_err!=null && v_err.size()>0) {
														
						ListIterator litmp = v_err.listIterator();
%>
									<div style="text-align:center;font-size:1.7em;color:red;font-weight:bold">
<%
						while(litmp.hasNext()) {
							String brs1 = (String)litmp.next();
							out.print("* "+brs1+"<br>");
						}
%>
									</div>
...
%>									
									Belum Dilakukan Tindakan Pengendalian
<%
					}
%>									
								</th>
...
%>							
						...
						<tbody>	
							<tr>
								<td rowspan="2" colspan="2" style="vertical-align:middle;width:40%;text-align:center;background:#369;color:white;padding:0 0 0 0;font-size:1.5em;font-weight:bold">
									PARAMETER YG DIEVALUASI
								</td>
								<td rowspan="2" colspan="2" style="vertical-align:middle;width:40%;text-align:center;background:#369;color:white;padding:0 0 0 0;font-size:1.5em;font-weight:bold">
									INDIKATOR CAPAIAN
								</td>
								<td colspan="1" style="width:10%;text-align:center;background:#369;color:white;font-size:1.25em;font-weight:bold">
									BESARAN TARGET
								</td>
								<td colspan="1" style="width:10%;text-align:center;background:#369;color:white;font-size:1.25em;font-weight:bold">
									BESARAN RIL
							...
							<tr>
								<td colspan="2" style="text-align:center;font-size:1.15em;background:#369;color:white;font-weight:bold"><%=target_unit_used %></td>
							</tr>
							<tr>
								<td colspan="2" style="vertical-align:middle;padding:0 5px 0 10px;font-size:1.25em;"><%=param %></td>
								<td colspan="2" style="vertical-align:middle;padding:0 5px 0 10px;font-size:1.25em;"><%=indikator %></td>
								<td style="vertical-align:middle;text-align:center;font-size:1.15em;font-weight:bold"><%=target_val_eval %></td>
								<td style="vertical-align:middle;text-align:center;font-size:1.15em;font-weight:bold"><%=real_val_eval %></td>
							</tr>
							
							<tr>
								<td colspan="2" style="vertical-align:middle;background:#369;color:white;font-size:1.0em;text-align:left;font-weight:bold;padding:0 5px 0 10px">
									SITUASI DAN KONDISI LAPANGAN
								</td>
								<td colspan="4" style="text-align:left;padding:0 5px 0 10px"><%=kondisi_eval %></td>
							...
							<tr>
								...
								<td colspan="4" style="text-align:left;padding:0 5px 0 10px"><%=analisa_eval %></td>
							</tr>	
							<tr>
								<td colspan="2" style="vertical-align:middle;background:#369;color:white;font-size:1.0em;text-align:left;font-weight:bold;padding:0 5px 0 10px">
									REKOMENDASI
								</td>
								<td colspan="4" style="text-align:left;padding:0 5px 0 10px"><%=rekomen_eval %></td>
...
%>
						<tbody>	
							<tr>
								<td colspan="6" style="border: 1px solid #ffcccc;vertical-align:middle;width:40%;text-align:center;background:#ffcccc;color:#369;padding:0 0 0 0;font-size:1.5em;font-weight:bold">
									FORM PENGENDALIAN
								</td>
							</tr>
							<tr>
								<td colspan="6" style="border: 1px solid #ff4d4d;vertical-align:middle;width:40%;text-align:left;background:#ff4d4d;color:white;padding:0 5px 0 10px;font-size:1.2em;font-weight:bold">
									MANUAL PENGENDALIAN PELAKSANAAN
								</td>
							</tr>
							<tr>
								<td colspan="6" style="border: 1px solid #ffe6e6;vertical-align:middle;width:40%;text-align:left;background:#ffe6e6;color:#369;padding:0 5px 0 10px;font-size:1em;font-weight:bold">
									<%=Checker.pnn(prosedur)%>
								</td>
							</tr>
							<tr>
								<td colspan="6" style="border: 1px solid #ff4d4d;vertical-align:middle;width:40%;text-align:left;background:#ff4d4d;color:white;padding:0 5px 0 10px;font-size:1.2em;font-weight:bold">
									RASIONALE PENGAMBILAN TINDAKAN PENGENDALIAN
								</td>
							</tr>
							<tr>
								<td colspan="6" style="border: 1px solid #fff;vertical-align:middle;width:40%;text-align:left;background:#fff;color:white;padding:0 0 0 0;font-size:1em;font-weight:bold">
<%
					if(!Checker.isStringNullOrEmpty(target_id_eval) && target_id_eval.equalsIgnoreCase(id_eval) && !Checker.isStringNullOrEmpty(target_rasionale_kendal)) {
					%>								
...
%>
							...
							<tr>
								<td colspan="4" style="border: 1px solid #ff4d4d;vertical-align:middle;width:40%;text-align:left;background:#ff4d4d;color:white;padding:0 5px 0 10px;font-size:1.2em;font-weight:bold">
									APAKAH TELAH TERJADI PENYIMPANGAN?
								</td>
								<td colspan="1" style="border: 1px solid #ff4d4d;vertical-align:middle;width:40%;text-align:left;background:#ff4d4d;color:white;padding:0 5px 0 10px;font-size:1.2em;font-weight:bold">
									<input type="radio" name="tilang" value="true"> YA
								</td>
								<td colspan="1" style="border: 1px solid #ff4d4d;vertical-align:middle;width:40%;text-align:left;background:#ff4d4d;color:white;padding:0 10 0 0px;font-size:1.2em;font-weight:bold">
							...
							<tr>
								...
								<tr>
									<td colspan="3" style="border: 1px solid #ff4d4d;vertical-align:middle;width:40%;text-align:left;background:#ff4d4d;color:white;padding:5px 5px 5px 10px;font-size:1.2em;font-weight:bold">
			<%
			...
			%>				
									...
									<td colspan="3" style="border: 1px solid #ff4d4d;vertical-align:middle;width:40%;text-align:left;background:#ff4d4d;color:white;padding:5px 5px;font-size:1.2em;font-weight:bold">
			<%
			...
			%>					
							...
							<tr>
								<td colspan="6" style="border: 1px solid #ffe6e6;background:#ffe6e6;padding:5px 0">
									<section class="gradient">
     									<div style="text-align:center">
           									<button class="button1" style="padding: 5px 50px;font-size: 20px;">Insert Tindakan Pengendalian</button>
     									</div>
									</section>
								</td>
							</tr>
...
<%				
				...
				//tampilkan hasil pengendalian
%>
						<tbody>	
							<tr>
								<td colspan="6" style="border: 1px solid #fff;vertical-align:middle;width:40%;text-align:center;background:#fff;color:#369;padding:0 0 0 0;font-size:1.5em;font-weight:bold">
									HASIL PENGENDALIAN
								</td>
							</tr>
							<tr>
								...
								<td colspan="4" style="text-align:center;padding:0 5px 0 10px"><%=Checker.getNmmhs(npm_kendal) %></td>
							</tr>
							<tr>
								<th colspan="2" style="vertical-align:middle;background:#369;color:white;font-size:1.0em;text-align:left;font-weight:bold;padding:0 5px 0 10px">
									TANGGAL PENGENDALIAN
								</th>
								<td colspan="4" style="text-align:center"><%=Converter.convertFormatTanggalKeFormatDeskriptif(tgl_kendal) %> [<%=waktu_kendal %>]</td>
							</tr>
							<tr>
<%
					...
					%>							
								...
								<td colspan="4" style="font-weight:bold;color:red;text-align:left;padding:0 5px 0 10px">TELAH DITEMUKAN PENYIMPANGAN</td>
<%					
					}
					else {
%>							
								...
								<td colspan="4" style="font-weight:bold;text-align:left;padding:0 5px 0 10px">TIDAK DITEMUKAN PENYIMPANGAN</td>
<%
					}
%>								
							</tr>
							<tr>
								...
								<td colspan="4" style="text-align:center;text-align:left;padding:0 5px 0 10px"><%=tindakan_kendal%></td>
							</tr>
							<tr>
								<th colspan="2" style="vertical-align:middle;background:#369;color:white;font-size:1.0em;text-align:left;font-weight:bold;padding:0 5px 0 10px">
									RASIONALE PENGAMBILAN TINDAKAN
								</th>
								<td colspan="4" style="text-align:center;text-align:left;padding:0 5px 0 10px"><%=rasionale_kendal%></td>
							</tr>
						</tbody>	
<%				
				}
...
%>
							<tr>
								<td colspan="6" style="background:#fff">
									Belum Ada Manual Pengendalian Yg Aktif
								</td>
							</tr>
<%				
			}
