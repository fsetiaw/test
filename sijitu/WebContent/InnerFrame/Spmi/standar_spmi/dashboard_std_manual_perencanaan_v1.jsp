...
//System.out.println("id_tipe_std00="+id_tipe_std);
String id_master_std = request.getParameter("id_master_std");
//System.out.println("id_master_std00="+id_master_std);
String id_std = request.getParameter("id_std");
//System.out.println("id_std00="+id_std);
String at_menu_dash = request.getParameter("at_menu_dash");
//System.out.println("at_menu_dash00="+at_menu_dash);
...
<div id="header">
<ul>
	<li><a href="#form" onclick="document.getElementById('chart').style.display='block';document.links[6].className = 'active';document.getElementById('form1').style.display='none';document.links[7].className = '';document.getElementById('upload').style.display='none';document.links[8].className = '';" >MANUAL PENETAPAN<br>STANDAR</a></li>
	<li><a href="#form" onclick="document.getElementById('chart').style.display='none';document.links[6].className = '';document.getElementById('form1').style.display='block';document.links[7].className = 'active';document.getElementById('upload').style.display='none';document.links[8].className = '';" >KEGIATAN MANUAL<br>PENETAPAN STANDAR</a></li>
...
Vector v_kendal= sm.searchListManualPerencanaanUmum(Integer.parseInt(id_std));
...
%>
	<thead>	
		...
		<tr>
			...
			PROSEDUR DAN LANGKAH PENETAPAN STANDAR
			</th>
...
%>	
		<tr>
			<td colspan="3" style="text-align:center;padding:0 0;font-size:0.9em">
				Belum Ada Manual Penetapan Standar				
			</td>
		</tr>
<%
}
...
<%
			...
			%>	
		<tr onclick="javascript:alert('Manual harus diaktifkan terlebih dahulu untuk melihat / manambah riwayat kegiatan penetapan');">
	<%			
		}
%>
...
<%		
		...
		if(false) {	
%>
			<form action="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual/form_insert_manual_perencanaan_v1.jsp" method="post">
				...
				<input type="hidden" name="fwdto" value="dashboard_std_manual_perencanaan_v1.jsp"/>
...
%>									
			...
			<td onclick="window.location.href='<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_perencanaan/riwayat_perencanaan_v1.jsp?status_manual=<%=status_manual%>&starting_no=1&unit_used=<%=unit_used %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=riwayat&at_menu_dash=plan'" style="text-align:left;padding:0 2px;font-size:0.9em;padding:0 0 0 5px">
				<%=prosedur %>						
			</td>
			<td style="text-align:center;padding:0 2px;font-size:0.9em">
				...
				if(status_manual.equalsIgnoreCase("AKTIF")) {
					%>
				<a class="img2" href="set.aktivasiMan_v1?status=aktif&tipe_manual=perencanaan&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_perencanaan_v1.jsp" style="text-decoration: none;" onclick="if (confirm('Apakah anda yakin akan memberhentikan penggunaan manual ini ?!? \nNote:\n1. Anda tidak dapat mengaktifkan kembali manual yg sudah diberhentikan.\n2. Pemberhentian manual dilakukan bila manual digantikan dengan revisi yg baru')) return true; ">
					...
					%>
				<a class="img2" href="set.aktivasiMan_v1?status=nonaktif&tipe_manual=perencanaan&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_perencanaan_v1.jsp" style="text-decoration: none;" onclick="if (confirm('Apakah anda yakin akan mengaktifkan penggunaan manual ini ?!? \nSyarat Aktivasi:\n1. Draf manual telah melalui kegiatan perumusan, pemeriksaan, persetujuan dan penetapan.\n2. Pastikan draf manual telah diisi dengan benar, karena manual yg telah diaktifkan tidak dapat diedit')) return true;return false; ">
...
%>	
		<tr>
			...
			<a class="img" href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual/form_insert_manual_perencanaan_v1.jsp?mode=insert&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_perencanaan_v1.jsp" style="vertical-align:middle;valign:middle">
...
<%	
	...
	if(li.hasNext()) {
		...
		String no_dok_spmi = ts.getPenomoranDokMutuUmum(id_std, "MANUAL", "PERENCANAAN");
%>	
	<br>&nbsp<br>
<%
	...
	%>			
				...
				/*
					%>
				<a href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual/scope_std_to_copy.jsp?tipe_manual=perencanaan&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_perencanaan_v1.jsp">
				...
				*/
				...
				<a href="#" onclick="if(confirm('Copy/Salin manual versi <%=id_versi%> ini untuk seluruh standar?\nCatatan:\nBila standar sudah memiliki manual versi <%=id_versi%> maka akan digantikan dengan salinan ini.'))(function(){
					...
					location.href='set.copyManForAll?status=aktif&tipe_manual=perencanaan&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_perencanaan_v1.jsp'
					})();return false;"
				>
					<img border="0" alt="COPY" src="<%=Constants.getRootWeb() %>/images/copy.png" width="70" height="30">
				</a>
				...
				if(status_manual.equalsIgnoreCase("AKTIF")) {
				%>
				<a class="img2" href="set.aktivasiMan_v1?status=aktif&tipe_manual=perencanaan&id_versi=<%=id_versi %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_perencanaan_v1.jsp" style="text-decoration: none;" onclick="if (confirm('Apakah anda yakin akan memberhentikan penggunaan manual ini ?!? \nNote:\n1. Anda tidak dapat mengaktifkan kembali manual yg sudah diberhentikan.\n2. Pemberhentian manual dilakukan bila manual digantikan dengan revisi yg baru.\n3. Manual pelaksanaan, evaluasi, pengendalian, dan peningkatan otomatis ikut di non-aktifkan')) return true; ">
					<img border="0" alt="STOP" src="<%=Constants.getRootWeb() %>/images/stop_red.png" width="30" height="30">
				</a>	
					<%	
				}
	...
	<table class="table0" id="table0" style="width:100%">
		<thead>	
			...
			<tr>
				<th style="text-align:left;vertical-align:middle;width:100%;font-size:1.5em;font-weight:bold;padding:0 0 0 5px">
					2. TUJUAN DAN MAKSUD DOKUMEN MANUAL PENETAPAN STANDAR
				</th>
			</tr>
			<tr>	
			...
			<tr>
				<th style="text-align:left;vertical-align:middle;width:100%;font-size:1.5em;font-weight:bold;padding:0 0 0 5px">
					4. DEFINISI ISTILAH TERKAIT MANUAL PENETAPAN STANDAR
				</th>
			</tr>
			<tr>	
				<td style="background:white;text-align:left;vertical-align:middle;width:100%;font-size:0.9em;font-weight:bold;padding:5px 5px 5px 20px">
			...
			<tr>
				<th style="text-align:left;vertical-align:middle;width:100%;font-size:1.5em;font-weight:bold;padding:0 0 0 5px">
					5. PROSEDUR DAN LANGKAH PENETAPAN STANDAR 
				</th>
			</tr>
			<tr>	
				<td style="background:white;text-align:left;vertical-align:middle;width:100%;font-size:0.9em;font-weight:bold;padding:5px 5px 5px 20px">
...
%>						
	...
	<div id="upload">
		<%
	session.setAttribute("ppepp_asal", "penetapan");
	...
	%>
		<iframe id="glue"  onload="myFunction()" src="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/dashArsipPerencanaan.jsp?kdpst=<%=kdpst %>&id_tipe_std=<%=id_tipe_std %>&id_master_std=<%=id_master_std %>&id_std=<%=id_std %>&id_versi=<%=versi_standar %>" seamless="seamless" width="100%" height="1000"  name="inner_iframe"></iframe>
	</div>
		<!-- Column 1 start -->
	</div>
</div>	
