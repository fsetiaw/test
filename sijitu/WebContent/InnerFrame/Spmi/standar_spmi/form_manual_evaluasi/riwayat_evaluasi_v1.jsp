...
// ini lumayan yakin kaloo ini nggga dipake
session.removeAttribute("current_id_ami");
session.removeAttribute("current_kode_activity");
session.removeAttribute("current_tgl_plan");
session.removeAttribute("current_ketua_tim");
session.removeAttribute("current_anggota_tim");
session.removeAttribute("current_id_cakupan_std");
session.removeAttribute("current_tgl_ril");
session.removeAttribute("current_ket_cakupan_std");
session.removeAttribute("current_tgl_ril_done");
session.removeAttribute("current_btn");
//----END kegiatan  param audit mutu internal----------------

beans.login.InitSessionUsr validUsr = (beans.login.InitSessionUsr)session.getAttribute("validUsr");
ToolSpmi ts = new ToolSpmi();
//System.out.println("masuk sasaran evaluasi");
...
String kegiatan = "";
String ppepp = "evaluasi";

String tgl_sta_man = ""+ts.getTglStartManualUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std), "evaluasi");
...
<body>
<jsp:include page="menu_man_eval_v1.jsp" />
...
v_hist_plan=shk.getListRiwayatEvaluasiUmum(Integer.parseInt(id_versi), Integer.parseInt(id_std),offset,Integer.parseInt(src_manual_limit)-1);
...
%>	
		...
		<tr>
			<td colspan="7" style="text-align:center;padding:0 0;font-size:2.0em;font-weight:bold">
				Belum Ada Riwayat Kegiatan Evaluasi	Pelaksanaan Standar				
			</td>
		</tr>
		<%
	...
	//if(editor) {
		...
		if(man_sdh_aktif) {
		%>	
				<form action="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/form_manual_evaluasi/form_survey_evaluasi_v1.jsp">
		<%
		}
		%>		
		...
		%>
					...
					TAMBAH KEGIATAN EVALUASI PELAKSANAAN STANDAR
		<%
		}
		else {
			%>
		...
		<tr>
			<td style="text-align:center;font-size:1.3em;font-weight:bold" align="center">
				<input type="image" src="<%=Constants.getRootWeb() %>/images/add_ico.png" width="35" height="35" alt="Submit Form" onclick="this.form.submit()" />
				TAMBAH KEGIATAN EVALUASI PELAKSANAAN STANDAR<br>[Anda tidak memiliki akses untuk menambah kegiatan]
			</td>
		</tr>			
		<%
		}
...
%>
	...
	if(!Checker.isStringNullOrEmpty(tgl_tetap_man) && Checker.isStringNullOrEmpty(tgl_sta_man)&&!man_expired) {
		...
		<section style="align:center;text-align:center;font-weight:bold;color:red">	
			...
			<input type="image" src="<%=Constants.getRootWeb() %>/images/add_ico.png" width="35" height="35" alt="Submit Form" />TAMBAH KEGIATAN EVALUASI PELAKSANAAN STANDAR
			<br>
		...
		<section style="align:center;text-align:center">	
			<p>
				<form action="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/form_manual_evaluasi/form_survey_evaluasi_v1.jsp" style="font-weight:bold;color:#369">
					<input type="hidden" name="status_manual" value="<%=status_manual %>" />
					<input type="hidden" name="scope_std" value="<%=scope_std %>" />
					<input type="hidden" name="std_kdpst" value="<%=std_kdpst %>" />
					<input type="hidden" name="id_versi" value="<%=id_versi %>" />
...
<table align="center" style="width:99%">
	<tr>
		<td colspan="8" style="font-size:2em;text-align:center;font-weight:bold;color:#369">
			RIWAYAT EVALUASI PELAKSANAAN STANDAR
		</td>
	</tr>
...
%>
<form action="go.updRiwayatEvaluasi_v1" method="post">
	<input type="hidden" name="status_manual" value="<%=status_manual %>" />
	<input type="hidden" name="target_unit_used" value="<%=target_unit_used %>" />
	<input type="hidden" name="kdpst_nmpst_kmp" value="<%=kdpst_nmpst_kmp %>" />
	<input type="hidden" name="id_versi" value="<%=id_versi %>" />
			...
			<tr>
				...
				*/
				if(jenis_kegiatan_hist!=null && jenis_kegiatan_hist.equalsIgnoreCase("kendali_std")) {
					%>
						<option value="kendali_std" selected="selected">Evaluasi Pelaksanaan Standar</option>
					<%	
				}
				else {
				%>
						<option value="kendali_std">Evaluasi Pelaksanaan Standar</option>
				<%
				}
						%>
					</select>	
...
%>				
<form action="go.updRiwayatHasilEvaluasi_v1" method="post">
	<input type="hidden" name="status_manual" value="<%=status_manual %>" />
	<input type="hidden" name="target_unit_used" value="<%=target_unit_used %>" />
	<input type="hidden" name="kdpst_nmpst_kmp" value="<%=kdpst_nmpst_kmp %>" />
	<input type="hidden" name="id_versi" value="<%=id_versi %>" />
