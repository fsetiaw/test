...
String tgl_sta_man = ""+ts.getTglStartManual(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi), Integer.parseInt(norut_man), "evaluasi");
...
<body>
<jsp:include page="menu_man_eval.jsp" />
...
v_hist_plan=shk.getListRiwayatEvaluasi(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi), Integer.parseInt(norut_man),offset,Integer.parseInt(src_manual_limit)-1);
...
%>	
		...
		<tr>
			<td colspan="7" style="text-align:center;padding:0 0;font-size:2.0em;font-weight:bold">
				Belum Ada Riwayat Kegiatan Evaluasi					
			</td>
		</tr>
		<%
	...
	//if(editor) {
		...
		<tr>
			<td style="text-align:center;font-size:1.3em;font-weight:bold" align="center">
				...
				<form action="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/form_manual_evaluasi/form_survey_evaluasi.jsp">
					...
					TAMBAH KEGIATAN EVALUASI STANDAR		
				</form>
			</td>
		</tr>
		<%
		...
		<tr>	
			<td style="text-align:center;font-size:1em;font-weight:bold;color:red" align="center">
				Saat ini kegiatan belum bisa ditambah, karena manual evaluasi pelaksanaan standar belum ada yang aktif<br>
...
%>
	...
	if(!Checker.isStringNullOrEmpty(tgl_tetap_man) && Checker.isStringNullOrEmpty(tgl_sta_man)) {
		...
		<section style="align:center;text-align:center;font-weight:bold;color:red">	
			...
			<input type="image" src="<%=Constants.getRootWeb() %>/images/add_ico.png" width="35" height="35" alt="Submit Form" />TAMBAH KEGIATAN EVALUASI STANDAR
			<br>
...
<table align="center" style="width:100%">
	<tr>
		<td colspan="8" style="font-size:2em;text-align:center;font-weight:bold;color:#369">
			RIWAYAT EVALUASI MANUAL & STANDAR
		</td>
	</tr>
...
%>
<form action="go.updRiwayatEvaluasi" method="post">
	<input type="hidden" name="status_manual" value="<%=status_manual %>" />
	<input type="hidden" name="target_unit_used" value="<%=target_unit_used %>" />
	<input type="hidden" name="kdpst_nmpst_kmp" value="<%=kdpst_nmpst_kmp %>" />
	<input type="hidden" name="id_versi" value="<%=id_versi %>" />
...
%>				
<form action="go.updRiwayatHasilEvaluasi" method="post">
	<input type="hidden" name="status_manual" value="<%=status_manual %>" />
	<input type="hidden" name="target_unit_used" value="<%=target_unit_used %>" />
	<input type="hidden" name="kdpst_nmpst_kmp" value="<%=kdpst_nmpst_kmp %>" />
	<input type="hidden" name="id_versi" value="<%=id_versi %>" />
