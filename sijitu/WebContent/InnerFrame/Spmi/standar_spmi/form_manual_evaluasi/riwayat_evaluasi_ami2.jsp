...
v_hist_eval=shk.getListRiwayatEvaluasiAmi(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi), Integer.parseInt(norut_man),offset,Integer.parseInt(src_manual_limit));
	...
	//if(((am_i_surveyor||team_spmi) && sdh_aktif)||editor) {
		...
		<tr>
			<td style="text-align:center;font-size:1.3em;font-weight:bold" align="center">
				...
				<form action="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/form_manual_evaluasi/form_survey_evaluasi_ami.jsp">
...
%>
	...
	if((am_i_surveyor && sdh_aktif)||editor) {
		...
		<section style="align:center;text-align:center">	
			<p>
				<form action="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/form_manual_evaluasi/form_survey_evaluasi_ami.jsp" style="font-weight:bold;color:#369">
					<input type="hidden" name="scope_std" value="<%=scope_std %>" />
					<input type="hidden" name="std_kdpst" value="<%=std_kdpst %>" />
					<input type="hidden" name="id_versi" value="<%=id_versi %>" />
					<input type="hidden" name="id_std_isi" value="<%=id_std_isi %>" />
					...
					<input type="hidden" name="am_i_controller" value="<%=am_i_controller %>" />
					<input type="hidden" name="am_i_surveyor" value="<%=am_i_surveyor %>" />
					<input type="image" src="<%=Constants.getRootWeb() %>/images/add_ico.png" width="35" height="35" alt="Submit Form" onclick="this.form.submit()" />TAMBAH KEGIATAN AMI
				</form>
			</p>	
		</section>
...
<%	
	...
	if(li.hasNext()) {
		...
		while(li.hasNext()&& (no_yg_tampil <= max_yg_tampil)) {
			no_yg_tampil++;
			String brs = (String)li.next();
			//id_eval+"~"+versi_id+"~"+std_isi_id+"~"+norut_man+"~"+npm_eval+"~"+tgl_eval+"~"+waktu_eval+"~"+eval_timestamp+"~"+sikon+"~"+analisa+"~"+rekomendasi+"~"+tgl_next_eval+"~"+waktu_next_eval+"~"+tgl_sta+"~"+tgl_end+"~"+tkn_jab_rumus+"~"+tkn_jab_cek+"~"+tkn_jab_stuju+"~"+tkn_jab_tetap+"~"+tkn_jab_kendali+"~"+tujuan+"~"+lingkup+"~"+definisi+"~"+prosedur+"~"+kuali+"~"+doc+"~"+ref
...
%>
	...
	<table class="StateTable" rules="all" cellpadding="0" cellspacing="0" style="border:none;width:100%;background:<%=Constant.lightColorBlu()%>;">
		<thead>
			...
			<tr>
				<td>
					<table class="StateTable" rules="all" cellpadding="0" cellspacing="0" >
						<thead>
							<tr class="statetablerow">
								<th colspan="2" style="width:40%;font-size:1.0em;text-align:left;font-weight:bold;padding:0 5px 0 10px">
									SURVEYOR
								</th>
								<td colspan="4" style="width:60%;text-align:center"><%=Checker.getNmmhs(npm_eval) %></td>
							</tr>
							<tr class="statetablerow">
						...
						<tbody>	
							...
							<tr>
								...
								<td style="vertical-align:middle;text-align:center;font-size:1.15em;font-weight:bold"><%=target_val %></td>
								<td style="vertical-align:middle;text-align:center;font-size:1.15em;font-weight:bold"><%=ril_val %></td>
							</tr>
							...
							<tr>
								...
								<td colspan="4" style="text-align:left;padding:0 5px 0 10px"><%=sikon %></td>
							...
							<tr>
								...
								<td colspan="4" style="text-align:left;padding:0 5px 0 10px"><%=analisa %></td>
							</tr>	
							<tr>
								<td colspan="2" style="vertical-align:middle;background:#369;color:white;font-size:1.0em;text-align:left;font-weight:bold;padding:0 5px 0 10px">
									REKOMENDASI
								</td>
								<td colspan="4" style="text-align:left;padding:0 5px 0 10px"><%=rekomendasi %></td>
