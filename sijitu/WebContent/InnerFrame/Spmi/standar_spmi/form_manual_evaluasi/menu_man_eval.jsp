...
<div id="header">
	...
	<ul>
	<li><a href="get.prepInfoStd?id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_pengendalian_eval.jsp&darimana=riwayat">BACK <span><b style="color:#eee">&nbsp</b></span></a></li>
<%
if(at_menu_kendal.equalsIgnoreCase("riwayat")) {
%>	
                                            
	<li><a href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_evaluasi/riwayat_evaluasi.jsp?status_manual=<%=status_manual %>&starting_no=1&unit_used=<%=target_unit_used %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&norut_man=<%=norut_man %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=riwayat&at_menu_dash=<%=at_menu_dash %>" class="active">RIWAYAT EVALUASI<span>PELAKSANAAN</span></a></li>
<%
}
else {
%>	
	<li><a href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_evaluasi/riwayat_evaluasi.jsp?status_manual=<%=status_manual %>&starting_no=1&unit_used=<%=target_unit_used %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&norut_man=<%=norut_man %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=riwayat&at_menu_dash=<%=at_menu_dash %>">RIWAYAT EVALUASI<span>PELAKSANAAN</span></a></li>
<%	
}
