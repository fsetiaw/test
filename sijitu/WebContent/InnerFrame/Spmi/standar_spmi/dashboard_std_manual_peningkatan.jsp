...
<div id="header">
<ul>
	<li><a href="#form" onclick="document.getElementById('chart').style.display='block';document.links[6].className = 'active';document.links[7].className = '';document.links[8].className = '';document.getElementById('form').style.display='none';document.getElementById('upload').style.display='none';" >MANUAL PENINGKATAN<br>STANDAR</a></li>
	<li><a href="#form" onclick="document.getElementById('chart').style.display='none';document.getElementById('form').style.display='block';document.links[7].className = 'active';document.links[6].className = '';document.links[8].className = '';document.getElementById('upload').style.display='none';" >AKTIFASI / EDIT / TAMBAH KEGIATAN<br>MANUAL PENINGKATAN STANDAR</a></li>
	<li><a href="#form" onclick="document.getElementById('glue').contentWindow.location.reload();document.getElementById('chart').style.display='none';document.getElementById('form').style.display='none';document.getElementById('upload').style.display='block';document.links[8].className = 'active';document.links[7].className = '';document.links[6].className = ''" >ARSIP FILE<br>DOKUMEN</a></li>
</ul>
...
%>	
		<tr>
			<td colspan="3" style="text-align:center;padding:0 0;font-size:0.9em">
				Belum Ada Manual Peningkatan Standar <br> Silahkan Input Manual Peningkatan Standar yang Sudah Dirumuskan Pada Proses Perencanaan Standar					
			</td>
		</tr>
<%
	}
...
%>
			<form action="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual/form_insert_manual_peningkatan.jsp" method="post">
				...
				<input type="hidden" name="fwdto" value="dashboard_std_manual_peningkatan.jsp"/>
...
%>									
			...
			<td onclick="window.open('<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_peningkatan/riwayat_peningkatan.jsp?status_manual=<%=status_manual%>&starting_no=1&unit_used=<%=unit_used %>&norut_man=<%=norut_man %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&norut_man=<%=norut_man %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=riwayat&at_menu_dash=<%= at_menu_dash%>','popup','width=850,height=600')" style="text-align:left;padding:0 2px;font-size:0.9em">
				<%=prosedur %>						
			</td>
			<td style="text-align:center;padding:0 2px;font-size:0.9em">
				...
				if(status_manual.equalsIgnoreCase("AKTIF")) {
					%>
				<a class="img2" href="set.aktivasiMan?status=aktif&tipe_manual=peningkatan&norut_man=<%=norut_man %>&id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_peningkatan.jsp" style="text-decoration: none;" onclick="if (confirm('Apakah anda yakin akan memberhentikan penggunaan manual ini ?!? \nNote:\n1. Anda tidak dapat mengaktifkan kembali manual yg sudah diberhentikan.\n2. Pemberhentian manual dilakukan bila manual digantikan dengan revisi yg baru')) return true; ">
					...
					%>
				<a class="img2" href="set.aktivasiMan?status=nonaktif&tipe_manual=peningkatan&norut_man=<%=norut_man %>&id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_peningkatan.jsp" style="text-decoration: none;" onclick="if (confirm('Apakah anda yakin akan mengaktifkan penggunaan manual ini ?!? \nSyarat Aktivasi:\n1. Draf manual telah melalui kegiatan perumusan, pemeriksaan, persetujuan dan penetapan.\n2. Pastikan draf manual telah diisi dengan benar, karena manual yg telah diaktifkan tidak dapat diedit')) return true;return false; ">
...
if(editor && !ada_manual_yg_bisa_diedit && ada_penetapan_man) {
	%>	
		<tr>
			...
			<a class="img" href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual/form_insert_manual_peningkatan.jsp?id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_peningkatan.jsp" style="vertical-align:middle;valign:middle">
...
<%	
	...
	if(li.hasNext()) {
		...
		String no_dok_spmi = ts.getPenomoranDokMutu(id_std_isi, id_versi, "MANUAL", "PENINGKATAN");
%>	
	<br>&nbsp<br>
...
%>						
	...
	<div id="upload">
		<iframe id="glue"  onload="myFunction()" src="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/dashArsipPeningkatan.jsp?kdpst=<%=kdpst %>&id_std_isi=<%=id_std_isi %>&id_versi=<%=id_versi %>" seamless="seamless" width="100%" height="1000"  name="inner_iframe"></iframe>
	</div>
		<!-- Column 1 start -->
	</div>
</div>		
