...
<div id="main">
    ...
    while(li.hasNext()) {
    	...
    	//String jenis_mode = "view"; //!!!!!! ommit //
    	String jenis_mode = "start";
    %>
   				<a href="go.getListAllStd?mode=<%=jenis_mode %>&id_tipe_std=<%=id %>&id_master_std=<%=id_master %>&atMenu=&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_page=<%=at_page %>&max_data_per_pg=<%=max_data_per_pg %>&at_menu=nama_std" target="inner_iframe" class="tile bg-darkOrange fg-white" data-role="tile">
            		<div class="padding10">
            		<p class="no-margin text-shadow" style="font-size:1.2em"><span class="text-bold"><%=keter %></p>
					</div>
