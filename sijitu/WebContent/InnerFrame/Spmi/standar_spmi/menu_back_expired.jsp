...
if(false) {
	...
	while(li.hasNext()) {
		...
		if(!Checker.isStringNullOrEmpty(id_master) && id_master.equalsIgnoreCase(id)) {
			...
			<li><a href="<%=Constants.getRootWeb()+"/" %>${PageContext.ServletContext.ContextPath}/InnerFrame/Spmi/standar_spmi/home_standar_nasional.jsp?at_page=<%=at_page %>&max_data_per_pg=<%=max_data_per_pg %>&id=<%=id %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>">BACK<span><b>&nbsp</b></span></a></li>
				<%			
		}
