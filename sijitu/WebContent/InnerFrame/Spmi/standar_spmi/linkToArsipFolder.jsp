...
if(!Checker.isStringNullOrEmpty(id_versi)) {
	...
	String alamat = "";
	if(ppepp.equalsIgnoreCase("penetapan")) {
		alamat = ts.getRootSpmiFolder(id_versi,ppepp).trim()+"/"+man_or_std.trim()+"/"+kegiatan.trim()+"/"+target_folder.trim();
	}
	else {
		alamat = ts.getRootSpmiFolder(id_versi,ppepp).trim()+"/"+man_or_std.trim()+"/"+target_folder.trim();
	}
	while(alamat.contains("//")) {
		alamat = alamat.replace("//", "/");
	}
	li.add("Folder Utama`"+alamat.trim()+"`reid");
	//System.out.println("oke aja ="+alamat);
