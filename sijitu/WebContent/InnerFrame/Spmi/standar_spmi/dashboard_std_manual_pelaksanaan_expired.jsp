...
%>
			<form action="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual/form_insert_manual_pelaksanaan.jsp" method="post">
				...
				<input type="hidden" name="fwdto" value="dashboard_std_manual_pelaksanaan.jsp"/>
...
%>									
			...
			<td onclick="window.open('<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual_pelaksanaan/riwayat_pelaksanaan.jsp?status_manual=<%=status_manual%>&starting_no=1&unit_used=<%=unit_used %>&norut_man=<%=norut_man %>&scope_std=<%=scope_std%>&std_kdpst=<%=std_kdpst %>&id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&norut_man=<%=norut_man %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp %>&at_menu_kendal=riwayat&at_menu_dash=do','popup','width=850,height=600')" style="text-align:left;padding:0 2px;font-size:0.9em">
				<%=prosedur %>						
			</td>
			<td style="text-align:center;padding:0 2px;font-size:0.9em">
				...
				<a class="img2" href="set.aktivasiMan?status=aktif&tipe_manual=pelaksanaan&norut_man=<%=norut_man %>&id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_pelaksanaan.jsp" style="text-decoration: none;" onclick="if (confirm('Apakah anda yakin akan memberhentikan penggunaan manual no.<%=norut %> ?!? \nNote:\n1. Anda tidak dapat mengaktifkan kembali manual yg sudah diberhentikan.\n2. Pemberhentian manual dilakukan bila manual digantikan dengan revisi yg baru')) return true; ">
					...
					%>
				<a class="img2" href="set.aktivasiMan?status=nonaktif&tipe_manual=pelaksanaan&norut_man=<%=norut_man %>&id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_pelaksanaan.jsp" style="text-decoration: none;" onclick="if (confirm('Apakah anda yakin akan mengaktifkan penggunaan manual no.<%=norut %> ?!? \nSyarat Aktivasi:\n1. Tidak ada manual lain yang aktif.\n2. Telah melakukan kegiatan pelaksanaan yang menghasilkan penetapan manual')) return true;return false; ">
...
if(editor && !ada_manual_yg_bisa_diedit && ada_penetapan_man) {
	%>	
		<tr>
			...
			<a class="img" href="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/form_manual/form_insert_manual_pelaksanaan.jsp?id_versi=<%=id_versi %>&id_std_isi=<%=id_std_isi %>&id_std=<%=id_std %>&kdpst_nmpst_kmp=<%=kdpst_nmpst_kmp%>&at_menu_dash=<%= at_menu_dash%>&fwdto=dashboard_std_manual_pelaksanaan.jsp" style="vertical-align:middle;valign:middle">
...
<%	
	...
	if(li.hasNext()) {
		...
		String no_dok_spmi = ts.getPenomoranDokMutu(id_std_isi, id_versi, "MANUAL", "PELAKSANAAN");
%>	
	<br>&nbsp<br>
...
%>						
	...
	<div id="upload">
		<iframe id="glue"  onload="myFunction()" src="<%=Constants.getRootWeb() %>/InnerFrame/Spmi/standar_spmi/dashArsipPelaksanaan.jsp?kdpst=<%=kdpst %>&id_std_isi=<%=id_std_isi %>&id_versi=<%=id_versi %>" seamless="seamless" width="100%" height="1000"  name="inner_iframe"></iframe>
	</div>
		<!-- Column 1 start -->
	</div>
</div>		
