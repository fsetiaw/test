...
//System.out.println("id_std_isi1="+id_std_isi);
int norut_terakhir = 0;
if(!Checker.isStringNullOrEmpty(id_std_isi)&&!Checker.isStringNullOrEmpty(id_versi)) {
	norut_terakhir = ts.getNorutTerakhirManPeningkatanStd(Integer.parseInt(id_std_isi), Integer.parseInt(id_versi));
}
...
%>		
	<center>
	<form action="go.updManualPeningkatan" method="post">
		...
		<table class="table1" id="tabel" style="width:100%" >
			<thead>
				<tr>
					<th colspan="10" style="border-left: none;text-align:center;height:30px;vertical-align:middle;padding:0 5px;width:95%;border:none;font-size:2.0em;font-weight:bold">
					DRAFT / TARGET STANDAR UNTUK DIBUATKAN MANUAL
					</th>
				</tr>
			</thead>
			<tbody>
		...
		<br>
		<%
if(Checker.isStringNullOrEmpty(norut_man)||(mode!=null&&mode.equalsIgnoreCase("insert"))) {
	//berarti add baru
	//System.out.println("add baru");
%>
		<input type="hidden" name="mode" value="insert" />
		<input type="hidden" name="norut_man" value="<%=norut_terakhir+1 %>" />
<%
}
else {
	//edit yg ada
	...
	String brs = sm.searchListManualPeningkatan(Integer.parseInt(id_versi), Integer.parseInt(id_std_isi), Integer.parseInt(norut_man));
	...
	String std_isi_id=st1.nextToken();
	norut_man=st1.nextToken();
			...
			<thead>
				<tr>
					<th colspan="2" style="font-size:2em;text-align:center;font-weight:bold;">
						FORM INPUT MANUAL PENINGKATAN STANDAR
					</th>
				</tr>
				<tr>
...
%>					
						<textarea name="tujuan" style="width:100%;height:100px;row:5;border:none">Tujuan dan maksud manual penetapan standar pengelolaan penelitian disusun untuk memberikan pedomman kepada pihak yag akan terlibat dalam proses peningkatan dan pemantauan pemenuhan ketercapaian isi standar [sebutkan nama standar]</textarea>
<%
}
else {
	%>					
...
%>					
						<textarea name="doc" placeholder="sebutkan dokumen-dokumen terkait" style="width:100%;height:100px;row:5;border:none"></textarea>
<%
}
else {
	%>						
						<textarea name="doc" style="width:100%;height:100px;row:5;border:none"><%=doc %></textarea>
<%	
}
%>							
					</td>
