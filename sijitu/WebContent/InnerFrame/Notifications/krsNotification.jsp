...
<div id="header">
	<ul>
		<li><a href="#" onclick="(function(){
		 ...
		 target="inner_iframe">BACK<span>&nbsp</span></a></li>
	</ul>
</div>
...
if(tknKrsNotifications!=null && !Checker.isStringNullOrEmpty(tknKrsNotifications)) {
   ...
   <tr> 
	...
	%>
	<form id="<%=npmSender %>" action="go.directToHistKrs" methode="post">
		<input type="hidden" name="kword" value="<%=npmSender %>"/>
	...
	<tr onclick="(function(){
           //scroll(0,0);
				parent.scrollTo(0,0);
           		...
           		x.style.display = 'block';
            	y.style.display = 'none';
            	document.getElementById('<%=npmSender %>').submit()
            	})()">
		<td style="color:#000;text-align:center;"  rowspan="1"><%=counter %></td>
		<td style="color:#000;text-align:center;"><%=npmSender %></td>
		<td style="color:#000;text-align:left;"><%=nmmSender %></td>
