...
//System.out.println("npm@innerMenu="+npm);
...
//System.out.println("v_nmmhs "+v_nmmhs);
	...
	boolean isObjTamu = false;
	
	if(ToolMhs.getObjName(Integer.parseInt(v_id_obj)).contains("GUEST")) {
		isObjTamu = true;
	}%>	
<script>
		...
		$(document).on("click", "#histKrs", function() {
        	$.ajax({
        	    ...
        	    success:function(data){
        	        ...
        	        <%if(ToolMhs.getSistemPerkuliahan(v_id_obj, v_kdpst).equalsIgnoreCase("CONTINUE")) {%>
...
<ul>
	...
	%>
		<li><a href="get.profile?id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs%>&obj_lvl=<%=v_obj_lvl%>&kdpst=<%=v_kdpst%>&cmd=dashboard" target="_self" class="active">HOME <span><b style="color:#eee">&nbsp</b></span></a></li>
	<%
		}
		else {
	%>
		<li><a href="get.profile?id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs%>&obj_lvl=<%=v_obj_lvl%>&kdpst=<%=v_kdpst%>&cmd=dashboard" target="_self">HOME <span><b style="color:#eee">&nbsp</b></span></a></li>
	<%
		}
		if(validUsr.isAllowTo("iciv")>0 && isObjTamu) {
			if(cmd.equalsIgnoreCase("jdMhs")) {
	%>
		<li><a href="go.ubahJadiMhs?id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs%>&obj_lvl=<%=v_obj_lvl%>&kdpst=<%=v_kdpst%>&cmd=jdMhs" target="_self" class="active">DAFTARKAN MENJADAI<span>MAHASISWA/CIVITAS</span></a></li>
	<%
		}
			else {
	%>
		<li><a href="go.ubahJadiMhs?id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs%>&obj_lvl=<%=v_obj_lvl%>&kdpst=<%=v_kdpst%>&cmd=jdMhs" target="_self" >DAFTARKAN MENJADAI<span>MAHASISWA/CIVITAS</span></a></li>
	<%
		}
		}
		...
		String obj_desc = ToolMhs.getObjDesc(Integer.parseInt(v_id_obj)) ;
		//System.out.println("obj_nick="+obj_desc);
		
		/*
		PINDAH KE DATA PROFILE
		if(obj_desc!=null && !obj_desc.contains("MHS") && !obj_desc.contains("mhs")){
	...
	<%
		...
		*/
		if(ada_akses_vop || ada_akses_insDataDosen ||ada_akses_viewDataDosen) {
			//ada_akses_insDataDosen - deprecated , diganti allowViewDataDosen (r,e,i,d)
			if(ada_akses_vop||ada_akses_viewDataDosen) {
		if(cmd.equalsIgnoreCase("viewProfile")) {
		/*
		UPDATED YG VERSI LAMA TANPA _v1
		*/
	%>
		<li><a href="get.profile_v1?id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs%>&obj_lvl=<%=v_obj_lvl%>&kdpst=<%=v_kdpst%>&cmd=profile&atMenu=pribadi" target="_self" class="active">DATA <span>PROFIL</span></a></li>
	<%
		}
		else {
	%>
		<li><a href="get.profile_v1?id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs%>&obj_lvl=<%=v_obj_lvl%>&kdpst=<%=v_kdpst%>&cmd=profile&atMenu=pribadi" target="_self" >DATA <span>PROFIL</span></a></li>
	<%
		}
			}
			else if(false) {
		...
		if(jsoaDsn!=null && jsoaDsn.length()>0 && !isObjTamu) {
	...
	%>
					...
					<%
							...
							if(!ToolMhs.getObjNickname(Integer.parseInt(v_id_obj)).contains("MHS") && validUsr.isUsrAllowTo("insDataDosen", v_npmhs, v_obj_lvl) && !isObjTamu) {
								if(cmd!=null && cmd.equalsIgnoreCase("dataDosen")) {
					%>
	...
	if(validUsr.isUsrAllowTo("vbak", v_npmhs, v_obj_lvl) && !isObjTamu) {	
	...
	if(validUsr.isUsrAllowTo("viewKrs", v_npmhs, v_obj_lvl) && !isObjTamu) {
		allowViewKrs = true;
		/*
		...
		*/
		...
		if(validUsr.getObjNickNameGivenObjId().contains("MHS") || validUsr.getObjNickNameGivenObjId().contains("mhs")) {
			%>
			<!--  li><a href="get.histKrs?id_obj=<v_id_obj%>&nmm=<v_nmmhs%>&npm=<v_npmhs %>&obj_lvl=<v_obj_lvl %>&kdpst=<v_kdpst %>&cmd=histKrs" target="_self" >KRS / KHS <span>& BAHAN AJAR</span></a></li -->
			<li><a href="#" id="histKrs" target="_self" >KRS / KHS <span>& BAHAN AJAR</span></a></li>
			<%	
		}
		else {
			%>
			<li><a href="#" id="histKrs" target="_self" >DATA<span>KRS/KHS</span></a></li>
			<%
		}	
		//}
		//else {
...
*/
	...
	if(validUsr.isUsrAllowTo("hasCetakMenu", v_npmhs, v_obj_lvl) && !keluar && !dropout && !isObjTamu) {	
	...
	if(validUsr.isUsrAllowTo("hasPengajuanMhsMenu", v_npmhs, v_obj_lvl) && !keluar && !dropout && !isObjTamu) {	
		...
		<li><a href="<%=Constants.getRootWeb()+"/" %>InnerFrame/indexPengajuanMhs.jsp?id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>&cmd=reqMhs" target="_self" class="active">PENGAJUAN<span><div style="color:#eee">&nbsp</div></span></a></li>
		<%
		}
		else {
		%>
		...
		<li><a href="<%=Constants.getRootWeb()+"/" %>InnerFrame/indexPengajuanMhs.jsp?id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>&cmd=reqMhs" target="_self" >PENGAJUAN<span><div style="color:#eee">&nbsp</div></span></a></li>
		<%
		}	
	}

	//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	//DEPRACATED DIGANTI FILE ARSIP
	//JANGAN PERNAH DIPAKE LAGI
	//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
	...
	if(validUsr.isUsrAllowTo("resetUrsPwd", v_npmhs, v_obj_lvl) && !keluar && !dropout) {	
		//requestKonversiMakul = true;
	...
	if(validUsr.isUsrAllowTo("cmd1", v_npmhs, v_obj_lvl)) {	
		//requestKonversiMakul = true;
		if(cmd.equalsIgnoreCase("pengaturan")) {
			%>
			<!--  li><a href="upAndDownloadFile.jsp?id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>&cmd=upDownFile" target="_self" class="active">DOWNLOAD<span>UPLOAD FILES</span></a></li -->
			
			<li><a href="<%=Constants.getRootWeb()+"/" %>InnerFrame/TabPengaturan/Main.jsp?id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>&cmd=pengaturan" target="_self" class="active">PENGATURAN<span>SETTING</span></a></li>
			<%
		}
		else {
			%>
			<li><a href="<%=Constants.getRootWeb()+"/" %>InnerFrame/TabPengaturan/Main.jsp?id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>&cmd=pengaturan" target="_self" >PENGATURAN<span>SETTING</span></a></li>
			<%
		}	
	}
	*/
	//validUsr.
	//isUsrAllowTo_updated(String command_code, String target_npm)
	/*
	copy paste dari /ToUnivSatyagama/WebContent/InnerFrame/Akademik/IndexAkademikSubMenu0.jsp
	cuma disini kita harus masking agar update ditujukan untuk npmhs terkait bukan usr.
	*/
	if(validUsr.isUsrAllowTo_updated("mba", v_npmhs) && !keluar && !dropout && !isObjTamu) {	
	//if(validUsr.isAllowTo("mba")>0) {
	//if(true) {
		String target = Constants.getRootWeb()+"/InnerFrame/Akademik/dashKurikulum.jsp";
		String uri = request.getRequestURI();
		String url = PathFinder.getPath(uri, target);
		if(cmd!=null && cmd.equalsIgnoreCase("mba")) {
	%>		
		<li><a href="get.listScope?scope=mba&callerPage=&atMenu=mba&cmd=mba&scopeType=prodiOnly&id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>" target="_self" class="active">BAHAN<span>AJAR</b></span></a></li>		
	<%	
		}
		else {
	%>		
		<li><a href="get.listScope?scope=mba&callerPage=&atMenu=mba&cmd=mba&scopeType=prodiOnly&id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>" target="_self">BAHAN<span>AJAR</b></span></a></li>		
	<%
		}	
	}	
	
	if(!validUsr.getObjNickNameGivenObjId().contains("MHS") && validUsr.isUsrAllowTo("hasUpDownFileMhs", v_npmhs, v_obj_lvl)) {		
	
		if(cmd!=null && cmd.equalsIgnoreCase("arsip")) {
	%>		
		<li><a href="prep.personalArsipFolder?scope=hasUpDownFileMhs&callerPage=&atMenu=arsip&cmd=arsip&scopeType=prodiOnly&id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>" target="_self" class="active">FILE<span>ARSIP</b></span></a></li>		
	<%	
		}
		else {
	%>		
		<li><a href="prep.personalArsipFolder?scope=hasUpDownFileMhs&callerPage=&atMenu=arsip&cmd=arsip&scopeType=prodiOnly&id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>" target="_self">FILE<span>ARSIP</b></span></a></li>		
	<%
		}	
	}	
	//http://localhost:8080/com.otaku.rest/api/v1/citcat/dashboard/0000312100006/allow_send_msg_to/target_kdpst/74201/target_kmp/PST
	boolean boleh = false;
	
	JSONArray jsoa = Getter.readJsonArrayFromUrl("/v1/citcat/dashboard/"+validUsr.getNpm()+"/allow_send_msg_to/target_kdpst/"+kdpst+"/target_kmp/"+Getter.getDomisiliKampus(v_npmhs));
	try {
		...
		if(jsoa!=null && jsoa.length()>0) {
			for(int i=0;i<jsoa.length();i++) {
				...
				try {
					boleh = Boolean.parseBoolean((String)job.get("BOLEH"));
				}
	...
	//System.out.println("boleh="+boleh);
	if(boleh && !validUsr.iAmStu()) {		//khusus operator kalo mhs dari index
		
		if(cmd!=null && cmd.equalsIgnoreCase("msg")) {
	%>		
		<li><a href="go.prepYourRole?atMenu=msg&cmd=msg&id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>" target="_self" class="active">KIRIM<span>PESAN</b></span></a></li>		
	<%	
		}
		else {
	%>		
		<li><a href="go.prepYourRole?atMenu=msg&cmd=msg&&id_obj=<%=v_id_obj%>&nmm=<%=v_nmmhs%>&npm=<%=v_npmhs %>&obj_lvl=<%=v_obj_lvl %>&kdpst=<%=v_kdpst %>" target="_self">KIRIM<span>PESAN</b></span></a></li>		
	<%
		}	
	}	
